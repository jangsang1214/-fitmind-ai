/* GARANG Today Action Flow v1.1
   C direction: no-body hero + conditional body evidence.
...
function deriveModel(state,options={}){
  ...
  const signalBase=recoveryScore!==null?recoveryScore:(overallScore!==null?overallScore:progress),signalScore=signalBase===null?0:clamp(Math.round(signalBase),0,100),bodyEvidence=reasonCodes.includes('HIGH_SORENESS')||(soreness!==null&&soreness>=4);
  return {version:'garang-today-action-flow-v1.1',date,lang,planned,executed,remaining,progress,headline,support,cta,route,action,nextTitle:String(incomplete?.title||''),decisionLine,decisionReason,stateLabel,stateValue,stateUnit,mode,recoveryScore,overallScore,memoryCount,detailReasons,signalScore,bodyEvidence,soreness,rows:[
...
function markup(model,expanded=false){
  ...
  return `<section id="garangTodayFlow" class="gtf gtf-luxury" data-version="${model.version}" data-decision-mode="${esc(model.mode)}" data-body-evidence="${model.bodyEvidence?'1':'0'}" style="--gtf-signal:${model.signalScore}" aria-label="${model.lang==='en'?'Today decision':'오늘의 판단'}">
    <div class="gtf-state" aria-label="${model.lang==='en'?'Current state':'현재 상태'}"><div class="gtf-state-copy"><span>STATE · ${esc(dateLabel)}</span><strong>${esc(model.stateLabel)}</strong></div><div class="gtf-state-visual"><span class="gtf-signal" aria-hidden="true"><i></i></span><p><b>${esc(model.stateValue)}</b><small>${esc(model.stateUnit)}</small></p></div></div>
...
return Object.freeze({version:'garang-today-action-flow-v1.1.0',deriveModel,markup,mount,todayLocal});
});