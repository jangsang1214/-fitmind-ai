/* GARANG Today Action Flow v1.3.1
...
function deriveTracks(day,safe,date,lang){
  const domains=day?.plan?.domains&&typeof day.plan.domains==='object'?day.plan.domains:{},planItems=list(day?.plan?.items),draft=safe?.meta?.dailyPlanDrafts?.[date],draftItems=list(draft?.items);
  return TRACK_DOMAINS.map(domain=>{
    const row=domains?.[domain]||{},planned=Math.max(0,Number(row?.planned)||0),executed=Math.max(0,Number(row?.executed)||0),rawRate=finite(row?.rate),draftItem=draftItems.find((item,index)=>trackDomain(item,index)===domain),planItem=planItems.find((item,index)=>trackDomain(item,index)===domain);
    ...
    const title=String((draft?.status==='draft'?draftItem?.title:null)||planItem?.title||'').trim();
    return {domain,label:trackLabel(domain,lang),planned,executed,rate,state,value,title};
  });
}
const MODE_COPY={
...
function mount(root){
  ...
  function renderKey(value){let hash=2166136261;for(let i=0;i<value.length;i++){hash^=value.charCodeAt(i);hash=Math.imul(hash,16777619);}return `${value.length}:${hash>>>0}`;}
  ...
  function render(){scheduled=false;const current=main.querySelector('#garangTodayFlow');if(!isToday()){if(current)current.remove();expanded=false;clearCMode();return;}const snapshot=state();if(!snapshot)return;const model=deriveModel(snapshot,{lang:doc.documentElement.lang==='en'?'en':'ko',PlanExecution:root.GarangPlanExecution,StateIntelligence:root.GarangStateIntelligence,DecisionIntelligence:root.GarangDecisionIntelligence,PerformanceScore:root.GarangPerformanceScore,date:todayLocal()}),html=markup(model,expanded),key=renderKey(html);if(current?.dataset?.gtfRenderKey===key){syncCMode(model);bindGoldenPath(current);try{root.GarangProductConsolidationV1?.reconcileNow?.();}catch{}try{root.GarangTodaySingleNextActionV1?.syncNow?.();}catch{}return;}const host=doc.createElement('div');host.innerHTML=html;const next=host.firstElementChild;if(!next)return;next.dataset.gtfRenderKey=key;if(current){carryOrchestratedState(current,next);current.replaceWith(next);}else{const head=main.querySelector(':scope > .page-head');if(head)head.insertAdjacentElement('afterend',next);else main.prepend(next);}syncCMode(model);bindGoldenPath(next);try{root.GarangProductConsolidationV1?.reconcileNow?.();}catch{}try{root.GarangTodaySingleNextActionV1?.syncNow?.();}catch{}}
...
return Object.freeze({version:'garang-today-action-flow-v1.3.1',deriveModel,markup,mount,todayLocal});
});