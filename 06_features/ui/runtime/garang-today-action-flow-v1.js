...
function mount(root){
  ...
  function render(){scheduled=false;const current=main.querySelector('#garangTodayFlow');if(!isToday()){if(current)current.remove();expanded=false;clearCMode();return;}const snapshot=state();if(!snapshot)return;const model=deriveModel(snapshot,{lang:doc.documentElement.lang==='en'?'en':'ko',PlanExecution:root.GarangPlanExecution,StateIntelligence:root.GarangStateIntelligence,DecisionIntelligence:root.GarangDecisionIntelligence,PerformanceScore:root.GarangPerformanceScore,date:todayLocal()}),html=markup(model,expanded),key=renderKey(html);if(current?.dataset?.gtfRenderKey===key){syncCMode(model);bindGoldenPath(current);return;}const host=doc.createElement('div');host.innerHTML=html;const next=host.firstElementChild;if(!next)return;next.dataset.gtfRenderKey=key;if(current){carryOrchestratedState(current,next);current.replaceWith(next);}else{const head=main.querySelector(':scope > .page-head');if(head)head.insertAdjacentElement('afterend',next);else main.prepend(next);}syncCMode(model);bindGoldenPath(next);}
