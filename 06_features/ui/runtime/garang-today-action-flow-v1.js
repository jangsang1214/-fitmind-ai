...
function deriveModel(state,options={}){
  ...
  const planned=Math.max(0,Number(day?.plan?.planned)||0),executed=clamp(Math.max(0,Number(day?.plan?.executed)||0),0,planned||Number.MAX_SAFE_INTEGER),remaining=Math.max(0,planned-executed),dailyDraft=safe?.meta?.dailyPlanDrafts?.[date]?.status==='draft';
  ...
  if(planned===0){route='planner';cta=dailyDraft?(lang==='en'?'Apply today plan and start':'오늘 계획 반영하고 시작'):(lang==='en'?'Review today plan':'오늘 계획 보기');headline=lang==='en'?'Set today’s direction first.':'오늘의 방향부터 정리하세요.';support=lang==='en'?'A plan connects your records to the next decision.':'계획이 있어야 기록이 다음 판단으로 연결됩니다.';}
...
function markup(model,expanded=false){
  ...
  const actionAttrs=model.dailyDraft&&model.route==='planner'?'data-gdp-confirm-today':model.route?`data-gtf-route="${esc(model.route)}"${model.route==='planner'?' data-golden-path="planner-entry"':''}`:`data-gtf-action="${esc(model.action||'')}"`,dateLabel=String(model.date||'').slice(5).replace('-','.');
