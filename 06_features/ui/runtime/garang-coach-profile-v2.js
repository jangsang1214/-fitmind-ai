/* GARANG Coach Profile v2
   - Single owner for the read-only Coach understanding panel and every profile entry point.
   - Normalizes legacy/current check-in aliases before State/Decision Intelligence reads them.
   - Does not rewrite the decision card, prompt strip, message text, thread state, or route state.
*/
(() => {
'use strict';
...
if(!Bridge0||!StateEngine||!DecisionEngine||window.__garangCoachProfileV2)return;
window.__garangCoachProfileV2=true;
const VERSION='garang-coach-profile-v2.0.0';
...
function canonicalCheckins(state){
 const sources=[{list:rows(state?.dailyCheckins),priority:1},{list:rows(state?.checkins),priority:2}];
 const byDate=new Map(),undated=[];
 for(const source of sources){for(const raw of source.list){
...
function intelligenceState(){const state=clone(Bridge0.getState());const checkins=canonicalCheckins(state);state.dailyCheckins=checkins;state.checkins=checkins;return state;}
function memoryContext(){try{return Bridge0.getMemoryContext?.('',{limit:24,budgetChars:6000})||null;}catch{return null;}}
...
const MODE={collect_data:{ko:'데이터 필요',en:'More data needed'},caution:{ko:'주의',en:'Caution'},recover:{ko:'회복 우선',en:'Recovery first'},reduce:{ko:'강도 조정',en:'Reduce load'},maintain:{ko:'계획 유지',en:'Maintain'},progress:{ko:'점진 향상',en:'Progress'},goal_focus:{ko:'목표 집중',en:'Goal focus'}};
...
function memoryRows(state){const active=rows(state.memory?.entries).filter(x=>x.status!=='superseded'&&x.status!=='expired'&&x.userConfirmed!==false);const priority=x=>String(x.type||'')==='goal'?0:String(x.memoryClass||x.type||'')==='preference'?1:2;return active.slice().sort((a,b)=>priority(a)-priority(b)||Number(b.importance||0)-Number(a.importance||0)).slice(0,5);}
function panelHTML(){
 ...
 const value=(n,suffix='')=>n==null?'—':`${n}${suffix}`;const metric=(label,valueText,band)=>`<div class="gcp-metric"><small>${label}</small><b>${esc(valueText)}</b><span style="font-size:10px;opacity:.58">${esc(bandLabel(band))}</span></div>`;const data=(label,n)=>`<div class="gcp-data"><b>${n}</b><small>${label}</small></div>`;
 return `<div class="gcp-head"><div class="gcp-head-copy"><small>GARANG COACH</small><h2>${english()?'How GARANG understands you':'GARANG이 나를 이해하는 방식'}</h2></div><button type="button" class="gcp-close" aria-label="${english()?'Close':'닫기'}">×</button></div><section class="gcp-decision"><div class="gcp-decision-top"><strong>${esc(modeLabel(d?.mode||'collect_data'))}</strong><span>${english()?'Confidence':'판단 신뢰도'} ${confidence}%</span></div><p class="gcp-summary">${esc(summary||(english()?'GARANG is still collecting enough evidence for a stable decision.':'GARANG이 안정적인 판단을 위해 데이터를 확인하고 있어.'))}</p></section><section class="gcp-grid">${metric(english()?'Readiness':'준비도',value(signals.readinessValue,'/100'),signals.readinessBand)}${metric(english()?'Fatigue':'피로',value(signals.fatigueScore,'/100'),signals.fatigueBand)}${metric(english()?'Training load':'훈련 부하',signals.loadRatio==null?'—':`${Number(signals.loadRatio).toFixed(2)}×`,signals.loadBand)}${metric(english()?'Goal alignment':'목표 정렬',value(signals.goalScore,'/100'),signals.goalBand)}</section><section class="gcp-section"><h3>${english()?'DATA GARANG IS USING':'GARANG이 참고 중인 데이터'}</h3><div class="gcp-data-grid">${data(english()?'Workouts':'운동',c.workouts)}${data(english()?'Check-ins':'체크인',c.checkins)}${data(english()?'Meals':'식단',c.meals)}${data(english()?'Runs':'러닝',c.runs)}${data(english()?'Body':'신체',c.body)}${data(english()?'Memories':'기억',c.memory)}</div></section><section class="gcp-section"><h3>${english()?'CURRENT UNDERSTANDING':'현재 기억 중'}</h3><div class="gcp-list">${goal?`<div class="gcp-row"><b>${english()?'Primary goal':'현재 목표'}</b><span>${esc(goal)}</span></div>`:''}${mem.map(x=>`<div class="gcp-row"><b>${esc(String(x.type||x.memoryClass||'memory'))}</b><span>${esc(String(x.value||x.content||''))}</span></div>`).join('')||`<div class="gcp-row"><span>${english()?'No confirmed long-term memory yet.':'확인된 장기 기억이 아직 없어.'}</span></div>`}</div></section><section class="gcp-section"><h3>${english()?'RECENT PATTERNS':'최근 감지 패턴'}</h3><div class="gcp-list">${patterns.map(p=>`<div class="gcp-row"><b>${esc(String(p.id||'pattern').replaceAll('_',' '))}</b><span>${esc(String(p.summary||''))}</span></div>`).join('')||`<div class="gcp-row"><span>${english()?'No strong recent pattern detected yet.':'아직 강하게 감지된 최근 패턴은 없어.'}</span></div>`}</div></section><div class="gcp-note">${english()?'This profile shows the evidence GARANG is using. Opening it never changes your data.':'이 프로필은 GARANG이 참고하는 근거를 보여줘. 이 화면을 열어도 어떤 데이터도 변경되지 않아.'}</div>`;
}
...
function bindHeader(root){const head=root.querySelector('.g2-chat-head');if(!head)return;let trigger=head.querySelector('.gcp-profile-trigger');if(!trigger){const logo=head.querySelector('.garang-code-mark,.garang-exact-logo');if(!logo)return;trigger=document.createElement('button');trigger.type='button';trigger.className='gcp-profile-trigger';logo.replaceWith(trigger);trigger.appendChild(logo);trigger.addEventListener('click',()=>openPanel(root));}trigger.setAttribute('aria-label',english()?'Open GARANG Coach profile':'GARANG 코치 프로필 열기');const copy=head.querySelector('.g2-chat-head-copy');if(copy&&!copy.querySelector('.gcp-profile-hint')){const hint=document.createElement('span');hint.className='gcp-profile-hint';hint.textContent='›';copy.querySelector('small')?.appendChild(hint);}copy?.querySelector('small')?.setAttribute('title',english()?'Tap the GARANG mark for Coach profile':'GARANG 로고를 눌러 코치 프로필 보기');}
function avatarContainers(root){const direct=[...root.querySelectorAll('.g2-message.assistant .g2-avatar,.g2-message.assistant .gpt-avatar,.gpt-message.assistant .gpt-avatar')];const imageParents=[...root.querySelectorAll('.g2-message.assistant img,.gpt-message.assistant img')].map(img=>img.closest('.g2-avatar,.gpt-avatar')||img);return [...new Set([...direct,...imageParents])];}
function bindAvatars(root){for(const avatar of avatarContainers(root)){if(avatar.dataset.gcpBound==='1')continue;avatar.dataset.gcpBound='1';avatar.classList.add('gcp-coach-avatar');avatar.setAttribute('role','button');avatar.setAttribute('tabindex','0');avatar.setAttribute('aria-label',english()?'Open GARANG Coach details':'GARANG 코치 상세 보기');avatar.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();openPanel(root);});avatar.addEventListener('keydown',event=>{if(event.key!=='Enter'&&event.key!==' ')return;event.preventDefault();openPanel(root);});}}
let queued=false;function run(){queued=false;const root=main.querySelector('.garang-coach-v2');if(!root)return;ensureStyle();bindHeader(root);bindAvatars(root);}
function queue(){if(queued)return;queued=true;requestAnimationFrame(run);}
window.addEventListener('garang:screen-rendered',queue);window.addEventListener('garang:coach-mounted',queue);window.addEventListener('garang:coach-message-rendered',queue);window.addEventListener('garang:state-hydrated',queue);window.addEventListener('garang:agent-write',queue);window.addEventListener('garang:agent-proposal-resolved',queue);window.addEventListener('garang:sync-durable',queue);new MutationObserver(queue).observe(document.documentElement,{attributes:true,attributeFilter:['lang']});document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;const root=main.querySelector('.garang-coach-v2');if(root?.classList.contains('gcp-open'))closePanel(root);});
window.GarangCoachProfile=Object.freeze({version:VERSION,canonicalCheckins,stateForIntelligence:()=>clone(intelligenceState()),open:()=>{const root=main.querySelector('.garang-coach-v2');if(root)openPanel(root);}});
queue();
})();
