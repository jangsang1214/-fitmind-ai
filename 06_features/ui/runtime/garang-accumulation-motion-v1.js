...
#main[data-garang-screen="today"] .gtd3-ripple{position:relative;isolation:isolate;overflow:hidden!important;height:118px!important;margin-top:5px!important;background:radial-gradient(ellipse at 50% 67%,rgba(242,239,233,.018),rgba(8,9,8,0) 55%)!important}
...
@media(max-width:390px){#main[data-garang-screen="today"] .gtd3-ripple{height:112px!important}}
@media(min-width:760px){#main[data-garang-screen="today"] .gtd3-ripple{height:166px!important}}
...
`;doc.head.appendChild(style);
  ...
  function reflectiveDrop(ctx,x,y,r,stretch,alpha,tilt=0){
    ...
    ctx.beginPath();ctx.moveTo(rr*.29,-rr*.05);ctx.quadraticCurveTo(rr*.48,rr*.26,rr*.23,rr*.55);ctx.stroke();
    ctx.restore();
  }
  ...
  function reflectiveBowl(ctx,cx,cy,rx,ry,alpha,variation){
    ...
    softGlow(ctx,cx,cy,rx*.82,ry*2.3,alpha*.42,'silver');
    const g=ctx.createLinearGradient(cx-rx,cy,cx+rx,cy);
    g.addColorStop(0,'rgba(235,234,228,0)');
    g.addColorStop(.17,`rgba(205,207,202,${alpha*.14})`);
    g.addColorStop(.43,`rgba(246,245,239,${alpha*.64})`);
    g.addColorStop(.58,`rgba(116,161,146,${alpha*.18})`);
    g.addColorStop(.83,`rgba(224,225,220,${alpha*.12})`);
    g.addColorStop(1,'rgba(240,239,233,0)');
    ctx.strokeStyle=g;ctx.lineWidth=.85;
    ellipsePath(ctx,cx,cy,rx,ry,.015+variation*.004,variation*TAU,Math.PI*.02,Math.PI*.98,60);ctx.stroke();
    ctx.restore();
  }

  function brokenRipple(ctx,cx,cy,rx,ry,alpha,width,phase,index,variation){
    if(alpha<=.002)return;
    const warp=.008+index*.0025+variation*.004;
    ctx.save();
    ctx.globalCompositeOperation='screen';

    ctx.strokeStyle=`rgba(176,181,178,${alpha*.11})`;ctx.lineWidth=Math.max(.45,width*.7);
    ellipsePath(ctx,cx,cy,rx,ry,warp,phase,0,TAU,92);ctx.stroke();

    const silver=ctx.createLinearGradient(cx-rx,cy,cx+rx,cy);
    silver.addColorStop(0,'rgba(245,244,238,0)');
    silver.addColorStop(.25,`rgba(220,221,216,${alpha*.17})`);
    silver.addColorStop(.55,`rgba(249,248,242,${alpha*.76})`);
    silver.addColorStop(.82,`rgba(181,185,182,${alpha*.2})`);
    silver.addColorStop(1,'rgba(245,244,238,0)');
    ctx.strokeStyle=silver;ctx.lineWidth=width;ctx.shadowColor=`rgba(235,235,229,${alpha*.12})`;ctx.shadowBlur=3.2;
    const a0=.12*Math.PI+(variation-.5)*.16;
    const a1=.8*Math.PI+index*.035;
    ellipsePath(ctx,cx,cy,rx,ry,warp,phase,a0,a1,54);ctx.stroke();

    ctx.shadowBlur=0;ctx.strokeStyle=`rgba(121,174,157,${alpha*.18})`;ctx.lineWidth=Math.max(.45,width*.62);
    const j0=1.06*Math.PI+index*.04,j1=1.42*Math.PI+(variation-.5)*.1;
    ellipsePath(ctx,cx,cy,rx,ry,warp,phase,j0,j1,28);ctx.stroke();

    ctx.strokeStyle=`rgba(242,241,235,${alpha*.34})`;ctx.lineWidth=Math.max(.42,width*.48);
    const b0=1.55*Math.PI+index*.025,b1=1.84*Math.PI+variation*.035;
    ellipsePath(ctx,cx,cy,rx,ry,warp,phase,b0,b1,24);ctx.stroke();
    ctx.restore();
  }

  function impactColumn(ctx,cx,waterY,w,h,pulse,variation){
    if(pulse<=.002)return;
    const height=h*(.14+.12*pulse),half=Math.max(2,w*(.009+.004*variation));
    ctx.save();ctx.globalCompositeOperation='screen';
    const g=ctx.createLinearGradient(cx,waterY-height,cx,waterY);
    g.addColorStop(0,`rgba(248,247,241,${.28*pulse})`);
    g.addColorStop(.22,`rgba(187,195,190,${.2*pulse})`);
    g.addColorStop(.55,`rgba(28,39,35,${.08*pulse})`);
    g.addColorStop(.82,`rgba(115,166,150,${.12*pulse})`);g.addColorStop(1,'rgba(238,238,233,0)');
    ctx.fillStyle=g;
    ...
    ctx.strokeStyle=`rgba(247,246,240,${.18*pulse})`;ctx.lineWidth=.55;ctx.beginPath();ctx.moveTo(cx-.6,waterY-height*.84);ctx.quadraticCurveTo(cx-half*.32,waterY-height*.46,cx-half*.22,waterY-height*.2);ctx.stroke();
    ctx.restore();
  }

  function drawHero(surface,time,staticOnly=false){
    fit(surface);const {ctx,w,h}=surface;ctx.clearRect(0,0,w,h);if(w<2||h<2)return;
    const cycle=MOBILE?4.45:4.9;
    const elapsed=Math.max(0,(time-surface.startedAt)/1000);
    const cycleIndex=Math.floor(elapsed/cycle);
    const phase=staticOnly?.64:(elapsed%cycle)/cycle;
    const variation=staticOnly?.43:seeded(cycleIndex+1.7);
    const drift=(variation-.5)*w*.035;
    const cx=w*.51+drift,waterY=h*.67;

    inkPool(ctx,w,h,cx,waterY,phase>.34?1:.35,variation);
    softGlow(ctx,cx+w*.05,waterY+h*.01,w*.18,h*.2,.028,'silver');

    if(phase<.37){
      const t=smooth(phase/.37),fall=easeIn(t),y=lerp(h*.05,waterY-h*.105,fall),speed=clamp((t-.18)/.82,0,1);
      const r=Math.max(8.2,Math.min(12.4,w*.0275));
      const stretch=1+.46*speed;
      softGlow(ctx,cx,y,r*2.3,r*2.6,.065,'silver');
      reflectiveDrop(ctx,cx,y,r,stretch,.93,(variation-.5)*.035);
      reflectiveBowl(ctx,cx,waterY,w*.07,h*.025,.045*t,variation);
      return;
    }

    const impactT=clamp((phase-.37)/.14,0,1);
    const pulse=Math.sin(impactT*Math.PI);
    const rippleT=clamp((phase-.37)/.63,0,1);
    softGlow(ctx,cx,waterY,w*(.085+.08*pulse),h*(.16+.08*pulse),.12*pulse,'silver');
    softGlow(ctx,cx+w*.025,waterY,w*.1,h*.11,.035*pulse,'jade');
    reflectiveBowl(ctx,cx,waterY,w*(.055+.045*pulse),h*(.018+.014*pulse),.42*pulse,variation);

    if(phase<.53){
      const crown=pulse;
      ctx.save();ctx.globalCompositeOperation='screen';ctx.strokeStyle=`rgba(236,236,230,${.2*crown})`;ctx.lineWidth=.65;
      const base=w*(.032+.02*impactT),peak=h*(.085+.065*crown);
      ctx.beginPath();ctx.moveTo(cx-base,waterY);ctx.bezierCurveTo(cx-base*.62,waterY-peak*.12,cx-base*.43,waterY-peak*.42,cx-base*.2,waterY-peak*.16);ctx.quadraticCurveTo(cx,waterY-peak,cx+base*.18,waterY-peak*.18);ctx.bezierCurveTo(cx+base*.45,waterY-peak*.44,cx+base*.68,waterY-peak*.1,cx+base,waterY);ctx.stroke();ctx.restore();
      impactColumn(ctx,cx+(variation-.5)*1.5,waterY,w,h,crown,variation);
    }

    for(let i=0;i<5;i++){
      const delay=i*.092;
      const rt=clamp((rippleT-delay)/(1-delay),0,1);if(rt<=0)continue;
      const e=easeOut(rt);
      const rx=w*(.045+i*.027)+w*(.33+i*.023)*e;
      const ry=rx*(.175+i*.004);
      const alpha=(.31-i*.041)*Math.pow(1-rt,1.14);
      const xShift=((i%2?1:-1)*(1.2+i*.35))+(variation-.5)*i*1.1;
      brokenRipple(ctx,cx+xShift,waterY+(i-2)*.28,rx,ry,alpha,i<2?.92:.68,phase*TAU+i*.71,variation);
    }

    if(rippleT>.12&&rippleT<.52){
      const linger=Math.sin(clamp((rippleT-.12)/.4,0,1)*Math.PI);
      const offset=w*(.07+.11*rippleT);
      softGlow(ctx,cx-offset,waterY+h*.012,w*.055,h*.06,.027*linger,'jade');
    }
  }

  function drawScore(surface,time,staticOnly=false){
    fit(surface);const {ctx,w,h}=surface;ctx.clearRect(0,0,w,h);if(w<2||h<2)return;
    const elapsed=Math.max(0,(time-surface.startedAt)/1000),t=staticOnly?.34:(elapsed%6.4)/6.4;
    const y=h*.72,variation=.38;
    ctx.save();ctx.globalCompositeOperation='screen';
    const sweep=w*(.04+.92*t);
    softGlow(ctx,sweep,y,Math.max(10,w*.18),Math.max(6,h*.15),.025,'silver');
    ctx.strokeStyle='rgba(238,237,231,.075)';ctx.lineWidth=.5;
    ellipsePath(ctx,w*.5,y,w*.44,h*.08,.008,variation+t*TAU,Math.PI*.04,Math.PI*.96,58);ctx.stroke();
    ctx.strokeStyle='rgba(121,174,157,.055)';ctx.lineWidth=.45;
    ellipsePath(ctx,w*.5,y+2,w*.34,h*.055,.012,variation-t*TAU*.7,Math.PI*.1,Math.PI*.84,48);ctx.stroke();
    ctx.restore();
  }

  function drawCheckin(surface,time,staticOnly=false){
    fit(surface);const {ctx,w,h}=surface;ctx.clearRect(0,0,w,h);if(w<2||h<2)return;
    const elapsed=Math.max(0,(time-surface.startedAt)/1000),t=staticOnly?.31:(elapsed%7.2)/7.2;
    const baseY=h*.72;
    ctx.save();ctx.globalCompositeOperation='screen';
    ctx.strokeStyle='rgba(235,234,228,.035)';ctx.lineWidth=.5;
    ctx.beginPath();
    for(let x=w*.48;x<=w+5;x+=6){const nx=x/Math.max(1,w),py=baseY+Math.sin(nx*TAU*1.08-t*TAU)*1.25+Math.sin(nx*TAU*2.3+t*TAU*.4)*.42;if(x===w*.48)ctx.moveTo(x,py);else ctx.lineTo(x,py);}ctx.stroke();
    ctx.strokeStyle='rgba(121,174,157,.024)';ctx.beginPath();
    for(let x=w*.56;x<=w+5;x+=7){const nx=x/Math.max(1,w),py=baseY+5+Math.sin(nx*TAU*.96-t*TAU*.83+.7)*1.05;if(x===w*.56)ctx.moveTo(x,py);else ctx.lineTo(x,py);}ctx.stroke();
    softGlow(ctx,w*(.6+.34*t),baseY,Math.max(18,w*.085),Math.max(9,h*.17),.014,'jade');
    ctx.restore();
  }

  function draw(surface,time,staticOnly=false){
    if(surface.type==='hero')drawHero(surface,time,staticOnly);
    else if(surface.type==='score')drawScore(surface,time,staticOnly);
    else drawCheckin(surface,time,staticOnly);
  }
  ...
  function sync(){
    ...
    m.dataset.garangMotionArt=ART_DIRECTION;
    m.dataset.garangMotionPalette=PALETTE;
  ...
  function tick(time){
    ...
    if(time-lastFrame>=FRAME_MS){lastFrame=time;drawActive(time,false);}
    raf=requestAnimationFrame(tick);
  }
  function schedule(){if(raf||reduced()||!todayActive())return;raf=requestAnimationFrame(tick);}
  function armWatchdog(){
    clearTimeout(watchdogTimer);if(reduced()||!todayActive())return;
    watchdogTimer=setTimeout(()=>{if(reduced()||!todayActive())return;const now=performance.now();if(now-lastPaint>240)drawActive(now,false);schedule();armWatchdog();},IN_APP?120:220);
  }
  ...
  window.addEventListener('visibilitychange',()=>{if(doc.hidden&&!IN_APP)stop();else lifecycleSync();});
  ...
  const api=Object.freeze({
    ...
    sync:queueSync,
    stop,
    get activeSurfaces(){return [...surfaces.values()].filter(x=>x.host.isConnected).map(x=>x.type);},
    get reducedMotion(){return reduced();},
    get inAppBrowser(){return IN_APP;},
    get lastPaintAt(){return lastPaint;}
  });
  window.GarangAccumulationMotionV1=api;
  window.GarangAccumulationMotionV4=api;
  lifecycleSync();
})();