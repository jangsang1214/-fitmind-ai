...
(() => {
  ...
  function getState(){try{return window.GarangAgentStateBridge?.getLiveState?.()||window.GarangAgentStateBridge?.getState?.()||null;}catch{return null;}}
...
#main[data-garang-screen="today"] .gac5-media--idle[data-ready="1"]{opacity:1}
#main[data-garang-screen="today"] .gac5-media--event{z-index:2;opacity:0}
#main[data-garang-screen="today"] .gac5-media--event[data-ready="1"]{opacity:0}
#main[data-garang-screen="today"] .gac5-media--event[data-playing="1"]{opacity:1!important}
...
`;
  ...
  async function loadManifest(){
    ...
    manifestPromise=fetch(MANIFEST_URL,{cache:'no-cache'})
      ...
      .then(value=>{manifest=value&&typeof value==='object'?value:{};assetStatus=manifest.status==='ready'?'ready':String(manifest.status||'production-required');return manifest;})
  ...
  function makeVideo(role){
    ...
    video.preload=role==='idle'?'metadata':'none';video.tabIndex=-1;
    return video;
  }
  ...
  function sourceVideo(video,entry,{loop=false,preload='metadata'}={}){
    ...
    video.loop=!!loop;video.preload=preload;video.dataset.ready='0';video.dataset.playing='0';video.load();return true;
  }

  function setPoster(surface){
    const poster=resolveAsset(manifest?.hero?.poster);if(!poster)return;
    surface.shell.style.backgroundImage=`linear-gradient(180deg,rgba(5,6,5,.18),rgba(5,6,5,.38)),url("${poster.replace(/"/g,'%22')}")`;
    surface.shell.style.backgroundSize='cover';surface.shell.style.backgroundPosition='50% 58%';
  }

  function bindMedia(surface){
    if(surface.bound)return;surface.bound=true;
    ...
    const settle=()=>{event.dataset.playing='0';event.pause();try{event.currentTime=0;}catch{}surface.eventName='';if(todayActive()&&!reduced())idle.play().catch(()=>{});};
    event.addEventListener('ended',settle);event.addEventListener('error',settle);
  }

  function makeSurface(host){
    if(!host)return null;
    const current=surfaces.get(host);if(current&&current.shell.isConnected)return current;
    ...
    const idle=makeVideo('idle'),event=makeVideo('event');shell.append(fallback,idle,event);host.appendChild(shell);
    const surface={host,shell,fallback,idle,event,bound:false,configured:false,eventName:''};bindMedia(surface);surfaces.set(host,surface);return surface;
  }

  function removeStale(){for(const [host,surface] of surfaces){if(host.isConnected)continue;surface.idle.pause();surface.event.pause();surface.shell.remove();surfaces.delete(host);}}

  function configureSurface(surface){
    if(!surface||surface.configured)return;surface.configured=true;setPoster(surface);
    if(assetStatus!=='ready'){surface.host.dataset.garangCinematicReady='0';return;}
    const idleEntry=manifest?.hero?.idle;
    if(!sourceVideo(surface.idle,idleEntry,{loop:true,preload:SAVE_DATA?'metadata':'auto'})){surface.host.dataset.garangCinematicReady='0';return;}
  ...
  function playEvent(name='deposit'){
    ...
    const surface=[...surfaces.values()].find(item=>item.host.isConnected);if(!surface)return false;
    const entry=eventEntry(name);if(!entry)return false;
    const sourceKey=`${entry.webm||''}|${entry.mp4||''}`;
    if(surface.event.dataset.sourceKey!==sourceKey){if(!sourceVideo(surface.event,entry,{loop:false,preload:'auto'}))return false;surface.event.dataset.sourceKey=sourceKey;}
    surface.eventName=name;surface.event.dataset.playing='0';try{surface.event.currentTime=0;}catch{}
    surface.idle.pause();
  ...
  function markRoot(){
    ...
    m.dataset.garangMotion=reduced()?'reduced':'active';m.dataset.garangMotionQuality=QUALITY;m.dataset.garangMotionRenderer=RENDERER;m.dataset.garangCinematicStatus=assetStatus;
  }

  async function sync(){
    queued=false;ensureStyle();removeStale();
    const m=main();if(!m||m.dataset.garangScreen!=='today')return;
    const host=m.querySelector('.gtd3-ripple');if(!host){clearTimeout(retryTimer);retryTimer=setTimeout(queueSync,120);return;}
    const surface=makeSurface(host);markRoot();await loadManifest();markRoot();if(!surface.host.isConnected)return;
    configureSurface(surface);syncRecordState();
    if(reduced()){surface.idle.pause();surface.event.pause();surface.event.dataset.playing='0';setPoster(surface);}
    else if(assetStatus==='ready'&&todayActive()&&surface.event.dataset.playing!=='1')surface.idle.play().catch(()=>{});
  }
  ...
  function resume(){if(!todayActive()||reduced())return;for(const surface of surfaces.values())if(surface.host.isConnected&&assetStatus==='ready'&&surface.event.dataset.playing!=='1')surface.idle.play().catch(()=>{});}

  reducedQuery?.addEventListener?.('change',lifecycleSync);
  window.addEventListener('resize',lifecycleSync,{passive:true});window.addEventListener('orientationchange',lifecycleSync,{passive:true});
  ...
  window.addEventListener('garang:screen-rendered',lifecycleSync);window.addEventListener('garang:state-updated',lifecycleSync);window.addEventListener('garang:state-hydrated',lifecycleSync);window.addEventListener('garang:route-completed',lifecycleSync);window.addEventListener('garang:workout-intelligence-rendered',lifecycleSync);

  const api=Object.freeze({
    version:VERSION,quality:QUALITY,renderer:RENDERER,manifestUrl:MANIFEST_URL,sync:queueSync,stop,resume,playEvent,
    get assetStatus(){return assetStatus;},
    get activeSurfaces(){return [...surfaces.values()].filter(x=>x.host.isConnected).map(()=> 'hero');},
    get reducedMotion(){return reduced();},get inAppBrowser(){return IN_APP;},get mobile(){return MOBILE;},get lastPlaybackAt(){return lastPlaybackAt;}
  });
  window.GarangAccumulationMotionV1=api;window.GarangAccumulationMotionV5=api;lifecycleSync();
})();