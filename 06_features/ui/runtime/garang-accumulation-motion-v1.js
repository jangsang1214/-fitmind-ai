...
`;
  ...
  function drawHero(surface,time,staticOnly=false){
    ...
    const seed=surface.seed,cycle=4.35+seeded(seed)*1.15,now=staticOnly?cycle*.66:(time/1000+seeded(seed+3)*cycle)%cycle,impact=cycle*.43,cx=w*.52,waterY=h*.73;
    const ambient=ctx.createLinearGradient(0,0,w,h);
    ambient.addColorStop(0,'rgba(9,31,26,0)');ambient.addColorStop(.48,'rgba(18,62,52,.12)');ambient.addColorStop(1,'rgba(92,171,147,.055)');ctx.fillStyle=ambient;ctx.fillRect(0,0,w,h);softGlow(ctx,cx,waterY,w*.46,.055);
    const plane=ctx.createRadialGradient(cx,waterY,4,cx,waterY,w*.6);
    plane.addColorStop(0,'rgba(136,210,188,.11)');plane.addColorStop(.45,'rgba(81,154,134,.04)');plane.addColorStop(1,'rgba(5,14,12,0)');ctx.fillStyle=plane;ctx.beginPath();ctx.ellipse(cx,waterY,w*.62,h*.18,0,0,Math.PI*2);ctx.fill();
    if(now<impact){
      const t=clamp(now/impact,0,1),y=-h*.12+(waterY+h*.03)*easeInOut(t),stretch=1+Math.sin(t*Math.PI)*.55,r=Math.max(2.2,w*.025);
      ctx.save();ctx.translate(cx,y);ctx.scale(1/stretch,stretch);const drop=ctx.createRadialGradient(-r*.2,-r*.35,r*.08,0,0,r*1.25);
      drop.addColorStop(0,'rgba(232,255,248,.8)');drop.addColorStop(.22,'rgba(135,215,191,.68)');drop.addColorStop(.58,'rgba(63,145,123,.38)');drop.addColorStop(1,'rgba(26,79,65,.04)');ctx.fillStyle=drop;ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);ctx.fill();ctx.restore();softGlow(ctx,cx,y,r*3,.12*(1-t*.4));
    }
    if(now>=impact){
      const t=clamp((now-impact)/(cycle-impact),0,1),hit=Math.exp(-Math.pow(t-.055,2)/.0024);softGlow(ctx,cx,waterY,w*(.18+.14*t),.11*(1-t)+.02);
      ...
      for(let i=0;i<5;i++){
        ...
        const eased=easeOut(rt),jitter=(seeded(seed+i*9)-.5)*.035,maxRx=w*(.16+i*.075+jitter),rx=6+maxRx*eased,ry=rx*(.245+(seeded(seed+i*4)-.5)*.018),alpha=(.18-i*.023)*(1-rt)*(.78+.22*Math.sin((rt+i*.21)*Math.PI));
      ...
      for(let i=0;i<3;i++){const drift=((time/1000)*(7+i*2)+seeded(seed+i)*70)%(w*.82);ctx.strokeStyle=`rgba(120,187,167,${.016+i*.006})`;ctx.lineWidth=.55;ctx.beginPath();ctx.ellipse(cx,waterY+i*3,w*.22+drift*.22,h*(.028+i*.012),0,0,Math.PI*2);ctx.stroke();}
    }
  }

  function drawScore(surface,time,staticOnly=false){
    const {ctx,w,h}=surface;ctx.clearRect(0,0,w,h);const t=staticOnly?.45:(time/1000+surface.seed)%5.2/5.2,breath=(Math.sin(t*Math.PI*2-Math.PI/2)+1)/2,cx=w/2,cy=h/2;
    softGlow(ctx,cx,cy,Math.min(w,h)*(.45+.04*breath),.035+.022*breath);for(let i=0;i<3;i++){const p=(t+i*.29)%1,r=Math.min(w,h)*(.31+.17*p);ellipseRing(ctx,cx,cy,r,r,.055*(1-p),.5,0);}
  }

  function drawCheckin(surface,time,staticOnly=false){
    const {ctx,w,h}=surface;ctx.clearRect(0,0,w,h);const t=staticOnly?.52:(time/1000+surface.seed*.1)%6.4/6.4,cx=w*.92,cy=h*.52;
    const glow=ctx.createRadialGradient(cx,cy,0,cx,cy,w*.42);glow.addColorStop(0,'rgba(104,184,160,.075)');glow.addColorStop(.45,'rgba(62,127,109,.028)');glow.addColorStop(1,'rgba(13,31,27,0)');ctx.fillStyle=glow;ctx.fillRect(0,0,w,h);
  ...
  function sync(){
    ...
    if(!hero||!score||!checkin){clearTimeout(retryTimer);retryTimer=setTimeout(queueSync,220);}
  ...
  function tick(time){
    ...
    let active=0;for(const surface of surfaces.values()){
      if(!surface.host.isConnected||surface.host.closest('#main')?.dataset?.garangScreen!=='today'||surface.visible===false)continue;draw(surface,time,false);active++;
    }
  ...
  function lifecycleSync(){queueSync();clearTimeout(retryTimer);retryTimer=setTimeout(queueSync,280);}
