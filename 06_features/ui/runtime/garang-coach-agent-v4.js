...
async function processAssistant(messageEl){
 ...
 /* Never consume an assistant message before the authenticated bridge is ready.
    Hydration will queue another sync; marking it seen earlier permanently lost plan proposals. */
 if(!Contract||!Bridge?.ready?.())return;
 seenAssistantIds.add(messageId);
 try{
  const state=Bridge.getState(),context=contextFromState(state);if(Bridge.getMemoryContext)context.memory=Bridge.getMemoryContext(text,{limit:24,budgetChars:6000});if(Bridge.getUserStateContext)context.userState=Bridge.getUserStateContext();if(Bridge.getDecisionContext)context.decision=Bridge.getDecisionContext();
  ...
  const entries=result.proposals.map(proposal=>({session,proposal,status:'pending',reads:result.reads.length}));if(entries.length){sessionsByMessage.set(messageId,entries);entries.forEach(entry=>renderProposalCard(messageEl,entry));}messageEl.dataset.g4AgentProcessed='1';
 }catch(error){
  seenAssistantIds.delete(messageId);
