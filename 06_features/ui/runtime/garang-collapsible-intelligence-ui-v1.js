/* GARANG Collapsible Intelligence UI v1.1
   Advanced controls remain available, but every observer-driven DOM write is idempotent.
   This prevents MutationObserver -> DOM write -> MutationObserver feedback loops on iOS/WebKit. */
(() => {
'use strict';
...
const setHTML=(el,value)=>{const next=String(value??'');if(el&&el.innerHTML!==next)el.innerHTML=next;};
...
function writeOpen(key,value){try{const next=value?'1':'0',k=STORE_PREFIX+key;if(sessionStorage.getItem(k)!==next)sessionStorage.setItem(k,next);}catch{}}
...
function applyOpen(container,body,button,key,open){container.classList.toggle('gci-expanded',open);const expanded=open?'true':'false';if(container.dataset.gciExpanded!==expanded)container.dataset.gciExpanded=expanded;if(body.hidden===open)body.hidden=!open;setAttr(button,'aria-expanded',expanded);setHTML(button,toggleHTML(open));writeOpen(key,open);}
