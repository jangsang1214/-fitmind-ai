...
function mount(root){
  ...
  function enhanceCoach(snapshot){
    ...
    if(!host){host=doc.createElement('section');host.className='gcl-coach-actions';host.dataset.gclCoachActions='1';const decision=coachRoot.querySelector('.garang-decision-card');if(decision)decision.insertAdjacentElement('afterend',host);else coachRoot.querySelector('.g2-composer-wrap')?.insertAdjacentElement('beforebegin',host);}
    host.innerHTML=`<div class="gcl-mini-head"><span>${lang()==='en'?'ACT':'ACT / 행동'}</span><small>${lang()==='en'?'GARANG asks before changing data':'데이터 변경은 기존 승인 절차를 따릅니다'}</small></div><div>${actions.map((a,i)=>`<button type="button" data-gcl-coach="${i}">${esc(a.label)}</button>`).join('')}</div>`;host._gclActions=actions;
  }
