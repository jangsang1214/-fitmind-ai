...
function prefill(root,kind,payload){
  ...
  if(kind==='reuse-workout'){setInput(doc,'wName',p.name);setInput(doc,'wSets',p.sets);setInput(doc,'wReps',p.reps);setInput(doc,'wWeight',p.weight);setInput(doc,'wRpe',p.rpe);setInput(doc,'wDuration',p.duration);if(Array.isArray(p.setDetails)&&p.setDetails.length){const toggle=doc.getElementById('toggleSetDetails');if(toggle&&toggle.getAttribute('aria-expanded')!=='true')toggle.click();doc.querySelectorAll('#workoutSetDetails [data-set-row]').forEach((row,index)=>{const set=p.setDetails[index];if(!set)return;setInput({getElementById:id=>row.querySelector('[data-set-'+(id==='wReps'?'reps':id==='wWeight'?'weight':'rpe')+']')},'x',null);const reps=row.querySelector('[data-set-reps]'),weight=row.querySelector('[data-set-weight]'),rpe=row.querySelector('[data-set-rpe]');if(reps)reps.value=String(set.reps??'');if(weight)weight.value=String(set.weight??'');if(rpe)rpe.value=String(set.rpe??'');});}return true;}
...
function mount(root){
  ...
  function enhanceAccumulation(snapshot){
    ...
    if(main.querySelector('#garangPlanExecution,[data-gx-planner-shell="1"]')){return;}
    ...
    const domainRows=goal?.domains?.map(d=>`<div class="gcl-goal-row"><span>${esc(d.label)}</span><strong>${d.score===null?'—':`${d.score}%`}</strong><small>${esc(d.score===null?'기록이 더 필요합니다':d.evidence)}</small></div>`).join('')||'';
    const goalHtml=goal?`<div class="gcl-goal-head"><div><span>${model.lang==='en'?'GOAL FIT':'목표 적합도'}</span><strong>${goal.overall===null?'—':`${goal.overall}%`}</strong></div><small>${esc(goal.goalLabel||'목표 미설정')} · 최근 30일</small></div><div class="gcl-goal-rows">${domainRows}</div><p class="gcl-goal-note">${goal.overall===null?'운동·식단·회복 중 두 영역 이상에 기록이 쌓이면 판단합니다.':'각 점수는 저장된 기록만으로 계산하며, 기록이 없는 영역은 판단하지 않습니다.'}</p>`:'';
    const html=empty?`<section id="garangAccumulationOverview" class="gcl-accum gcl-accum-empty" data-gcl-empty="1"><div class="gcl-accum-copy"><span>${model.lang==='en'?'ACCUMULATION':'누적.'}</span><h2>${esc(model.headline)}</h2><p>${esc(model.support)}</p><button type="button" data-pagego="log" data-gcl-first-record="1">${model.lang==='en'?'Add your first record':'첫 기록 남기기'}</button></div>${goalHtml}</section>`:`<section id="garangAccumulationOverview" class="gcl-accum" data-gcl-streak-kind="recording"><div class="gcl-accum-copy"><span>${model.lang==='en'?'30 DAY ACCUMULATION':'30 DAY / 누적.'}</span><h2>${esc(model.headline)}</h2><p>${esc(model.support)}</p>${model.streakEvidence.length?`<details class="gcl-streak-details"><summary>기록 연속일 근거</summary><ul class="gcl-streak-evidence" aria-label="기록 연속일 근거">${evidence}</ul></details>`:''}${rhythmMarkup(model.rhythm)}</div><div class="gcl-accum-metrics"><div><strong>${model.activeDays}</strong><span>${model.lang==='en'?'active days':'운동 일수'}</span></div><div><strong>${plan}</strong><span>${model.lang==='en'?'plan rate':'계획 수행'}</span></div><div><strong>${model.totalDistance}</strong><span>${model.lang==='en'?'run km':'러닝 km'}</span></div><div><strong>${delta}</strong><span>${model.lang==='en'?'body change':'체중 변화'}</span></div>${goalHtml}</section>`;
