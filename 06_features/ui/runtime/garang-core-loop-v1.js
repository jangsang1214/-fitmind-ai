...
function sevenDayRhythm(state,end=localDate()){
  ...
  return Array.from({length:7},(_,index)=>{
    const offset=index-6,d=new Date(`${end}T12:00:00`);d.setDate(d.getDate()+offset);
    const date=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
    return {date,active:dates.has(date),today:offset===0};
  });
}
function deriveToday(state,options={}){
  ...
  const evidence=[workouts+runs>0,meals>0,body>0,check].filter(Boolean).length,completion=plans.length?Math.round(done/plans.length*100):null,streak=currentStreak(safe,date),rhythm=sevenDayRhythm(safe,date);
...
function deriveAccumulation(state,options={}){
  ...
  if(!recordDays){headline=lang==='en'?'Your accumulation starts with one record.':'첫 기록부터 누적이 시작됩니다.';support=lang==='en'?'There is not enough saved data to describe a streak yet.':'아직 저장된 기록이 없어 연속 기록을 판단할 수 없습니다.';}
  else if(streak>=3){headline=lang==='en'?`${streak} consecutive recording days.`:`최근 ${streak}일 연속 기록이 이어졌습니다.`;support=lang==='en'?'This is a recording streak, backed by the dates shown below.':'이것은 기록 연속일이며, 아래 날짜와 기록 영역을 근거로 계산했습니다.';}
  return {days,end,lang,workouts:workouts.length,runs:runs.length,meals:meals.length,activeDays,recordDays,planRate,totalDistance,avgProtein,bodyDelta:delta,streak,recordStreak,planStreak:completedPlanStreak,streakEvidence:evidence,hasRecords:recordDays>0,rhythm,headline,support};
}
  function displayWeight(root,value){const n=finite(value);if(n===null)return value;let unit='metric';try{unit=root.GarangAgentStateBridge?.getState?.()?.preferences?.unit==='imperial'?'imperial':'metric';}catch{}return unit==='imperial'?Math.round(n*2.2046226218*10)/10:n;}
...
function prefill(root,kind,payload){
  const doc=root.document,p=payload||{};
  if(kind==='reuse-workout'){setInput(doc,'wName',p.name);setInput(doc,'wSets',p.sets);setInput(doc,'wReps',p.reps);setInput(doc,'wWeight',displayWeight(root,p.weight));setInput(doc,'wRpe',p.rpe);setInput(doc,'wDuration',p.duration);if(Array.isArray(p.setDetails)&&p.setDetails.length){const toggle=doc.getElementById('toggleSetDetails');if(toggle&&toggle.getAttribute('aria-expanded')!=='true')toggle.click();doc.querySelectorAll('#workoutSetDetails [data-set-row]').forEach((row,index)=>{const set=p.setDetails[index];if(!set)return;const reps=row.querySelector('[data-set-reps]'),weight=row.querySelector('[data-set-weight]'),rpe=row.querySelector('[data-set-rpe]');if(reps)reps.value=String(set.reps??'');if(weight)weight.value=String(displayWeight(root,set.weight));if(rpe)rpe.value=String(set.rpe??'');[reps,weight,rpe].forEach(input=>input?.dispatchEvent(new Event('input',{bubbles:true})));});}return true;}
  if(kind==='reuse-meal'){doc.querySelector('.manual-entry')?.setAttribute('open','');setInput(doc,'foodSearch',p.name);setInput(doc,'foodGram',p.grams);setInput(doc,'foodKcal',p.kcal);setInput(doc,'foodProtein',p.protein);setInput(doc,'foodCarb',p.carbs);setInput(doc,'foodFat',p.fat);return true;}
  if(kind==='reuse-body'){doc.querySelector('.body-entry-drawer')?.setAttribute('open','');setInput(doc,'bDate',localDate());setInput(doc,'bWeight',p.weight);setInput(doc,'bMuscle',p.muscle);setInput(doc,'bFatPct',p.fatPercent);return true;}
...
function mount(root){
  function ensureCoachActionLayoutStyle(){
    ...
    const style=doc.createElement('style');style.id='garang-core-loop-action-layout-style';style.textContent='.g2-chat-main.garang-has-decision-card{grid-template-rows:auto auto auto minmax(0,1fr) auto!important}.g2-chat-main.garang-has-decision-card>.gcl-coach-actions,.g2-chat-main.garang-has-decision-card>.gcl-context-actions{grid-row:3!important}.g2-chat-main.garang-has-decision-card>.g2-chat-scroll{grid-row:4!important}.g2-chat-main.garang-has-decision-card>.g2-composer-wrap{grid-row:5!important}.gcl-context-actions{display:block;box-sizing:border-box;width:100%;max-width:100%;margin:8px 0 0;padding:0;border:0;overflow:visible}.gcl-context-actions>div{display:flex!important;flex-direction:row!important;align-items:center!important;gap:8px;flex-wrap:wrap;width:100%;max-width:100%}.gcl-context-actions button{display:inline-flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;flex:0 1 auto!important;width:auto!important;max-width:100%!important;min-width:0!important;min-height:32px!important;height:auto!important;padding:7px 11px!important;border:1px solid rgba(79,174,146,.25)!important;border-radius:999px!important;background:transparent!important;color:inherit!important;font-size:10px!important;line-height:1.2!important;white-space:nowrap!important;writing-mode:horizontal-tb!important}.g2-chat-head{box-sizing:border-box!important;width:100%!important;max-width:100%!important;min-width:0!important;overflow:hidden!important}.g2-chat-head-copy{min-width:0!important;max-width:100%!important;overflow:hidden!important}.g2-head-new{box-sizing:border-box!important;min-width:0!important;max-width:42%!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important}.garang-coach-v2,.g2-chat-main,.g2-composer-wrap{max-width:100vw!important;min-width:0!important}html,body{max-width:100%!important;overflow-x:hidden!important}@media(max-width:700px){.gcl-context-actions{margin-top:6px}.gcl-context-actions button{font-size:9px!important;padding:7px 9px!important}}.gcl-goal-head{display:flex;align-items:end;justify-content:space-between;gap:16px;margin-top:20px;padding-top:18px;border-top:1px solid rgba(242,239,233,.08)}.gcl-goal-head span{display:block;color:#4fae92;font-size:8px;letter-spacing:.16em}.gcl-goal-head strong{display:block;margin-top:4px;font:400 42px/.9 var(--g2-display,"Cormorant Garamond",Georgia,serif)}.gcl-goal-head>small{color:#a5a59f;font-size:10px}.gcl-goal-rows{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:14px}.gcl-goal-row{min-width:0;padding:11px 10px;border-top:1px solid rgba(242,239,233,.1)}.gcl-goal-row>span{display:block;color:#a5a59f;font-size:10px}.gcl-goal-row>strong{display:block;margin-top:7px;font-size:21px;font-weight:450}.gcl-goal-row>small{display:block;margin-top:7px;color:#7c807b;font-size:9px;line-height:1.4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.gcl-goal-note{margin:12px 0 0;color:#7c807b;font-size:9px;line-height:1.5}.gcl-streak-details{margin-top:13px}.gcl-streak-details summary{color:#7c807b;font-size:9px;cursor:pointer}@media(max-width:700px){.gcl-goal-rows{grid-template-columns:repeat(2,minmax(0,1fr))}}';doc.head.appendChild(style);
  }
  ...
  function ensureCoachActionDisclosureStyle(){
    if(doc.getElementById('garang-core-loop-action-disclosure-style'))return;
    const style=doc.createElement('style');style.id='garang-core-loop-action-disclosure-style';style.textContent='.gcl-context-actions[data-gcl-coach-actions="1"]{display:inline-flex!important;align-items:center!important;box-sizing:border-box!important;width:max-content!important;max-width:100%!important;min-width:0!important;margin:0!important;padding:0!important;border:0!important;overflow:visible!important}.gcl-action-toggle{display:inline-flex!important;align-items:center!important;gap:6px!important;box-sizing:border-box!important;min-width:0!important;min-height:30px!important;height:30px!important;padding:6px 10px!important;border:1px solid rgba(79,174,146,.3)!important;border-radius:999px!important;background:transparent!important;color:inherit!important;font:500 10px/1 var(--g2-ui,system-ui)!important;white-space:nowrap!important;cursor:pointer!important}.gcl-action-mark{display:inline-grid!important;place-items:center!important;width:15px!important;height:18px!important;color:#4fae92!important}.gcl-action-mark svg{display:block!important;width:15px!important;height:18px!important;fill:none!important;stroke:currentColor!important;stroke-width:1.4!important}.gcl-action-mark text{fill:currentColor!important;stroke:none!important;font:700 12px/1 system-ui!important}.gcl-actions-panel{display:none!important;align-items:center!important;gap:6px!important;min-width:0!important}.gcl-actions-panel[hidden]{display:none!important}.gcl-context-actions[data-gcl-actions-expanded="true"]{display:flex!important;gap:8px!important;width:100%!important;max-width:100%!important}.gcl-context-actions[data-gcl-actions-expanded="true"] .gcl-actions-panel{display:flex!important;flex-wrap:nowrap!important;overflow-x:auto!important;overflow-y:hidden!important;scrollbar-width:none!important}.gcl-context-actions[data-gcl-actions-expanded="true"] .gcl-actions-panel::-webkit-scrollbar{display:none!important}.gcl-actions-panel button[data-gcl-coach]{display:inline-flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;flex:0 0 auto!important;min-width:0!important;min-height:30px!important;height:30px!important;padding:7px 10px!important;border:1px solid rgba(79,174,146,.24)!important;border-radius:999px!important;background:transparent!important;color:inherit!important;font-size:10px!important;line-height:1.2!important;white-space:nowrap!important;cursor:pointer!important}@media(max-width:700px){.gcl-action-toggle,.gcl-actions-panel button[data-gcl-coach]{font-size:9px!important;padding-left:8px!important;padding-right:8px!important}}';doc.head.appendChild(style);
  }
  ...
  function enhanceToday(snapshot){
    if(main.dataset.garangScreen!=='today')return;
    const model=deriveToday(snapshot,{lang:lang()}),current=main.querySelector('#garangCoreToday');
    const html=`<section id="garangCoreToday" class="gcl-today"><div><span>${model.lang==='en'?'NOW':'NOW / 지금'}</span><strong>${esc(model.headline)}</strong><p>${esc(model.support)}</p></div><aside><b>${model.streak}</b><small>${model.lang==='en'?'day streak':'일 연속'}</small>${rhythmMarkup(model.rhythm)}</aside></section>`;
    if(current)current.outerHTML=html;else{const flow=main.querySelector('#garangTodayFlow');if(flow)flow.insertAdjacentHTML('afterend',html);else main.querySelector('.page-head')?.insertAdjacentHTML('afterend',html);}
  }
  function enhanceRecord(snapshot){
    const sheet=doc.querySelector('.garang-record-sheet');if(!sheet)return;
    const rows=deriveRecent(snapshot,{lang:lang()});sheet.querySelector('[data-gcl-recent]')?.remove();if(!rows.length)return;
    const host=doc.createElement('section');host.className='gcl-record-recent';host.dataset.gclRecent='1';
    host.innerHTML=`<div class="gcl-mini-head"><span>${lang()==='en'?'RECENT':'RECENT / 최근'}</span><small>${lang()==='en'?'Reuse without saving automatically':'자동 저장 없이 값만 불러옵니다'}</small></div>${rows.map((row,index)=>`<button type="button" data-gcl-reuse="${index}"><span><small>${esc(row.label)}</small><strong>${esc(row.title)}</strong><em>${esc(row.meta)}</em></span><b>↗</b></button>`).join('')}`;
    host._gclRows=rows;sheet.querySelector('.garang-record-routes')?.insertAdjacentElement('afterend',host);
  }
  ...
  function enhanceAccumulation(){
    // The canonical accumulation surface is owned by garang-plan-execution-ui-v1.
    // deriveAccumulation remains the deterministic model for the Coach/context layer;
    // this module must not paint a second accumulation or streak surface.
    return;
  }
  function render(){
  ...
  doc.addEventListener('click',event=>{
    const firstRecord=event.target.closest?.('[data-gcl-first-record]');
    if(firstRecord){event.preventDefault();root.GarangRouter?.navigate?.('log',{source:'accumulation-first-record',force:true});return;}
    const reuse=event.target.closest?.('[data-gcl-reuse]');
    if(reuse){const host=reuse.closest('[data-gcl-recent]'),row=host?._gclRows?.[Number(reuse.dataset.gclReuse)];if(!row)return;event.preventDefault();pendingPrefill=row;root.GarangRouter?.navigate?.(row.route,{source:'core-loop-reuse',force:true});schedule();return;}
    const actionToggle=event.target.closest?.('[data-gcl-actions-toggle]');
    if(actionToggle){const host=actionToggle.closest('[data-gcl-coach-actions]'),panel=host?.querySelector('[data-gcl-actions-panel]');if(!host||!panel)return;event.preventDefault();const expanded=host.dataset.gclActionsExpanded!=='true';host.dataset.gclActionsExpanded=expanded?'true':'false';panel.hidden=!expanded;actionToggle.setAttribute('aria-expanded',expanded?'true':'false');const symbol=actionToggle.querySelector('[data-gcl-action-symbol]');if(symbol)symbol.textContent=expanded?'−':'+';return;}
