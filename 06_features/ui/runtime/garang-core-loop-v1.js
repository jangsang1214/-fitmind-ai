...
function prefill(root,kind,payload){
  ...
  if(kind==='reuse-workout'){setInput(doc,'wName',p.name);setInput(doc,'wSets',p.sets);setInput(doc,'wReps',p.reps);setInput(doc,'wWeight',p.weight);setInput(doc,'wRpe',p.rpe);setInput(doc,'wDuration',p.duration);return true;}
