...
function startRest(setType='working'){const custom=Math.max(15,num(document.getElementById('workoutRestSeconds')?.value,90)),type=String(setType||'working');if(type==='drop'){restUntil=0;updateLive();return;}const seconds=type==='warmup'?Math.min(custom,60):type==='failure'?Math.max(custom,120):custom;restUntil=Date.now()+seconds*1000;updateLive();}
function updateLive(){
...
function targetFor(row){
  ...
  if(setType==='warmup'){delta=0;reason='워밍업 기준 유지';}
  else if(setType==='drop'){delta=0;reason='드롭 세트 유지';}
