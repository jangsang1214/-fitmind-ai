...
function resultCard(){
  ...
  const log=builder.closest('.gws-panel[data-garang-workout-surface="log"]');if(!log)return;log.appendChild(card);const pr=card.querySelector('#workoutResultPr'),comparisons=Array.isArray(lastResult.prComparisons)?lastResult.prComparisons:[];if(pr&&comparisons.length){const improved=comparisons.filter(x=>num(x.sessionMetric)>num(x.baselineMetric)+.05),best=comparisons.reduce((a,b)=>num(b.sessionMetric)>num(a.sessionMetric)?b:a,comparisons[0]),shown=bridge()?.displayWeight?.(best.sessionMetric,1)??best.sessionMetric;pr.textContent=improved.length?('NEW PR · '+improved.map(x=>x.name).join(', ')+' · 최고 estimated 1RM '+shown+' '+displayUnit()):('PR 유지 · 최고 estimated 1RM '+shown+' '+displayUnit());}lastResult=null;
}
function enhance(){
