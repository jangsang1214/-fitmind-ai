...
function render(){
 ...
 card.innerHTML=`<button type="button" class="garang-decision-toggle" aria-expanded="${wasExpanded?'true':'false'}"><span class="garang-decision-kicker">${isEn?'GARANG DECISION':'GARANG 판단'}</span><b class="garang-decision-mode">${esc(text(mode))}</b><span class="garang-decision-chevron" aria-hidden="true">⌄</span></button><div class="garang-decision-details" ${wasExpanded?'':'hidden'}>${summary?`<p class="garang-decision-summary">${esc(summary)}</p>`:''}<div class="garang-decision-signals"><span>${text(SIGNAL.readinessBand)} · ${esc(signals.readinessBand||'unknown')}</span><span>${text(SIGNAL.fatigueBand)} · ${esc(signals.fatigueBand||'unknown')}</span><span>${text(SIGNAL.loadBand)} · ${esc(signals.loadBand||'unknown')}</span></div><div class="garang-decision-foot"><span>${isEn?'Confidence':'판단 신뢰도'} ${confidence}% · ${isEn?'No silent changes':'자동 변경 없음'}</span>${canPlan?`<button type="button" class="garang-decision-action">${isEn?'Propose plan':'계획 제안'}</button>`:''}</div></div>`;
