/* GARANG sync durability runtime v1
   Loaded before app.js so local persistence is hardened first.
   Firestore is patched after app.js initializes Firebase, before auth callbacks can sync state.
*/
(() => {
'use strict';
...
function readStateKey(key){return safeParse(readRaw(key));}
...
function markPending(uid,reason,{increment=true}={}){
  if(!uid)return;
  const previous=readPending(uid)||{},attempt=increment?(Number(previous.attempt)||0)+1:(Number(previous.attempt)||0);
  const value={uid,attempt,reason:String(reason||'pending'),updatedAt:new Date().toISOString()};writePending(uid,value);scheduleRetry(uid);
}
function scheduleRetry(uid,{immediate=false}={}){
  if(!uid||uid!==currentUid()||navigator.onLine===false)return;
  if(retryTimer)clearTimeout(retryTimer);
  const pending=readPending(uid);if(!pending)return;
  const wait=immediate?180:Core.retryDelay(pending.attempt||0);
  retryTimer=setTimeout(()=>{retryTimer=null;triggerRetry(uid);},wait);
}
function triggerRetry(uid){
  if(!uid||uid!==currentUid()||navigator.onLine===false)return;
  const badge=document.getElementById('syncBadge');
  if(!badge){retryTimer=setTimeout(()=>triggerRetry(uid),900);return;}
  badge.click();
}
function syncError(code,message=code){const e=new Error(message);e.code=code;return e;}
...
/* The wrapper is installed before agent-state-hook-v1. The Agent hook wraps this wrapper,
   so both systems keep working and no live-state capture is lost. */
Storage.prototype.setItem=function(key,value){
  if(this!==localStorage||!Core.isStateKey(key))return baseSetItem.call(this,key,value);
  const previousRaw=readRaw(key),previous=safeParse(previousRaw),next=safeParse(String(value));
  if(!next)return baseSetItem.call(this,key,value);
  const ownerUid=Core.ownerFromKey(key),deletes=Core.collectNewTombstones(previous,next,{ownerUid,clock:Date.now()});
  let hardened;
...
function patchFirestore(){
  ...
  try{
    ...
    getOwner.get=async function(...args){
      ...
      const local=readUserState(uid);if(!local)return snapshot;
      const remote=stripCloudFields(snapshot.data());
    ...
    setOwner.set=async function(data,options){
      ...
      try{
        await firestore.runTransaction(async transaction=>{
          const snapshot=await transaction.get(ref),remote=snapshot.exists?stripCloudFields(snapshot.data()):null;
          const merged=Core.mergeActiveStates(outgoing,remote,{ownerUid:uid,clock:Date.now()}),payload=Core.compactForCloud(merged);
          payload.meta={...(payload.meta||{}),syncOwnerUid:uid};
          payload.clientUpdatedAt=payload.meta.updatedAt||outgoing.clientUpdatedAt||new Date().toISOString();
          payload.cloudUpdatedAt=window.firebase.firestore.FieldValue.serverTimestamp();
...
async function exportVerifiedBackup(){
  const uid=currentUid(),key=uid?Core.stateKey(uid):Core.DEMO_KEY,state=readStateKey(key);
  if(!state)return toast('내보낼 저장 데이터를 찾지 못했습니다.');
  try{if(window.GarangSchema?.validateImport)window.GarangSchema.validateImport(state);}
  catch(error){console.warn('[GARANG] export validation failed',error);return toast('데이터 검증에 실패해 내보내기를 중단했습니다.');}
  const envelope=Core.createExportEnvelope(state,{scope:uid?'authenticated':'demo'});
  if(!Core.verifyExportEnvelope(envelope))return toast('백업 검증에 실패했습니다.');
  const blob=new Blob([nativeStringify(envelope,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');
  a.href=url;a.download=`GARANG_BACKUP_${new Date().toISOString().slice(0,10)}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1200);
  window.dispatchEvent(new CustomEvent('garang:data-exported',{detail:{format:envelope.format,scope:envelope.scope}}));toast('검증된 GARANG 백업을 내보냈습니다.');
}

document.addEventListener('click',event=>{
  const button=event.target?.closest?.('#exportData');if(!button)return;
  event.preventDefault();event.stopImmediatePropagation();exportVerifiedBackup().catch(error=>{console.warn('[GARANG] export failed',error);toast('데이터 내보내기에 실패했습니다.');});
},true);
...
window.GarangSyncDurabilityRuntime=Object.freeze({version:Core.VERSION,status:()=>({uid:currentUid(),online:navigator.onLine!==false,firestorePatched,pending:currentUid()?readPending(currentUid()):null}),forceSync:()=>{const uid=currentUid();if(uid){markPending(uid,'manual',{increment:false});scheduleRetry(uid,{immediate:true});}},exportVerifiedBackup});
})();
