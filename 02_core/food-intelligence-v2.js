...
const VERSION='garang-food-intelligence-v2';
...
const NOISE=/\b(?:serving|portion|piece|pieces|bowl|plate|cup|slice|그램|gram|grams|g|ml|개|인분|그릇|접시|조각)\b/gi;
function normalize(value){
 let s=clean(value);try{s=s.normalize('NFKC');}catch{}
 return s.toLocaleLowerCase('en-US').replace(NOISE,' ').replace(/[()[\]{}.,/\\·_\-:+]+/g,' ').replace(/\d+(?:\.\d+)?\s*(?:g|gram|grams|ml|그램)?/gi,' ').replace(/\s+/g,' ').trim();
}
...
function keyScore(query,key){
 ...
 const tok=overlap(tokens(query),tokens(key)),tri=dice(trigrams(query),trigrams(key)),edit=editRatio(query,key),concept=conceptOverlap(query,key);
 const score=Math.max(contains*.9,tok*.88,tri*.78,edit*.86,concept*.82,concept*.65+tri*.25);
...
function index(corpus=[]){if(Array.isArray(corpus)&&INDEX_CACHE?.has(corpus))return INDEX_CACHE.get(corpus);const built=list(corpus).map((raw,order)=>{const food=Foundation.canonicalize(raw),brand=clean(raw?.brand||raw?.brand_name||raw?.manufacturer),product=clean(raw?.product_name||raw?.productName),keys=[{value:food.name,kind:'primary'},{value:food.nameEn,kind:'english'},{value:product,kind:'product'},{value:brand&&food.name?`${brand} ${food.name}`:'',kind:'brand-primary'},{value:brand&&product?`${brand} ${product}`:'',kind:'brand-product'},...list(food.aliases).map(value=>({value,kind:'alias'}))].filter(x=>clean(x.value));return {raw,food,order,keys};});if(Array.isArray(corpus)&&INDEX_CACHE)INDEX_CACHE.set(corpus,built);return built;}
