...
function constraints(state,memoryContext,now){
 ...
 return {availableMinutes:minutes==null?null:clamp(Math.round(minutes),0,240),painCaution:latest.painCaution===true,avoidances:avoid.slice(0,8),preferences:pref.slice(0,8),weeklyFrequency:weeklyFrequency(state),goal:goalClass(state)};
}
...
function deriveMode(userState,decision,score,constraint){
 ...
 if(constraint.painCaution||mode==='caution')return 'caution';if(mode==='collect_data')return 'collect_data';if(constraint.availableMinutes===0)return 'maintain';if(recovery!=null&&recovery<40&&['maintain','progress','goal_focus'].includes(mode))mode='recover';if(recovery!=null&&recovery<55&&mode==='progress')mode='reduce';return mode;
}
...
function recommendToday(stateInput,{now=new Date(),userState={},decision=null,score=null,memoryContext=null}={}){
 ...
 if(constraint.painCaution)reasonCodes.push('PAIN_CAUTION');if(constraint.availableMinutes!=null)reasonCodes.push('TIME_CONSTRAINT');if(constraint.availableMinutes===0)reasonCodes.push('NO_TIME_AVAILABLE');if(finite(score?.components?.recovery?.score)!=null&&score.components.recovery.score<55)reasonCodes.push('RECOVERY_SCORE_LOW');
 let type=base.type;if(constraint.availableMinutes===0)type='rest';else if(mode==='caution'||mode==='recover')type='recovery';if(mode==='collect_data'&&existing[0])type=normalizeType(existing[0].type||existing[0].title);if(locked)type=normalizeType(locked.type||locked.title);
 ...
 if(!locked&&mode!=='collect_data'&&mode!=='caution'&&type!=='rest')actionProposal={tool:proposal.existingId?'updatePlan':'createPlan',requiresConfirmation:true,args:{...(proposal.existingId?{id:proposal.existingId}:{}),date,title:title.ko,type,duration,intensityScale:scale.intensity,volumeScale:scale.volume,decisionEngineVersion:String(decision?.engineVersion||Decision?.ENGINE_VERSION||''),decisionMode:mode,reasonCodes:proposal.reasonCodes}};
 return {engineVersion:ENGINE_VERSION,asOf:date,mode,confidence:proposal.confidence,today:proposal,supportActions:supportActions(score),actionProposal,guardrails:{requiresConfirmation:true,noSilentMutation:true,preserveCompletedPlans:true,painCautionBlocksIntensity:true,cautionBlocksPlanProposal:true,zeroTimeBlocksPlanProposal:true,medicalDiagnosis:false}};
}
