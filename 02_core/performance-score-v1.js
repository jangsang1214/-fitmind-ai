...
const clamp=(v,min=0,max=100)=>Math.max(min,Math.min(max,finite(v)??0));
const round=(v,d=0)=>{const n=finite(v);if(n==null)return null;const p=10**d;return Math.round((n+Number.EPSILON)*p)/p;};
...
const weeklyFrequency=s=>clamp(finite(s?.userModel?.weeklyFrequency)??finite(s?.onboarding?.weeklyFrequency)??4,1,7);
const latestWeight=s=>{const body=rows(s.body).filter(x=>finite(x.weight)>0).sort((a,b)=>dateOf(b.date).localeCompare(dateOf(a.date)));return finite(body[0]?.weight)??finite(s?.profile?.weight);};

function component(score,confidence,evidence,reasons=[]){
 if(score==null||finite(score)==null)return {score:null,confidence:0,evidence,reasons};
...
function workoutComponent(s,userState,end){
 ...
 const score=mean(values),confidence=clamp((Math.min(1,sessions/8)*.65)+(finite(userState?.load?.confidence)??0)*.35,0,1);
...
function recoveryComponent(s,userState,end){
 const checkins=rows(s.dailyCheckins||s.checkins).filter(x=>inWindow(x,end,7)),readiness=finite(userState?.readiness?.value),fatigue=finite(userState?.fatigue?.score),parts=[];
 if(readiness!=null)parts.push({v:clamp(readiness),w:.6});if(fatigue!=null)parts.push({v:clamp(100-fatigue),w:.4});
 if(!parts.length)return component(null,0,{checkins7:checkins.length},['RECOVERY_DATA_MISSING']);
 const score=parts.reduce((sum,p)=>sum+p.v*p.w,0)/parts.reduce((sum,p)=>sum+p.w,0),signalConfidence=Math.max(finite(userState?.readiness?.confidence)??0,finite(userState?.fatigue?.confidence)??0),confidence=clamp(signalConfidence*.7+Math.min(1,checkins.length/4)*.3,0,1);
 const reasons=[];if(readiness!=null&&readiness<55)reasons.push('READINESS_LOW');if(fatigue!=null&&fatigue>=60)reasons.push('FATIGUE_HIGH');if(readiness!=null&&fatigue!=null&&readiness>=75&&fatigue<40)reasons.push('RECOVERY_STRONG');
 return component(score,confidence,{checkins7:checkins.length,readiness:round(readiness),fatigue:round(fatigue)},reasons);
}
function nutritionComponent(s,end){
 const meals=rows(s.meals).filter(x=>inWindow(x,end,14)),byDay=new Map();for(const meal of meals){const d=dateOf(meal.date);if(!byDay.has(d))byDay.set(d,{protein:0,kcal:0});const row=byDay.get(d);row.protein+=Math.max(0,finite(meal.protein)??0);row.kcal+=Math.max(0,finite(meal.kcal)??0);}
...
function bodyComponent(s,userState,end){
 const body=rows(s.body).filter(x=>inWindow(x,end,28)&&(finite(x.weight)??0)>0),count=body.length;if(!count)return component(null,0,{measurements28:0},['BODY_DATA_MISSING']);
 const goal=goalText(s),trend=String(userState?.trends?.bodyWeight?.direction||'unknown'),goalScore=finite(userState?.goalAlignment?.score),goalConfidence=finite(userState?.goalAlignment?.confidence)??0;
 let score=65;if(goalScore!=null&&goalConfidence>=.25)score=clamp(55+(goalScore-50)*.7);else if(/fat|cut|lose|체지방|감량|다이어트/.test(goal)&&trend==='down')score=82;else if(/muscle|gain|bulk|strength|근육|증량|벌크|근력/.test(goal)&&(trend==='up'||trend==='stable'))score=80;
 ...
 return component(score,confidence,{measurements28:count,weightTrend:trend,goalAlignment:round(goalScore)},reasons);
}
