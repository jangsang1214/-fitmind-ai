...
function estimateTargets(state,date=todayLocal()){
  ...
  const proteinTarget=weight&&weight>0?Math.round(weight*1.6):null,explicitCalorieTarget=finite(state?.profile?.calorieTarget);
  ...
  let bmr=null,tdee=null,calorieTarget=null,factor=null,adjustment=0,targetSource='derived_profile';const adult=age!==null&&age>=18;
  ...
  if(adult&&explicitCalorieTarget!==null&&explicitCalorieTarget>=1000&&explicitCalorieTarget<=5000){calorieTarget=Math.round(explicitCalorieTarget);targetSource='user_confirmed';reasons.push('EXPLICIT_CALORIE_TARGET');}
  const hasGoal=!!(state?.profile?.goal||state?.onboarding?.goal),confidence=calorieTarget?(targetSource==='user_confirmed'?clamp(finite(state?.profile?.calorieTargetConfidence)??.75,.5,.95):clamp(0.65+(activeDays(state,date,14)>=4?0.15:0)+(hasGoal?0.1:0),0,0.9)):clamp(0.25+(proteinTarget?0.15:0),0,0.45);
  return {version:TARGET_VERSION,date,goal,goalLabel:goalLabel(goal),calorieTarget,proteinTarget,bmr:bmr===null?null:Math.round(bmr),tdee:tdee===null?null:Math.round(tdee),activityFactor:factor,goalAdjustment:targetSource==='derived_profile'&&calorieTarget?adjustment:null,targetSource,confidence:round(confidence,2),estimateOnly:targetSource!=='user_confirmed',medicalTarget:false,reasons:[...new Set(reasons)]};
}
