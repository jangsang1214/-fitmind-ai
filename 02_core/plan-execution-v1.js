...
function workoutEvidence(state,date){
  ...
  const sessions=keys.size||(rows.length?1:0),duration=rows.reduce((s,row)=>s+(finite(row?.duration)||0),0),volume=rows.reduce((s,row)=>s+workoutVolume(row),0),names=[...new Set(rows.map(row=>String(row?.name||row?.title||'').trim()).filter(Boolean))];
