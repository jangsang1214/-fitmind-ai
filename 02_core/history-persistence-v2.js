...
const VERSION='garang-history-persistence-v2.3';
...
function trimAuxiliary(out){if(Array.isArray(out.aiChat))out.aiChat=out.aiChat.slice(-30);if(Array.isArray(out.actionLog))out.actionLog=out.actionLog.slice(-120);if(Array.isArray(out.errors))out.errors=out.errors.slice(-40);if(Array.isArray(out.planner))out.planner=out.planner.slice(-160);if(Array.isArray(out.checkins))out.checkins=out.checkins.slice(-120);if(object(out.analytics)&&Array.isArray(out.analytics.events))out.analytics.events=out.analytics.events.slice(-120);if(object(out.memory)){if(Array.isArray(out.memory.entries))out.memory.entries=out.memory.entries.slice(-180);if(Array.isArray(out.memory.events))out.memory.events=out.memory.events.slice(-120);}}
function historyUpdatedAt(input){const stamps=[iso(input?.meta?.updatedAt),iso(input?.clientUpdatedAt),Number(input?.updatedAtMs)||0];for(const domain of DOMAINS)for(const row of rows(input?.[domain]))stamps.push(rowStamp(row));const latest=Math.max(0,...stamps);return latest?new Date(latest).toISOString():null;}
function compactShell(input){const out=clone(object(input)?input:{});for(const domain of DOMAINS){const limit=SHELL_LIMITS[domain],list=rows(out[domain]).slice().sort((a,b)=>rowStamp(a)-rowStamp(b)).slice(-limit);out[domain]=domain==='runs'?list.map(run=>({...run,coords:[]})):list;}out.meta=object(out.meta)?out.meta:{};out.meta.historyV2={version:2,collections:{...COLLECTIONS},counts:Object.fromEntries(DOMAINS.map(d=>[d,rows(input?.[d]).length])),updatedAt:historyUpdatedAt(input)};if(estimateJsonBytes(out)>TARGET_SHELL_BYTES)trimAuxiliary(out);out.meta.historyV2.shellBytes=estimateJsonBytes(out);return out;}
