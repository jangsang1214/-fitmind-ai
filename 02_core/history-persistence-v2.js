...
const VERSION='garang-history-persistence-v2';
...
function normalizeRecord(domain,input,ownerUid){
  if(!DOMAINS.includes(domain)||!object(input))return null;
  const row=clone(input),id=recordId(domain,row),baseTime=fallbackTime(row);
  row.id=id;
  row.createdAt=iso(row.createdAt)?new Date(iso(row.createdAt)).toISOString():baseTime;
  row.updatedAt=iso(row.updatedAt)?new Date(iso(row.updatedAt)).toISOString():baseTime;
  row.revision=Math.max(1,Number(row.revision)||1);
...
function mergeRows(domain,a,b,state={}){
  const map=new Map();
  for(const row of [...rows(a),...rows(b)]){
    if(deletedBy(state,domain,row))continue;
    const normalized=normalizeRecord(domain,row,row.ownerUid||state?.meta?.syncOwnerUid||null);if(!normalized)continue;
    const old=map.get(normalized.id);
    if(!old||rowStamp(normalized)>=rowStamp(old))map.set(normalized.id,normalized);
  }
  return [...map.values()].sort((x,y)=>rowStamp(x)-rowStamp(y));
}
function mergeStateWithHistory(stateInput,historyByDomain={}){
  const state=clone(object(stateInput)?stateInput:{});state.meta=object(state.meta)?state.meta:{};
  for(const domain of DOMAINS)state[domain]=mergeRows(domain,state[domain],historyByDomain?.[domain],state);
  return state;
}
function compactShell(input){
  const out=clone(object(input)?input:{});
  for(const domain of DOMAINS){
    const limit=SHELL_LIMITS[domain],list=rows(out[domain]).slice().sort((a,b)=>rowStamp(a)-rowStamp(b)).slice(-limit);
    out[domain]=domain==='runs'?list.map(run=>({...run,coords:[]})):list;
  }
...
return Object.freeze({VERSION,DOMAINS,COLLECTIONS,SHELL_LIMITS,rows,rowStamp,recordId,normalizeRecord,tombstonesFor,mergeRows,mergeStateWithHistory,compactShell,docPayload,recordFromDoc,fingerprint,historyIndex,estimateJsonBytes,stableStringify,fnv1a});
});
