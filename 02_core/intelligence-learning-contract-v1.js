...
function buildGraph(stateInput,{planExecution=null,now=new Date(),days=28}={}){
  const state=object(stateInput)?stateInput:{},limit=Math.max(1,Math.min(56,Number(days)||28)),end=todayLocal(now),dates=[...new Set(list(state?.planner).map(row=>dateKey(row?.date)).filter(Boolean))].sort().filter(date=>date<=end).slice(-limit),dayGraphs=dates.map(date=>buildDayGraph(state,date,{planExecution,now})),cycles=dayGraphs.flatMap(row=>row.cycles),edges=dayGraphs.flatMap(row=>row.edges);
