...
function pickDiverse(candidates,count,state,now=new Date()){
 const recent=recentByName(state),asOf=dateMs(localDate(now)),sorted=candidates.map((ex,index)=>{const name=String(ex.exercise_name||'').trim(),last=recent.get(name.toLowerCase()),lastMs=dateMs(last?.date),daysSince=Number.isFinite(lastMs)?Math.max(0,(asOf-lastMs)/DAY_MS):Infinity,penalty=last?Math.max(0,4-daysSince):0;return {ex,index,penalty,family:exerciseFamily(name)};}).sort((a,b)=>a.penalty-b.penalty||a.index-b.index);
...
function autoTarget(state,exerciseDb,now){const loads=analyzeMuscleLoad(state,exerciseDb,{now,days:7}),map=new Map(loads.map(x=>[x.key,x.effort])),r=readiness(state,{now}),soreKey=r.checkin?.soreArea?muscleKey(r.checkin.soreArea):null,keys=['chest','back','legs','shoulders','core'];return keys.slice().sort((a,b)=>{const ap=soreKey===a&&num(r.checkin?.sorenessMax)>=4?999:0,bp=soreKey===b&&num(r.checkin?.sorenessMax)>=4?999:0;return ((map.get(a)||0)+ap)-((map.get(b)||0)+bp);})[0]||'full';}
function generateWorkoutPlan(state,exerciseDb=[],spec={},options={}){
 ...
 if(target==='full'){
  const order=['legs','chest','back','shoulders','core'];for(let i=0;i<count;i++){const key=order[i%order.length],candidate=pickDiverse(candidatesFor(exerciseDb,key),1,state,now)[0];if(candidate&&!items.some(x=>x.name===candidate.exercise_name))items.push({candidate,key});}
 }else{
  const picked=pickDiverse(candidatesFor(exerciseDb,target),count,state,now);picked.forEach(candidate=>items.push({candidate,key:muscleKey(candidate.primary_muscle)}));
 }
...
function classifyCoachIntent(q){
 const x=String(q||'').toLowerCase();
 if(/식단|단백질|영양|먹|nutrition|protein|meal/.test(x))return 'nutrition';
 if(/계획|루틴|프로그램|짜줘|짜 줘|만들어줘|만들어 줘|create a plan|workout plan|routine/.test(x))return 'plan';
 if(/운동\s*강도|훈련\s*강도|강도.*(?:정해|추천|조절)|training\s*intensity|\bintensity\b/.test(x))return 'training_intensity';
 if(/회복|수면|피로|컨디션|상태|recovery|readiness|fatigue|sleep/.test(x))return 'recovery';
 if((/최근|기록|분석|recent|record|analy/.test(x)&&/운동|workout|training|exercise/.test(x)))return 'recent_workout';
 return 'other';
}
