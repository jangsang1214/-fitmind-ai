(function(root,factory){
 ...
 else root.GarangDecisionIntelligence=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(){
'use strict';

const ENGINE_VERSION='decision-intelligence-v1';
const MODES=Object.freeze(['collect_data','caution','recover','reduce','maintain','progress','goal_focus']);
...
const patternIds=s=>new Set(rows(s?.patterns).map(p=>String(p.id||'')));

const MODE_POLICY=Object.freeze({
 collect_data:{priority:55,intensityScale:null,volumeScale:null,duration:null,type:null},
 caution:{priority:100,intensityScale:0,volumeScale:0,duration:null,type:null},
 recover:{priority:90,intensityScale:.5,volumeScale:.55,duration:30,type:'recovery'},
 reduce:{priority:75,intensityScale:.7,volumeScale:.7,duration:40,type:'workout'},
 maintain:{priority:30,intensityScale:1,volumeScale:1,duration:50,type:'workout'},
 progress:{priority:45,intensityScale:1.05,volumeScale:1.05,duration:55,type:'workout'},
 goal_focus:{priority:60,intensityScale:.9,volumeScale:.9,duration:50,type:'workout'}
});
const TITLES=Object.freeze({
 recover:{ko:'회복 중심 세션',en:'Recovery-focused session'},
 reduce:{ko:'강도 조정 세션',en:'Reduced-load session'},
 maintain:{ko:'계획된 훈련 세션',en:'Planned training session'},
 progress:{ko:'점진적 향상 세션',en:'Progression session'},
 goal_focus:{ko:'목표 집중 세션',en:'Goal-focused session'}
});
const COPY=Object.freeze({
 collect_data:{ko:'판단할 데이터가 아직 부족합니다. 최근 체크인과 기록을 더 쌓아 주세요.',en:'There is not enough recent data for a confident training decision yet.'},
 caution:{ko:'통증·주의 신호가 있어 훈련 강도를 자동 제안하지 않습니다.',en:'A pain/caution signal is present, so GARANG will not suggest training intensity automatically.'},
 recover:{ko:'최근 부하와 회복 신호를 함께 보면 회복 중심 선택이 우선입니다.',en:'Recent load and recovery signals favor a recovery-focused choice.'},
 reduce:{ko:'오늘은 평소보다 강도나 볼륨을 낮추는 편이 적절합니다.',en:'Reducing intensity or volume is the safer fit for today.'},
 maintain:{ko:'현재 신호에서는 계획한 훈련을 유지할 수 있는 범위입니다.',en:'Current signals support keeping the planned training level.'},
 progress:{ko:'회복과 부하가 안정적이라 작은 점진적 향상을 시도할 수 있습니다.',en:'Recovery and load are stable enough for a small, controlled progression.'},
 goal_focus:{ko:'현재 상태는 훈련 가능하지만 목표와 최근 행동의 정렬을 더 높일 필요가 있습니다.',en:'You can train, but recent behavior could align more closely with your goal.'}
});
...
function actionProposal(mode,reasons,confidence){
 if(!TITLES[mode]||confidence<.25)return null;
