...
<section class="card meal-scan-card">
 ...
 ${unmatched.length?`<div class="meal-scan-unmatched"><strong>자동 계산 보류</strong>${unmatched.map(i=>`<span>${esc(i.name)} · 약 ${Math.round(i.grams)}g</span>`).join('')}<small>신뢰할 수 있는 DB 또는 웹 출처를 확인하지 못했습니다. 아래 직접 입력으로 보정해 주세요.</small></div>`:''}</div>`:`<button id="pickMealScan" class="meal-scan-empty"><span class="camera-glyph">◎</span><strong>식사 사진 추가</strong><small>촬영하면 음식 인식과 영양 계산을 한 번에 시작합니다.</small></button>`}
</section>
