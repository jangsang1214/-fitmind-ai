...
<section class="card meal-scan-card">
 <div class="meal-scan-header"><div><span class="eyebrow">MEAL SCAN</span><h3>사진 한 장으로 기록</h3><p>GARANG DB를 먼저 쓰고, 없으면 신뢰 가능한 웹 출처를 찾아 계산합니다.</p></div><button id="pickMealScan" class="${scan?.url?'ghost':'primary'}">${scan?.url?'사진 변경':'사진 촬영 / 선택'}</button></div>
...
async function analyzeMealScan(){
 ...
 if(SERVICES.mealScanEndpoint){
  try{
   ...
   captureError('meal_scan',e);
   const noFood=e?.code==='MEAL_SCAN_NO_FOOD_DETECTED'||e?.message==='MEAL_SCAN_NO_FOOD_DETECTED';
   toast(noFood?'음식이 확인되지 않았습니다. 음식이 잘 보이게 다시 촬영해 주세요.':e?.message==='MEAL_SCAN_IMAGE_DECODE_FAILED'?'이 사진 형식은 분석용으로 변환하지 못했습니다. JPG/PNG 사진을 사용해 주세요.':'Vision 분석에 실패했습니다. 임의 결과를 만들지 않습니다.');
   return;
  }
 }
