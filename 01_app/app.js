...
${scan?.url?`<section class="card scan-confirm-panel photo-evidence-analysis"><div class="photo-evidence-head"><div><span class="eyebrow">MEAL EVIDENCE</span><h3>${scan?.items?.length?'분석 초안':'사진 분석'}</h3><p>${scan?.items?.length?'GARANG 추정값 · 저장 전 음식과 양을 확인하세요.':'사진에서 음식 후보를 만든 뒤 직접 확인합니다.'}</p></div><span class="photo-evidence-optional">${SERVICES.mealScanEndpoint?'VISION':'DB ASSIST'}</span></div><div class="form-grid"><div class="field"><label>음식명 / 보정</label><input id="scanFoodName" value="${esc(scan?.manualName||'')}" placeholder="예: 닭가슴살 샐러드"></div><div class="field"><label>예상 중량 g</label><input id="scanFoodGram" type="number" min="1" value="${scan?.grams||100}"></div></div><div class="actions photo-evidence-analysis-actions"><button id="analyzeMealScan" class="primary">${SERVICES.mealScanEndpoint?'사진 분석':'DB 초안 생성'}</button><button id="clearMealScan" class="ghost">사진 제거</button></div>${scan?.items?.length?`<div class="scan-result-card photo-evidence-result">${scan.items.map(i=>`<div><strong>${esc(i.name)}</strong><span>${i.grams}g</span><b>${Math.round(i.kcal)} kcal</b><small>P ${Math.round(i.protein)} · C ${Math.round(i.carbs)} · F ${Math.round(i.fat)}</small></div>`).join('')}</div><div class="photo-evidence-estimate-note"><span>GARANG 추정값</span><small>저장 전 음식과 양을 확인하세요.</small></div><button id="confirmMealScan" class="primary wide">확인하고 식사에 추가</button>`:''}</section>`:''}
