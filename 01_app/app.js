...
const coachSummarySafe=(score,lang)=>window.GarangPerformance?GarangPerformance.coachSummary(score,lang):'';
...
const KOREAN_PROCESSED_MANIFEST_PATH='04_data/knowledge/food-db-supplemental-korea-processed-v1/manifest.json';
const KOREAN_PROCESSED_BASE_PATH='04_data/knowledge/food-db-supplemental-korea-processed-v1/';
let knowledge = [];
...
function normalizedNutritionSource(value){
...
function normalizeMeals(){
  state.meals=(Array.isArray(state.meals)?state.meals:[]).map(m=>{
    const items=Array.isArray(m.items)&&m.items.length?m.items.map(normalizeMealItem):[normalizeMealItem({id:uid(),name:String(m.name||'음식'),grams:num(m.grams,100),kcal:num(m.kcal),protein:num(m.protein),carbs:num(m.carbs),fat:num(m.fat),foodId:m.foodId||null,nutritionStatus:m.nutritionStatus||'unknown',nutritionSource:m.nutritionSource||null,userOverride:m.userOverride===true})];
...
function normalizeState(){
  ...
  for(const k of ['checkins','planner','workouts','meals','runs','body','physiologicalSignals','aiChat','actionLog','errors']) state[k]=Array.isArray(state[k])?state[k]:[];
...
const FIRST_RECORD_EVENT_TYPES=Object.freeze({workout_saved:'workout',meal_saved:'nutrition',run_saved:'running',inbody_saved:'body'});
function saveState(opts={}){const event=opts.event||'state_saved',source=opts.source||'app';writeLocal();trackEvent(event,{source},true);const recordType=FIRST_RECORD_EVENT_TYPES[event];if(recordType&&!state.meta.firstRecordTrackedAt){state.meta.firstRecordTrackedAt=isoNow();trackEvent('first_record_created',{recordType,source},true);}if(firebaseReady&&currentUser)queueCloudSync();updateSyncUI();emitLifecycle('garang:state-updated',{source,event});}
...
function cloudPayload(){
  const out=clone(state);delete out.syncState;delete out.analytics;delete out.errors;
...
document.addEventListener('visibilitychange',()=>{if(document.hidden)pauseRunForBackground('visibility_hidden');else syncRunPauseUI();});window.addEventListener('pagehide',()=>pauseRunForBackground('pagehide'));
...
async function loadDB(){const r=await Promise.allSettled([loadJSON('04_data/knowledge/exercise-db.json'),loadJSON('04_data/knowledge/food-db.json'),loadJSONL('04_data/knowledge/exercise_knowledge.jsonl'),loadJSONL('04_data/knowledge/food_knowledge.jsonl'),loadJSONL('04_data/knowledge/fitmind_rules.jsonl')]);if(r[0].status==='fulfilled')db.exercise=r[0].value;if(r[1].status==='fulfilled')db.food=r[1].value;knowledge=r.slice(2).filter(x=>x.status==='fulfilled').flatMap(x=>x.value).slice(0,1800);if(!db.exercise.length||!db.food.length)toast('운동/식단 데이터 일부를 불러오지 못했습니다.');}
...
function initFirebase(){
  try{
    ...
    if(firebaseReady&&typeof firebase.auth().getRedirectResult==='function')firebase.auth().getRedirectResult().catch(e=>{captureError('social_auth_redirect',e);toast(firebaseError(e));});
...
async function socialAuth(kind){
  if(!firebaseReady)return toast('Firebase 설정을 확인해 주세요.');
  const p=kind==='google'?new firebase.auth.GoogleAuthProvider():new firebase.auth.OAuthProvider('apple.com');
  try{
    if(prefersRedirectAuth()){trackEvent('social_auth_redirect_started',{provider:kind},false);await firebase.auth().signInWithRedirect(p);return;}
    await firebase.auth().signInWithPopup(p);
  }catch(e){
    if(['auth/popup-blocked','auth/operation-not-supported-in-this-environment','auth/cancelled-popup-request'].includes(e?.code)&&firebase.auth().signInWithRedirect){
      trackEvent('social_auth_redirect_fallback',{provider:kind,code:e?.code||null},false);
      try{await firebase.auth().signInWithRedirect(p);return;}catch(redirectError){return toast(firebaseError(redirectError));}
    }
...
function bodyModelPaths(gender='male'){
  ...
  return {
    front:female
      ?`<path class="body-silhouette" d="M80 9c-10 0-17 8-17 18 0 8 5 14 10 17l-14 9c-8 5-13 13-15 23l-8 44-13 42 12 5 17-38 9-32 4 57-6 45-9 101h23l7-82 7 82h23l-9-101-6-45 4-57 9 32 17 38 12-5-13-42-8-44c-2-10-7-18-15-23l-14-9c5-3 10-9 10-17 0-10-7-18-17-18Z"/>`
      :`<path class="body-silhouette" d="M80 8c-11 0-19 8-19 19 0 8 4 14 11 18L50 52c-11 4-18 13-21 25l-12 45-10 38 14 5 18-38 10-32 4 60-7 45-8 100h25l10-81h14l10 81h25l-8-100-7-45 4-60 10 32 18 38 14-5-10-38-12-45c-3-12-10-21-21-25l-22-7c7-4 11-10 11-18C99 16 91 8 80 8Z"/>`,
    back:female
      ?`<path class="body-silhouette" d="M80 9c-10 0-17 8-17 18 0 8 5 14 10 17l-14 9c-8 5-13 13-15 23l-8 44-13 42 12 5 17-38 9-32 4 57-6 45-9 101h23l7-82 7 82h23l-9-101-6-45 4-57 9 32 17 38 12-5-13-42-8-44c-2-10-7-18-15-23l-14-9c5-3 10-9 10-17 0-10-7-18-17-18Z"/>`
      :`<path class="body-silhouette" d="M80 8c-11 0-19 8-19 19 0 8 4 14 11 18L50 52c-11 4-18 13-21 25l-12 45-10 38 14 5 18-38 10-32 4 60-7 45-8 100h25l10-81h14l10 81h25l-8-100-7-45 4-60 10 32 18 38 14-5-10-38-12-45c-3-12-10-21-21-25l-22-7c7-4 11-10 11-18C99 16 91 8 80 8Z"/>`
  };
}
function anatomyFigureSvg(side='front',gender=userGender()){
  const female=gender==='female', p=bodyModelPaths(gender)[side], back=side==='back';
  const defs=`<defs><linearGradient id="skin-${side}-${gender}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#f0ede6"/><stop offset=".55" stop-color="#b7b1a8"/><stop offset="1" stop-color="#777168"/></linearGradient><linearGradient id="muscle-${side}-${gender}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#e17361"/><stop offset="1" stop-color="#9d3f32"/></linearGradient></defs>`;
  const frontMuscles=`
    <path class="muscle-zone muscle-shoulders" d="M49 57c7-8 15-10 22-7l-5 19c-8 0-14-3-17-12Zm62 0c-7-8-15-10-22-7l5 19c8 0 14-3 17-12Z"/>
    <path class="muscle-zone muscle-chest" d="M64 67c5-5 11-7 16-5v28c-11 0-18-5-20-13Zm32 0c-5-5-11-7-16-5v28c11 0 18-5 20-13Z"/>
    <path class="muscle-zone muscle-biceps" d="M43 77c8 1 11 8 9 19l-8 27c-5-1-8-5-7-12l3-28Zm74 0c-8 1-11 8-9 19l8 27c5-1 8-5 7-12l-3-28Z"/>
    <path class="muscle-zone muscle-triceps" d="M34 86c5 3 7 9 5 18l-7 25-6-3 5-29Zm92 0c-5 3-7 9-5 18l7 25 6-3-5-29Z"/>
    <path class="muscle-zone muscle-core" d="M69 92h10v20H68Zm12 0h10l1 20H81Zm-13 22h11v20H67Zm13 0h11l1 20H81Zm-14 22h12v18H65Zm14 0h12l2 18H81Z"/>
    <path class="muscle-zone muscle-legs" d="M58 162c10-7 18-4 21 8l-6 50-17 2-8-35Zm44 0c-10-7-18-4-21 8l6 50 17 2 8-35ZM54 227c9-5 16 0 17 12l-8 58H49Zm52 0c-9-5-16 0-17 12l8 58h14Z"/>`;
  const backMuscles=`
    <path class="muscle-zone muscle-shoulders" d="M48 57c8-9 16-10 24-7l-7 19c-9 0-14-4-17-12Zm64 0c-8-9-16-10-24-7l7 19c9 0 14-4 17-12Z"/>
    <path class="muscle-zone muscle-back" d="M64 55c5-6 11-8 16-7 5-1 11 1 16 7l10 35-12 42-14-18-14 18-12-42Z"/>
    <path class="muscle-zone muscle-triceps" d="M41 76c8 4 10 10 7 20l-8 30c-6-2-8-8-6-15l4-27Zm78 0c-8 4-10 10-7 20l8 30c6-2 8-8 6-15l-4-27Z"/>
    <path class="muscle-zone muscle-core" d="M72 116h16l6 32-14 16-14-16Z"/>
    <path class="muscle-zone muscle-legs" d="M57 157c9-5 16-1 23 10 7-11 14-15 23-10l8 29-10 35-16-6-5-29-5 29-16 6-10-35Zm-3 68c8-4 15 1 17 13l-8 59H49Zm52 0c-8-4-15 1-17 13l8 59h14Z"/>`;
  const etched=back
   ?`<path class="anatomy-line" d="M80 49v111M59 70c10 9 32 9 42 0M58 89c11 7 33 7 44 0M64 123c8 7 24 7 32 0M57 176c8 8 15 10 23 10m23-10c-8 8-15 10-23 10M54 240c6 4 11 5 16 4m36-4c-6 4-11 5-16 4"/>`
   :`<path class="anatomy-line" d="M80 49v112M64 74c8 5 24 5 32 0M67 100h26M67 122h26M65 144h30M56 177c8 5 16 7 24 5m24-5c-8 5-16 7-24 5M54 238c7 3 12 4 17 2m35-2c-7 3-12 4-17 2"/>`;
  const detail=`<path class="anatomy-line fine" d="M55 62l13 16m37-16L92 78M45 97l10 9m60-9-10 9M61 169l14 19m24-19-14 19M55 247l12 14m38-14-12 14"/>`;
  return `<svg viewBox="0 0 160 320" role="img" aria-label="${female?'여성':'남성'} ${back?'후면':'전면'} 근육 지도">${defs}<g class="anatomy-base">${p}</g>${back?backMuscles:frontMuscles}${etched}${detail}</svg>`;
}
function muscleMapSvg(active='full',id='muscleMap',extraClass='',gender=userGender()){
  const female=gender==='female';
  return `<div class="muscle-map-wrap ${extraClass}"><div class="muscle-map anatomical-pro" id="${id}" data-muscle="${active}" data-gender="${gender}">
  <div class="body-view"><span>FRONT · ${female?'WOMEN':'MEN'}</span>${anatomyFigureSvg('front',gender)}</div>
  <div class="body-view"><span>BACK · ${female?'WOMEN':'MEN'}</span>${anatomyFigureSvg('back',gender)}</div>
  </div></div>`;
}
function miniBodySvg(active='full',gender=userGender()){
  const female=gender==='female';
  return `<svg class="mini-body-svg mini-anatomy" viewBox="0 0 70 120" aria-hidden="true" data-gender="${gender}" data-muscle="${active}">
  <path class="mini-base" d="${female?'M35 4c-5 0-8 4-8 9 0 4 2 7 5 9l-9 5-8 25 7 3 8-18 2 22-4 18-3 39h9l1-31 1 31h9l-3-39-4-18 2-22 8 18 7-3-8-25-9-5c3-2 5-5 5-9 0-5-3-9-8-9Z':'M35 4c-5 0-9 4-9 9 0 4 2 7 5 9l-11 4-9 26 7 3 10-19 2 25-4 18-3 37h10l2-31 2 31h10l-3-37-4-18 2-25 10 19 7-3-9-26-11-4c3-2 5-5 5-9 0-5-4-9-9-9Z'}"/>
  <path class="mini-zone mini-chest" d="M27 28c4-4 7-4 8-2v12c-5 0-8-3-9-7Zm16 0c-4-4-7-4-8-2v12c5 0 8-3 9-7Z"/><path class="mini-zone mini-back" d="M27 25q8-6 16 0l4 19-12 13-12-13Z"/><circle class="mini-zone mini-shoulders" cx="24" cy="27" r="5"/><circle class="mini-zone mini-shoulders" cx="46" cy="27" r="5"/><path class="mini-zone mini-biceps" d="M19 34q5 1 4 9l-3 11-4-1 2-14Zm32 0q-5 1-4 9l3 11 4-1-2-14Z"/><path class="mini-zone mini-triceps" d="M16 37q4 2 3 9l-3 11-4-2 3-13Zm38 0q-4 2-3 9l3 11 4-2-3-13Z"/><path class="mini-zone mini-core" d="M30 40h10v24H30Z"/><path class="mini-zone mini-legs" d="M27 65q6-5 8 4l-3 45h-9Zm16 0q-6-5-8 4l3 45h9Z"/><path class="mini-line" d="M35 23v42M30 46h10M28 76l7 5 7-5"/></svg>`;
}
...
function todayPage(){
  ...
  const readiness=dec.readiness??null,decisionTitle=next?.title||dec.title,decisionSummary=next?.summary||dec.summary,decisionMode=loop?'NEXT BEST ACTION':dec.decision,decisionConfidence=loop?Math.round(num(loop.confidence)*100):Math.round(num(dec.confidence)*100);
  ...
  <section class="card today-hero visual-today-hero">
    ...
    <div class="today-decision-panel"><div class="today-score-line"><div class="score-orb" style="--score:${score.total||0}"><div><strong>${scoreText}</strong><span>GARANG SCORE</span></div></div><div><span class="eyebrow">GARANG ${decisionMode} · 신뢰 ${decisionConfidence}%</span><h2>${esc(decisionTitle)}</h2></div></div><p class="today-decision-copy">${esc(decisionSummary)}</p>${loop?.conflictResolution?.conflicts?.length?`<div class="helper">충돌 해결 · ${esc(loop.conflictResolution.conflicts.join(" · "))}</div>`:""}<div class="hero-actions">${loop?`<button class="primary" data-action="performance-decision-accept" ${loop.interaction?.status==="accepted"?"disabled":""}>${loop.interaction?.status==="accepted"?"적용됨":"이 행동 적용"}</button><button class="ghost" data-action="performance-decision-modify">수정</button><button class="ghost" data-action="performance-decision-reject">건너뛰기</button>`:`<button class="primary" data-action="apply-coach-plan">오늘 계획 적용</button><button class="ghost" data-pagego="coach">분석 보기</button>`}</div>${loop?`<div class="helper">추천 상태 · ${esc(decisionInteractionLabel(loop))} · 추천→실행→결과 ID 연결</div>`:""}</div>
  </section>
...
function workoutDetailRows(record){return Array.isArray(record?.setDetails)&&record.setDetails.length?record.setDetails:Array.isArray(record?.setsDetail)&&record.setsDetail.length?record.setsDetail:[];}
function workoutRecordVolume(record){const details=workoutDetailRows(record);if(details.length)return details.reduce((sum,row)=>sum+Math.max(0,num(row?.weight??row?.w))*Math.max(0,num(row?.reps??row?.r)),0);const stored=Number(record?.volume);if(Number.isFinite(stored)&&stored>=0)return stored;return Math.max(0,num(record?.sets))*Math.max(0,num(record?.reps))*Math.max(0,num(record?.weight));}
function workoutRecordMaxWeight(record){const details=workoutDetailRows(record);return details.length?Math.max(...details.map(row=>Math.max(0,num(row?.weight??row?.w)))):Math.max(0,num(record?.weight));}
function workoutRecordEstimated1RM(record){const details=workoutDetailRows(record);if(details.length)return Math.max(...details.map(row=>calcEstimated1RM(row?.weight??row?.w,row?.reps??row?.r)));return num(record?.estimated1RM)||calcEstimated1RM(record?.weight,record?.reps);}
function bestEstimated1RM(name){const q=String(name||'').trim().toLowerCase();return Math.max(0,...state.workouts.filter(x=>String(x.name||'').trim().toLowerCase()===q).map(workoutRecordEstimated1RM));}
function workoutHistoryFor(name,limit=4){const q=String(name||'').trim().toLowerCase();return state.workouts.filter(x=>String(x.name||'').trim().toLowerCase()===q).slice().reverse().slice(0,Math.max(1,limit));}
function workoutProgressionContext(name){const recent=workoutHistoryFor(name,4).map(w=>({date:w.date,estimated1RMMetric:workoutRecordEstimated1RM(w),volumeMetric:workoutRecordVolume(w),rpe:num(w.rpe,8),notes:String(w.notes||''),setDetails:workoutDetailRows(w)})),readiness=window.GarangWorkoutIntelligence?.readiness?.(state)||null;return {recent,readiness,goalClass:currentGoalClass()};}
function latestWorkoutNote(name){return String(workoutHistoryFor(name,1)[0]?.notes||'').trim();}

const ux=(ko,en)=>uiLang()==='en'?en:ko;
function renderWorkoutInsights(){
 if(!window.GarangPerformance?.workoutInsights)return '';
 const x=GarangPerformance.workoutInsights(state,db.exercise),fmt=n=>Math.round(num(n)).toLocaleString(),recent7=x.records.filter(r=>withinDays(r.date,7)&&dateMs(r.date)<=dateMs(today())),recent30=x.records.filter(r=>withinDays(r.date,30)&&dateMs(r.date)<=dateMs(today())),volume7=recent7.reduce((s,r)=>s+num(r.volume),0),volume30=recent30.reduce((s,r)=>s+num(r.volume),0),sessions7=new Set(recent7.map(r=>r.sessionId||r.id)).size,sessions30=new Set(recent30.map(r=>r.sessionId||r.id)).size,loads=window.GarangWorkoutIntelligence?.analyzeMuscleLoad?.(state,db.exercise,{days:7})||[];
 ...
 const row=e=>`<article class="pr-row"><div class="pr-head"><div><span class="muscle-chip">${esc(e.primaryMuscle)}</span><strong>${esc(e.name)}</strong></div><small>${esc(e.latest?.date||'—')}</small></div><div class="pr-metrics"><span><small>${ux('최고 중량','Max weight')}</small><b>${e.maxWeight?`${Number(shownWeight(e.maxWeight.weight,1)).toFixed(1)} ${weightUnit()}`:'—'}</b></span><span><small>${ux('추정 1RM','Est. 1RM')}</small><b>${e.maxEstimated1RM?`${Number(shownWeight(e.maxEstimated1RM.estimated1RM,1)).toFixed(1)} ${weightUnit()}`:'—'}</b></span><span><small>${ux('최고 볼륨','Max volume')}</small><b>${e.maxVolume?`${fmt(shownWeight(e.maxVolume.volume,0))} ${weightUnit()}`:'—'}</b></span></div></article>`;
 const loadRows=loads.slice(0,6).map(m=>`<div class="workout-load-row"><div><strong>${esc(m.label)}</strong><small>${m.sets} sets · RPE ${num(m.avgRPE).toFixed(1)}</small></div><b>${fmt(shownWeight(m.volume,0))} ${weightUnit()}</b><span data-intensity="${esc(m.intensity)}">${esc(String(m.intensity).replaceAll('_',' '))}</span></div>`).join('');
...
function renderRunningInsights(){
 ...
 <div class="insight-grid run-grid">
  ...
  ${card(ux('28일 중앙 페이스','28D median pace'),`${pace(x.recent?.days28?.medianPaceMinPerKm)} /km`,`${ux('거리 가중 평균','Distance-weighted average')} · ${x.recent?.days28?.sessions||0} sessions`)}
...
function workoutPageBase(){
  ...
  const draftVol=Math.round(sum(workoutDraft,x=>x.volume)),draftKcal=Math.round(sum(workoutDraft,x=>x.kcal)),draftMin=Math.round(sum(workoutDraft,x=>x.duration));
  return `${pageHead('LOG / WORKOUT','운동','')}
<section class="card workout-visual-hero workout-hero-v2">${muscleMapSvg(target.key)}<div class="workout-visual-copy"><span class="eyebrow">TARGET · ${userGender()==='female'?'WOMEN':'MEN'}</span><h2 id="muscleTargetLabel">${esc(target.label)}</h2><p class="workout-selected-name">${esc(selected)}</p>${workoutMuscleButtons(workoutMuscleFilter)}<div class="workout-mini-stats"><div><strong>${Math.round(shownWeight(draftVol,0)||0).toLocaleString()}</strong><span>VOLUME ${weightUnit()}</span></div><div><strong>${draftKcal}</strong><span>KCAL</span></div><div><strong>${draftMin}</strong><span>MIN</span></div></div></div></section>
...
<section class="card workout-builder workout-builder-v2"><div class="visual-section-head"><div><span class="eyebrow">SESSION</span><h3>세트 기록</h3></div><span class="pill">${workoutDraft.length} exercises</span></div>
<div class="one-rm-panel"><div class="one-rm-primary"><span>ESTIMATED 1RM</span><strong><b id="oneRmPreview">${Number(shownWeight(72,1)).toFixed(1)}</b><small>${weightUnit()}</small></strong><em>Epley</em></div><div class="one-rm-best"><span>이 종목 최고</span><strong>${best1?Number(shownWeight(best1,1)).toFixed(1):'—'} <small>${weightUnit()}</small></strong><em>estimated</em></div></div>
<div class="form-grid compact-fields workout-fields"><div class="field full"><label>운동</label><div class="workout-exercise-picker"><input id="wName" list="exerciseList" value="${esc(selected)}" placeholder="바벨 벤치프레스"><button id="openWorkoutExerciseSearch" class="workout-exercise-search-button" type="button" aria-label="운동 검색"></button></div><datalist id="exerciseList">${db.exercise.slice(0,700).map(x=>`<option value="${esc(x.exercise_name)}">`).join('')}</datalist></div><div class="field"><label>세트</label><input id="wSets" type="number" min="1" value="3"></div><div class="field"><label>반복</label><input id="wReps" type="number" min="1" value="10"></div><div class="field"><label>중량 ${weightUnit()}</label><input id="wWeight" type="number" min="0" step="0.5" value="${shownWeight(60,1)}"></div><div class="field"><label>RPE</label><input id="wRpe" type="number" min="1" max="10" step="0.5" value="8"></div><div class="field compact-secondary"><label>시간 분</label><input id="wDuration" type="number" min="1" value="15"></div><div class="field compact-secondary"><label>체중 ${weightUnit()}</label><input id="wBody" type="number" min="1" step="0.1" value="${shownWeight(state.profile?.weight||67,1)}"></div><div class="field compact-secondary"><label>세트 유형</label><select id="wSetType"><option value="working">Working</option><option value="warmup">Warm-up</option><option value="drop">Drop</option><option value="failure">Failure</option></select></div><div class="field compact-secondary"><label>RIR</label><input id="wRir" type="number" min="0" max="10" step="1" value="2"></div><div class="field full compact-secondary"><label>운동 메모</label><input id="wNotes" maxlength="160" placeholder="폼, 통증, 다음 세션 메모"></div></div><div class="set-detail-toolbar"><button id="toggleSetDetails" type="button" class="ghost small" aria-expanded="false">세트별 기록</button><span>세트마다 중량·반복·RPE를 다르게 입력</span></div><div class="workout-advanced-tools"><label>그룹<select id="wGroupType"><option value="none">일반</option><option value="superset">Superset</option><option value="circuit">Circuit</option></select></label><label>그룹 ID<input id="wGroupId" maxlength="20" placeholder="A"></label><label>바 프리셋<select id="wBarPreset"><option value="auto">Auto</option><option value="20kg">20 kg</option><option value="15kg">15 kg</option><option value="45lb">45 lb</option></select></label><label>바 중량 <input id="wBarWeight" type="number" min="0" step="0.5" value="${weightUnit()==='lb'?45:20}"></label><label>플레이트 구성<select id="wPlateProfile"><option value="full">Full gym</option><option value="basic">Basic</option></select></label><label>반올림<select id="wPlateRounding"><option value="0">Exact</option><option value="0.5">0.5</option><option value="1">1</option><option value="2.5">2.5</option></select></label><label>목표 중량 <input id="wPlateTarget" type="number" min="0" step="0.5" value="${shownWeight(60,1)}"></label><button id="calcWorkoutPlates" type="button" class="ghost small">Plate 계산</button><output id="workoutPlateResult">—</output><label>워밍업<select id="wWarmupScheme"><option value="standard">50 / 70 / 85%</option><option value="minimal">60 / 80%</option></select></label><button id="generateWorkoutWarmup" type="button" class="ghost small">Warm-up 생성</button><output id="workoutWarmupResult">—</output></div><div id="workoutSetDetails" class="workout-set-details" hidden></div><div class="actions workout-entry-actions"><button id="addWorkout" class="primary">세션에 추가</button><button id="clearWorkoutDraft" class="ghost">초기화</button></div><div id="workoutDraftArea" class="draft-area">${renderWorkoutDraft()}</div><button id="saveWorkoutSession" class="primary wide" ${workoutDraft.length?'':'disabled'}>세션 저장</button><div class="workout-secondary-capabilities"><section class="workout-program-card"><div class="visual-section-head"><div><span class="eyebrow">PROGRAM BUILDER</span><h3>현재 세션을 반복 계획으로</h3></div><span class="pill">1–8 weeks</span></div><div class="form-grid compact-fields"><div class="field full"><label>프로그램 이름</label><input id="wProgramName" maxlength="48" value="${esc((workoutDraft[0]?.name||selected)+' Program')}"></div><div class="field"><label>주 수</label><input id="wProgramWeeks" type="number" min="1" max="8" value="4"></div><div class="field"><label>주당 횟수</label><input id="wProgramFrequency" type="number" min="1" max="7" value="${clamp(num(state.onboarding?.weeklyFrequency,3),1,7)}"></div></div><button id="scheduleWorkoutProgram" class="ghost wide" ${workoutDraft.length?'':'disabled'}>현재 세션으로 프로그램 예약</button><small>현재 초안의 종목·세트 구성을 Planner에 반복 배치합니다. 예약일의 ‘운동 시작’으로 그대로 불러옵니다.</small></section><section class="workout-health-card"><div class="visual-section-head"><div><span class="eyebrow">HEALTH / WEARABLE</span><h3>운동·회복 데이터 연동</h3></div><span class="pill" id="workoutHealthStatus">${window.GarangNativeHealthBridge?'NATIVE READY':'IMPORT / EXPORT'}</span></div><p>네이티브 호스트에서는 Apple Health / Health Connect bridge를 사용하고, 웹에서는 표준 운동 JSON 내보내기와 건강 데이터 파일 가져오기를 지원합니다.</p><div class="actions"><button id="syncWorkoutHealth" class="ghost">${window.GarangNativeHealthBridge?'Health 동기화':'Health 데이터 내보내기'}</button><button id="importHealthSignals" class="ghost">Health 데이터 가져오기</button><input id="healthSignalImport" type="file" accept=".json,.csv,.xml,application/json,text/csv,text/xml,application/xml" hidden></div></section></div></section>
...
function renderWorkoutDraft(){return workoutDraft.length?`<div class="list">${workoutDraft.map((x,i)=>`<div class="list-item ${x.groupType&&x.groupType!=='none'?'is-workout-grouped':''}" data-workout-group="${x.groupType&&x.groupType!=='none'?esc(String(x.groupType)+':'+String(x.groupId||'')):''}"><div>${x.groupType&&x.groupType!=='none'?`<span class="workout-group-chip">${esc(x.groupType.toUpperCase())} ${esc(x.groupId||'')}</span>`:''}<strong>${esc(x.name)}</strong><div class="muted">${x.setDetails?.length?x.setDetails.map(row=>shownWeight(row.weight,1)+weightUnit()+" × "+row.reps+" · RPE "+row.rpe+(row.rir!=null?" · RIR "+row.rir:"")+(row.setType&&row.setType!=="working"?" · "+row.setType.toUpperCase():"")).join(" / "):x.sets+"×"+x.reps+" · "+shownWeight(x.weight,1)+weightUnit()} · 예상 1RM ${Number(shownWeight(x.estimated1RM,1)).toFixed(1)}${weightUnit()}${x.notes?" · "+esc(x.notes):""}</div></div><div class="actions"><button class="ghost small" data-move-workout="${i}" data-dir="-1" ${i===0?"disabled":""}>↑</button><button class="ghost small" data-move-workout="${i}" data-dir="1" ${i===workoutDraft.length-1?"disabled":""}>↓</button><button class="ghost small" data-replace-workout="${i}">${workoutReplaceIndex===i?'새 운동 선택 중':'교체'}</button><button class="ghost small" data-execute-workout="${i}">${String(workoutExecutionDraftId)===String(x.id)?'실행 중':'실행'}</button><button class="ghost small" data-edit-workout="${i}">수정</button><button class="ghost small" data-remove-workout="${i}">삭제</button></div></div>`).join('')}</div>`:'<div class="empty">운동을 추가하면 세션 초안이 여기에 표시됩니다.</div>';}
function mealNutritionSourceView(item){
 const source=item?.nutritionSource||{},provider=String(source?.provider||''),sourceName=String(source?.source||''),web=sourceName==='web_search',labelScan=sourceName==='nutrition_label_scan',kfind=/K-FIND|식품의약품안전처/i.test(provider)||sourceName==='식품영양성분 데이터베이스',usda=/USDA|FoodData Central/i.test(provider)||/USDA FoodData Central/i.test(sourceName);
 const label=web?'WEB ESTIMATE':labelScan?'LABEL SCAN · 확인 필요':kfind?'K-FIND · 식품영양성분 DB':usda?'USDA FDC':'GARANG DB';
 const attributable=web||kfind||usda,link=attributable&&source?.url?`<a class="meal-source-link" href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">출처</a>`:'';
 return `<span class="meal-source-chip ${web?'is-web':'is-db'}">${label}</span>${link}`;
}
function adaptiveNutritionModel(){
 const api=window.GarangAdaptiveNutritionLearningV1;if(!api?.build)return null;
 try{return api.build(state,{asOf:today(),days:28});}catch(error){captureError('adaptive_nutrition_model',error);return null;}
}
function adaptiveNutritionCard(){
 const model=adaptiveNutritionModel(),proposal=model?.recommendation?.targetProposal,current=Number(state.profile?.calorieTarget),currentValid=Number.isFinite(current)&&current>=1000&&current<=5000,source=String(state.profile?.calorieTargetSource||''),age=Number(state.profile?.age);
 if(!model)return '';
 const eligible=proposal?.eligible===true&&Number.isFinite(Number(proposal?.proposedDailyKcal))&&model.confidence>=.55&&Number.isFinite(age)&&age>=18;
 if(!eligible){
  const observed=model?.evidence?.primaryWindow?.intake?.validDays??model?.evidence?.secondaryWindow?.intake?.validDays??0,weightSamples=model?.evidence?.primaryWindow?.weight?.sampleSize??model?.evidence?.secondaryWindow?.weight?.sampleSize??0;
  return `<section class="card adaptive-nutrition-card" data-adaptive-nutrition="collect"><div class="visual-section-head"><div><span class="eyebrow">ADAPTIVE NUTRITION</span><h3>기록이 쌓이면 목표를 조정합니다</h3></div><span class="pill">EVIDENCE ${Math.round(model.confidence*100)}%</span></div><p>섭취와 체중 변화를 함께 보고 유지칼로리와 다음 목표를 제안합니다. 자동 변경하지 않습니다.</p><small>현재 근거 · 섭취 ${observed}일 · 체중 ${weightSamples}회</small>${currentValid&&source==='adaptive_nutrition_learning_v1'?`<div class="actions" style="margin-top:10px"><span class="pill">현재 ${Math.round(current)} kcal</span><button id="resetAdaptiveNutritionTarget" class="ghost small">기본 추정으로 복귀</button></div>`:''}</section>`;
 }
 const maintenance=Math.round(Number(model.estimate?.estimatedMaintenanceKcal)||0),next=Math.round(Number(proposal.proposedDailyKcal)),delta=Math.round(Number(proposal.deltaKcal)||0),same=currentValid&&Math.round(current)===next&&source==='adaptive_nutrition_learning_v1',range=model.estimate?.estimatedMaintenanceRange;
...
function resetAdaptiveNutritionTarget(){
 ...
 const previous=Number(state.profile?.calorieTarget);for(const key of ['calorieTarget','calorieTargetConfidence','calorieTargetSource','calorieTargetUpdatedAt','calorieTargetMaintenanceEstimate'])delete state.profile[key];
 state.memory.events.push({type:'nutrition_target_reset',date:today(),text:'적응형 칼로리 목표를 기본 추정으로 복귀',source:'adaptive_nutrition_learning_v1',previousTarget:Number.isFinite(previous)?Math.round(previous):null});
...
function bodyTrendSvg(metric=bodyTrendMetric,days=bodyRangeDays){
  const arr=bodyTrendData(metric,days),cfg=bodyMetricConfig()[metric];
  if(arr.length<2)return '<div class="empty trend-empty">기록이 2개 이상 쌓이면 변화 그래프가 표시됩니다.</div>';
  const vals=arr.map(x=>num(x[metric])),min=Math.min(...vals),max=Math.max(...vals),pad=Math.max((max-min)*.22,Math.abs(max||1)*.015,.5),lo=min-pad,hi=max+pad,range=hi-lo||1;
  const W=640,H=260,L=34,R=18,T=24,B=30, plotW=W-L-R,plotH=H-T-B;
  const pts=vals.map((v,i)=>({x:L+(i/(vals.length-1))*plotW,y:T+(1-(v-lo)/range)*plotH,v}));
  const line=pts.map(p=>`${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' '), area=`${L},${H-B} ${line} ${W-R},${H-B}`;
  const delta=vals.at(-1)-vals[0],deltaText=`${delta>=0?'+':''}${delta.toFixed(1)} ${cfg.unit}`;
  return `<div class="body-chart-wrap"><svg class="body-trend-svg" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet" aria-label="${cfg.label} 변화"><defs><linearGradient id="bodyArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#c75b49" stop-opacity=".26"/><stop offset="1" stop-color="#c75b49" stop-opacity="0"/></linearGradient></defs><g class="chart-grid"><line x1="${L}" y1="${T}" x2="${W-R}" y2="${T}"/><line x1="${L}" y1="${T+plotH/2}" x2="${W-R}" y2="${T+plotH/2}"/><line x1="${L}" y1="${H-B}" x2="${W-R}" y2="${H-B}"/></g><polygon class="chart-area" points="${area}"/><polyline class="chart-line" points="${line}"/>${pts.map((p,i)=>`<circle class="chart-dot ${i===pts.length-1?'latest':''}" cx="${p.x}" cy="${p.y}" r="${i===pts.length-1?5:3}"/>`).join('')}</svg><div class="body-chart-summary"><div><span>${arr[0].date}</span><b>${vals[0].toFixed(1)} ${cfg.unit}</b></div><div class="body-chart-delta"><span>CHANGE</span><b>${deltaText}</b></div><div><span>${arr.at(-1).date}</span><b>${vals.at(-1).toFixed(1)} ${cfg.unit}</b></div></div></div>`;
}
...
<section class="card body-hero body-hero-v2"><div class="body-hero-visual"><div class="body-hero-anatomy" data-muscle="full">${miniBodySvg('full')}</div><div><span class="eyebrow">LATEST BODY</span><strong>${b?.weight?`${b.weight} kg`:'기록 시작'}</strong><small>${userGender()==='female'?'WOMEN':'MEN'} · ${b?.date||today()}</small></div></div><div class="body-hero-metrics"><div><span>골격근</span><b>${b?.muscle?shownWeight(b.muscle,1):'—'}<small>${weightUnit()}</small></b></div><div><span>체지방</span><b>${b?.fatPercent??'—'}<small>%</small></b></div><div><span>BMI</span><b>${b?.bmi??'—'}</b></div><div><span>BMR</span><b>${b?.bmr??'—'}<small>kcal</small></b></div></div></section>
<section class="card body-trend-card body-trend-primary"><div class="visual-section-head"><div><span class="eyebrow">TREND</span><h3>${currentCfg.label} 변화</h3></div><div class="body-export-actions"><button id="exportBodyImage" class="ghost small">이미지</button><button id="exportBodyCSV" class="ghost small">CSV</button></div></div><div class="body-metric-tabs">${Object.entries(cfg).map(([k,v])=>`<button data-body-metric="${k}" class="${bodyTrendMetric===k?'active':''}">${v.label}</button>`).join('')}</div><div class="body-range-tabs">${[[7,'7일'],[30,'30일'],[90,'90일'],[365,'1년'],[0,'전체']].map(([n,l])=>`<button data-body-range="${n}" class="${bodyRangeDays===n?'active':''}">${l}</button>`).join('')}</div><div id="bodyTrendChart">${bodyTrendSvg()}</div></section>
<details class="card body-entry-drawer"><summary><div><span class="eyebrow">NEW MEASUREMENT</span><b>새 체성분 기록</b><small>체중 · 키 · 골격근량 · 체지방률 4개만 입력</small></div><span>＋</span></summary><div class="body-entry-content"><div class="form-grid"><div class="field"><label>측정일</label><input id="bDate" type="date" value="${today()}"></div><div class="field"><label>체중 ${weightUnit()}</label><input id="bWeight" type="number" step="0.1" value="${shownWeight(b?.weight||state.profile?.weight||'',1)}"></div><div class="field"><label>키 ${lengthUnit()}</label><input id="bHeight" type="number" step="0.1" value="${shownLength(state.profile?.height||'',1)}"></div><div class="field"><label>골격근량 ${weightUnit()}</label><input id="bMuscle" type="number" step="0.1" value="${shownWeight(b?.muscle||'',1)}"></div><div class="field full"><label>체지방률 %</label><input id="bFatPct" type="number" step="0.1" value="${b?.fatPercent||''}"></div></div><div class="derived-grid" id="bodyDerived"><div><span>체지방량</span><b>${d.fatMass?d.fatMass.toFixed(1):'—'} kg</b></div><div><span>제지방량</span><b>${d.leanMass?d.leanMass.toFixed(1):'—'} kg</b></div><div><span>BMI</span><b>${d.bmi?d.bmi.toFixed(1):'—'}</b></div><div><span>BMR</span><b>${d.bmr||'—'} kcal</b></div></div><details class="body-attachment-details"><summary>인바디 사진 / PDF <span id="bodyAttachmentName">${esc(bodyAttachmentDraft?.name||'선택 안 함')}</span></summary><div class="body-attachment-actions"><button id="pickBodyAttachment" class="ghost small" type="button">파일 선택</button><small>${SERVICES.bodyScanEndpoint?'OCR 연결 가능':'원본 보관 · OCR 미연결'}</small></div></details><button id="saveBody" class="primary wide body-save-button">체성분 저장</button></div></details>
...
function progressLearningSnapshot(){
  try{
...
function progressLearningHtml(snapshot){
  ...
  const loop=snapshot.decisionLoop,experimentRows=(loop?.experiments||[]).map(x=>`<div class="metric-row"><span>${esc(x.experimentKey)} <small>n=${x.sampleSize} · 신뢰 ${Math.round(num(x.confidence)*100)}%</small></span><b>${x.observedDelta==null?"수집 중":`${x.observedDelta>=0?"+":""}${x.observedDelta}`}</b></div>`).join(""),experimentBlock=loop?`<div class="helper"><strong>Decision Loop v2</strong> · ${esc(loop.nextAction?.title||"근거 수집 중")} · ${esc(loop.status||"prioritized")}</div>${experimentRows?`<details><summary>Personal Experiments</summary>${experimentRows}<div class="helper">개인 실험은 관찰적 패턴이며 인과효과로 해석하지 않습니다.</div></details>`:`<div class="helper">같은 행동의 실행→결과가 2회 이상 쌓이면 개인 실험 패턴을 표시합니다.</div>`}`:"";
  return `<section class="card"><div class="visual-section-head"><div><span class="eyebrow">LONGITUDINAL LEARNING</span><h3>GARANG이 배운 것</h3></div><span class="pill">읽기 전용</span></div>${focusLine}${runLine}${experimentBlock}${rows}<div class="helper"><strong>최근 해석</strong> · ${esc(insight)}</div><div class="helper"><strong>다음 판단</strong> · ${esc(nextText)}</div><div class="helper">추천 → 사용자 반응 → 실행 → 결과가 연결된 근거만 개인화에 사용합니다. 자동 증량이나 무단 변경은 하지 않습니다.</div></section>`;
}
...
function planRow(p){const executable=p.type==='workout'&&Array.isArray(p.templateExercises)&&p.templateExercises.length;return `<div class="plan-row"><div class="plan-time">${esc(p.time||'—')}</div><div><div class="plan-title">${esc(p.title)}</div><div class="plan-meta">${p.date} · ${esc(p.type||'plan')} · ${p.source==='ai'?'<span class="pill violet">AI Suggested</span>':p.source==='workout_program'?'<span class="pill">Program</span>':'<span class="pill">User</span>'} ${p.completed?'<span class="pill jade">Completed</span>':''}</div></div><div class="actions plan-actions">${executable?`<button class="primary small" data-plan-start="${p.id}">운동 시작</button>`:''}<button class="ghost small" data-plan-toggle="${p.id}">${p.completed?'되돌리기':'완료'}</button><button class="ghost small" data-plan-delete="${p.id}">삭제</button></div></div>`;}
...
<section class="card settings-block"><div class="settings-title"><div><span class="eyebrow">DATA & ACCOUNT</span><h3>데이터</h3></div><span class="pill ${state.syncState==='synced'?'jade':''}">${currentUser&&firebaseReady?'CLOUD':'LOCAL'}</span></div><div class="settings-row"><div><strong>Cloud Sync</strong><span>기록은 먼저 기기에 저장되고 이후 동기화됩니다.</span></div><button id="retrySync" class="ghost small">재시도</button></div><label class="settings-row" for="analyticsConsentSetting"><div><strong>익명 사용 데이터 공유</strong><span>선택 사항 · 동의 시 Golden Path 사용 이벤트와 안전하게 정리된 오류만 전송합니다. 원시 건강 기록·채팅·이메일·정밀 위치는 분석 이벤트에 포함하지 않습니다.</span></div><input id="analyticsConsentSetting" type="checkbox" ${state.privacy?.consent?.analytics===true?'checked':''}></label><div class="settings-actions"><button id="exportData" class="ghost">데이터 내보내기</button><button id="importLegacy" class="ghost">구버전 가져오기</button><button id="settingsLogout" class="ghost">로그아웃</button></div></section>
<section class="card settings-block app-info"><div class="metric-row"><span>Version</span><b>${esc(window.GARANG_BUILD?.version||'unknown')}</b></div><div class="metric-row"><span>Build</span><b>${esc(window.GARANG_BUILD?.buildId||'unknown')}</b></div><div class="metric-row"><span>Schema</span><b>v${SCHEMA_VERSION}</b></div><div class="metric-row"><span>Error Log</span><b>${state.errors.length}</b></div></section></div>`;}
...
function bindPage(){
  document.querySelectorAll('[data-pagego]').forEach(b=>b.onclick=event=>{event.preventDefault();navigatePage(b.dataset.pagego,'app-page-link');});
...
function navigatePage(page,source){const route=String(page||'').trim();if(!route)return false;const router=window.GarangRouter;if(router?.navigate?.(route,{source:source||'app-page-link',force:true})===true)return true;go(route);return true;}
...
function readWorkoutSetDetails(){const host=$('workoutSetDetails');if(!host||host.hidden)return [];const rows=[...host.querySelectorAll('[data-set-row]')],execution=rows.some(row=>row.dataset.executionEnhanced==='true'),selected=execution?rows.filter(row=>row.dataset.executionCompleted==='true'):rows;return selected.map((row,index)=>({set:index+1,reps:Math.max(1,num(row.querySelector('[data-set-reps]')?.value,1)),weight:Math.max(0,metricWeight(row.querySelector('[data-set-weight]')?.value)),rpe:clamp(num(row.querySelector('[data-set-rpe]')?.value,8),1,10),rir:clamp(num(row.querySelector('[data-set-rir]')?.value,$('wRir')?.value||2),0,10),setType:String(row.querySelector('[data-set-type]')?.value||$('wSetType')?.value||'working')}));}
function renderWorkoutSetDetails(){const host=$('workoutSetDetails');if(!host)return;const count=Math.max(1,num($('wSets')?.value,1)),rows=workoutSetDraft.length===count?workoutSetDraft:Array.from({length:count},(_,i)=>({set:i+1,reps:num($('wReps')?.value,10),weight:metricWeight($('wWeight')?.value),rpe:num($('wRpe')?.value,8),rir:num($('wRir')?.value,2),setType:String($('wSetType')?.value||'working')}));host.innerHTML=rows.map((row,i)=>'<div class="workout-set-row" data-set-row data-execution-completed="'+(row.executionCompleted===true?'true':'false')+'"><b>SET '+(i+1)+'</b><label>유형<select data-set-type><option value="working"'+(String(row.setType||'working')==='working'?' selected':'')+'>Working</option><option value="warmup"'+(row.setType==='warmup'?' selected':'')+'>Warm-up</option><option value="drop"'+(row.setType==='drop'?' selected':'')+'>Drop</option><option value="failure"'+(row.setType==='failure'?' selected':'')+'>Failure</option></select></label><label>반복<input data-set-reps type="number" min="1" value="'+row.reps+'"></label><label>중량 '+weightUnit()+'<input data-set-weight type="number" min="0" step="0.5" value="'+shownWeight(row.weight,1)+'"></label><label>RPE<input data-set-rpe type="number" min="1" max="10" step="0.5" value="'+row.rpe+'"></label><label>RIR<input data-set-rir type="number" min="0" max="10" step="1" value="'+num(row.rir,2)+'"></label></div>').join('');host.hidden=!workoutSetDetailsOpen;window.dispatchEvent(new CustomEvent('garang:workout-set-rows-rendered',{detail:{count,open:workoutSetDetailsOpen}}));}
function buildWorkout(){const name=$('wName').value.trim();if(!name){toast('운동 종목을 입력해 주세요.');return null;}const executionMode=!!$('workoutSetDetails')?.querySelector('[data-execution-set-complete]'),executionImport=$('addWorkout')?.dataset.executionImport==='true',details=readWorkoutSetDetails();if(executionMode&&!executionImport&&!details.length){toast('완료한 세트를 먼저 체크해 주세요.');return null;}const ref=exerciseFor(name),sets=details.length||Math.max(1,num($('wSets').value,1)),reps=details.length?Math.round(details.reduce((sum,row)=>sum+row.reps,0)/details.length):Math.max(1,num($('wReps').value,1)),weight=details.length?Number((details.reduce((sum,row)=>sum+row.weight,0)/details.length).toFixed(2)):Math.max(0,metricWeight($('wWeight').value)),rpe=details.length?Number((details.reduce((sum,row)=>sum+row.rpe,0)/details.length).toFixed(1)):clamp(num($('wRpe').value,8),1,10),duration=Math.max(1,num($('wDuration').value,15)),body=Math.max(1,metricWeight($('wBody').value)||state.profile?.weight||67),met=num(ref?.met_default,5),volume=details.length?details.reduce((sum,row)=>sum+(row.reps*row.weight),0):sets*reps*weight,estimated1RM=details.length?Math.max(...details.map(row=>calcEstimated1RM(row.weight,row.reps))):calcEstimated1RM(weight,reps);const notes=String($('wNotes')?.value||'').trim(),setType=String($('wSetType')?.value||'working'),rir=clamp(num($('wRir')?.value,2),0,10),groupType=String($('wGroupType')?.value||'none'),groupId=String($('wGroupId')?.value||'').trim();if(groupType!=='none'&&!groupId){toast('Superset/Circuit은 그룹 ID를 입력해 주세요.');return null;}return {id:uid(),name,primaryMuscle:ref?.primary_muscle||'전신/컨디셔닝',sets,reps,weight,rpe,rir,setType,notes,groupType,groupId,duration,body,met,kcal:calcWorkoutKcal({duration,body,rpe,met}),volume,estimated1RM:Number(estimated1RM.toFixed(1)),...(details.length?{setDetails:details}: {})};}
function workoutWarmupPlan(){
  const host=$('workoutSetDetails');if(host?.querySelector('[data-execution-completed="true"]')){toast('진행 중인 세트를 초기화한 뒤 워밍업을 생성해 주세요.');return false;}
  const liveRows=[...($('workoutSetDetails')?.querySelectorAll('[data-set-row]')||[])],liveWeight=liveRows[0]?.querySelector('[data-set-weight]')?.value,workingMetric=Math.max(0,metricWeight(liveWeight??$('wWeight')?.value));if(!workingMetric){toast('먼저 working weight를 입력해 주세요.');return false;}
  const scheme=String($('wWarmupScheme')?.value||'standard'),defs=scheme==='minimal'?[{pct:.6,reps:5},{pct:.8,reps:3}]:[{pct:.5,reps:8},{pct:.7,reps:5},{pct:.85,reps:3}],rounding=Math.max(0,num($('wPlateRounding')?.value)),workingSets=Math.max(1,liveRows.length||num($('wSets')?.value,1)),workingReps=Math.max(1,num(liveRows[0]?.querySelector('[data-set-reps]')?.value??$('wReps')?.value,1)),workingRpe=clamp(num(liveRows[0]?.querySelector('[data-set-rpe]')?.value??$('wRpe')?.value,8),1,10),workingRir=clamp(num(liveRows[0]?.querySelector('[data-set-rir]')?.value??$('wRir')?.value,2),0,10);
  const roundedMetric=pct=>{const display=shownWeight(workingMetric*pct,2),rounded=rounding?Math.round(display/rounding)*rounding:display;return Number(metricWeight(rounded).toFixed(3));},warmups=defs.map(({pct,reps})=>({reps,weight:roundedMetric(pct),rpe:Math.max(1,workingRpe-2),rir:Math.max(3,workingRir),setType:'warmup'})),working=Array.from({length:workingSets},()=>({reps:workingReps,weight:workingMetric,rpe:workingRpe,rir:workingRir,setType:'working'}));
  const plan=[...warmups,...working].map((row,index)=>({set:index+1,...row})),runtimeApplied=window.GarangWorkoutExecutionV2?.replaceSetPlan?.(plan)===true;if(!runtimeApplied){workoutSetDraft=plan;workoutSetDetailsOpen=true;$('wSets').value=String(plan.length);renderWorkoutSetDetails();}$('toggleSetDetails')?.setAttribute('aria-expanded','true');const result=$('workoutWarmupResult');if(result)result.textContent=warmups.length+' warm-up + '+workingSets+' working';return true;
}
function workoutExecutionRows(item){
  if(!item)return [];
  const source=Array.isArray(item._executionRows)&&item._executionRows.length?item._executionRows:(Array.isArray(item.setDetails)&&item.setDetails.length?item.setDetails:Array.from({length:Math.max(1,num(item.sets,1))},(_,i)=>({set:i+1,reps:Math.max(1,num(item.reps,1)),weight:num(item.weight),rpe:clamp(num(item.rpe,8),1,10),rir:clamp(num(item.rir,2),0,10),setType:String(item.setType||'working')})));
  return source.map((row,index)=>({set:index+1,reps:Math.max(1,num(row.reps??row.r,1)),weight:num(row.weightMetric??row.weight??row.w),rpe:clamp(num(row.rpe,8),1,10),rir:clamp(num(row.rir,2),0,10),setType:String(row.setType||'working'),executionCompleted:row.completed===true||row.executionCompleted===true}));
}
function workoutExecutionForm(item,index){
...
function persistWorkoutExecutionRows(rows){
  const item=workoutDraft.find(x=>String(x.id)===String(workoutExecutionDraftId));if(!item||!Array.isArray(rows))return false;
  item._executionRows=rows.map((row,index)=>({set:index+1,weightMetric:num(row.weightMetric??row.weight),reps:Math.max(1,num(row.reps,1)),rpe:clamp(num(row.rpe,8),1,10),rir:clamp(num(row.rir,2),0,10),setType:String(row.setType||'working'),completed:row.completed===true}));
  return true;
}
function workoutGroupExecutionContext(){
  const currentIndex=workoutDraft.findIndex(x=>String(x.id)===String(workoutExecutionDraftId));if(currentIndex<0)return null;
  const current=workoutDraft[currentIndex],groupType=String(current.groupType||'none'),groupId=String(current.groupId||'').trim();if(groupType==='none'||!groupId)return null;
  const key=groupType+':'+groupId.toLowerCase(),members=workoutDraft.map((item,index)=>({item,index})).filter(({item})=>String(item.groupType||'none')+':'+String(item.groupId||'').trim().toLowerCase()===key);
  if(members.length<2)return null;
  const position=members.findIndex(x=>x.index===currentIndex),rows=workoutExecutionRows(current),firstIncomplete=rows.findIndex(row=>row.executionCompleted!==true),round=firstIncomplete<0?rows.length:firstIncomplete,rounds=Math.max(...members.map(({item})=>workoutExecutionRows(item).length));
  return {key,groupType,groupId,currentIndex,position,round,rounds,members:members.map(({item,index})=>({index,id:item.id,name:item.name,sets:workoutExecutionRows(item).length,completed:workoutExecutionRows(item).filter(row=>row.executionCompleted===true).length}))};
}
function nextWorkoutGroupTarget(setIndex){
  const context=workoutGroupExecutionContext();if(!context)return null;const n=context.members.length,start=context.position;
  for(let step=1;step<=Math.max(1,n*context.rounds);step++){
    const absolute=start+step,pos=absolute%n,round=Math.max(0,num(setIndex))+Math.floor(absolute/n),member=context.members[pos];if(round>=member.sets)continue;
    const item=workoutDraft[member.index],row=workoutExecutionRows(item)[round];if(row?.executionCompleted===true)continue;
    return {target:{index:member.index,setIndex:round,name:member.name},roundEnded:pos===0,groupComplete:false,groupType:context.groupType,groupId:context.groupId};
  }
  return {target:null,roundEnded:true,groupComplete:true,groupType:context.groupType,groupId:context.groupId};
}
function finalizedWorkoutDraftItem(item){
  const copy=clone(item),rows=Array.isArray(copy._executionRows)?copy._executionRows.filter(row=>row.completed===true):[];delete copy._executionRows;
  if(!rows.length)return copy;
  const details=rows.map((row,index)=>({set:index+1,reps:Math.max(1,num(row.reps,1)),weight:Math.max(0,num(row.weightMetric)),rpe:clamp(num(row.rpe,8),1,10),rir:clamp(num(row.rir,2),0,10),setType:String(row.setType||'working')})),sets=details.length,reps=Math.round(details.reduce((sum,row)=>sum+row.reps,0)/sets),weight=Number((details.reduce((sum,row)=>sum+row.weight,0)/sets).toFixed(2)),rpe=Number((details.reduce((sum,row)=>sum+row.rpe,0)/sets).toFixed(1)),volume=details.reduce((sum,row)=>sum+row.weight*row.reps,0),estimated1RM=Math.max(...details.map(row=>calcEstimated1RM(row.weight,row.reps)));
  return {...copy,sets,reps,weight,rpe,volume,estimated1RM:Number(estimated1RM.toFixed(1)),setDetails:details};
}
function bindWorkout(){
  const name=$('wName');
  $('openWorkoutExerciseSearch')?.addEventListener('click',()=>{document.querySelector('[data-gws-step="exercise"]')?.click();requestAnimationFrame(()=>requestAnimationFrame(()=>{const search=document.querySelector('.garang-exercise-search input');if(!search)return;search.scrollIntoView({block:'center',behavior:'smooth'});try{search.focus({preventScroll:true});}catch{search.focus();}}));});
  name?.addEventListener('change',()=>{const picked=String(name.value||'').trim();if(!picked||!exerciseFor(picked))return;workoutSelectedExercise=picked;workoutMuscleFilter=exerciseMuscle(picked).key;});
  if(workoutEditForm){const edit=workoutEditForm;workoutEditForm=null;const values={wName:edit.name,wSets:edit.sets,wReps:edit.reps,wWeight:edit.weight,wRpe:edit.rpe,wRir:edit.rir,wSetType:edit.setType,wNotes:edit.notes,wGroupType:edit.groupType,wGroupId:edit.groupId,wDuration:edit.duration,wBody:edit.body};for(const [id,value] of Object.entries(values)){const node=$(id);if(node&&value!==undefined&&value!==null)node.value=String(value);}workoutSelectedExercise=edit.name||workoutSelectedExercise;}
  window.GarangWorkoutExecutionBridge=Object.freeze({version:'workout-execution-v2',toMetricWeight(value){return metricWeight(value);},displayWeight(value,digits=1){return shownWeight(value,digits);},displayUnit(){return weightUnit();},resetExerciseRows(){workoutSetDraft=[];workoutSetDetailsOpen=true;renderWorkoutSetDetails();},previousSets(exerciseName){const q=String(exerciseName||'').trim().toLowerCase(),prev=state.workouts.slice().reverse().find(x=>String(x.name||'').trim().toLowerCase()===q);if(!prev)return [];const rows=workoutDetailRows(prev);return (rows.length?rows:Array.from({length:Math.max(1,num(prev.sets,1))},(_,i)=>({set:i+1,weight:num(prev.weight),reps:num(prev.reps),rpe:num(prev.rpe),rir:num(prev.rir,Math.max(0,10-num(prev.rpe,8))),setType:String(prev.setType||'working')}))).map(row=>{const weightMetric=num(row.weight??row.w),reps=num(row.reps??row.r),rmMetric=calcEstimated1RM(weightMetric,reps);return {weightMetric,weight:Number(shownWeight(weightMetric,1)),reps,rpe:num(row.rpe),rir:num(row.rir,Math.max(0,10-num(row.rpe,8))),setType:String(row.setType||'working'),estimated1RMMetric:rmMetric,estimated1RM:Number(shownWeight(rmMetric,1))};});},previousContext(exerciseName){return {notes:latestWorkoutNote(exerciseName),...workoutProgressionContext(exerciseName)};},progressionContext(exerciseName){return workoutProgressionContext(exerciseName);},healthExport(){return {schema:'garang-health-workout-v1',exportedAt:isoNow(),workouts:state.workouts.slice(-350).map(w=>({id:w.id,date:w.date,name:w.name,duration:w.duration,kcal:w.kcal,sets:workoutDetailRows(w).length?workoutDetailRows(w):Array.from({length:Math.max(1,num(w.sets,1))},(_,i)=>({set:i+1,weight:num(w.weight),reps:num(w.reps),rpe:num(w.rpe),rir:num(w.rir)}))}))};},draftPRComparisons(){return workoutDraft.map(x=>{const q=String(x.name||'').trim().toLowerCase(),history=state.workouts.filter(w=>String(w.name||'').trim().toLowerCase()===q),baseline=history.reduce((best,w)=>Math.max(best,workoutRecordEstimated1RM(w)),0);return {name:x.name,baselineMetric:baseline,sessionMetric:num(x.estimated1RM)};});},exercisePRBaseline(exerciseName){return bestEstimated1RM(exerciseName);},sessionDraft(){return clone(workoutDraft);},sessionContext(){return {activePlanId:workoutActivePlanId||null,executionDraftId:workoutExecutionDraftId||null};},restoreSessionContext(value){workoutActivePlanId=value?.activePlanId?String(value.activePlanId):null;workoutExecutionDraftId=value?.executionDraftId?String(value.executionDraftId):null;return true;},restoreSessionDraft(items){if(!Array.isArray(items))return false;workoutDraft=items.map(x=>clone(x)).filter(Boolean);const active=workoutDraft.findIndex(x=>String(x.id)===String(workoutExecutionDraftId));if(active>=0){stageWorkoutDraftExecution(active,false);}else workoutSelectedExercise=workoutDraft.at(-1)?.name||workoutSelectedExercise;render();return true;},persistExecutionRows(rows){return persistWorkoutExecutionRows(rows);},groupExecutionContext(){return workoutGroupExecutionContext();},nextGroupedExecution(setIndex){return nextWorkoutGroupTarget(setIndex);},activateGroupedExercise(index){return stageWorkoutDraftExecution(index,true);},draftSummary(){return {exercises:workoutDraft.length,sets:workoutDraft.reduce((n,x)=>n+Math.max(1,num(x.sets,1)),0),volume:Number(shownWeight(workoutDraft.reduce((n,x)=>n+num(x.volume),0),0)),unit:weightUnit()};}});
  const update1RM=()=>{const el=$('oneRmPreview');if(el){const details=readWorkoutSetDetails(),rm=details.length?Math.max(...details.map(row=>calcEstimated1RM(row.weight,row.reps))):calcEstimated1RM(metricWeight($('wWeight')?.value),$('wReps')?.value);el.textContent=Number(shownWeight(rm,1)||0).toFixed(1);}};
  ...
  $('wWeight')?.addEventListener('input',update1RM);$('wReps')?.addEventListener('input',update1RM);$('workoutSetDetails')?.addEventListener('input',update1RM);update1RM();
  if(!workoutSetBridgeBound){window.addEventListener('garang:set-options-toggled',event=>{workoutSetDetailsOpen=event.detail?.open===true;renderWorkoutSetDetails();update1RM();});workoutSetBridgeBound=true;}
  ...
  document.querySelectorAll('[data-exercise-pick]').forEach(b=>b.onclick=()=>{const picked=b.dataset.exercisePick||'';if(Number.isInteger(workoutReplaceIndex)&&workoutReplaceIndex>=0&&workoutReplaceIndex<workoutDraft.length){const current=workoutDraft[workoutReplaceIndex],ref=exerciseFor(picked),met=num(ref?.met_default,current.met||5);workoutDraft[workoutReplaceIndex]={...current,name:picked,primaryMuscle:ref?.primary_muscle||current.primaryMuscle||'전신/컨디셔닝',met,kcal:calcWorkoutKcal({duration:current.duration,body:current.body,rpe:current.rpe,met})};workoutSelectedExercise=picked;workoutReplaceIndex=null;toast('운동을 교체했습니다.');render();requestAnimationFrame(()=>document.querySelector('[data-gws-step="log"]')?.click());return;}workoutSelectedExercise=picked;const m=exerciseMuscle(workoutSelectedExercise);workoutMuscleFilter=m.key;render();requestAnimationFrame(()=>document.querySelector('[data-gws-step="log"]')?.click());});
  ...
  $('addWorkout').onclick=()=>{const x=buildWorkout();if(x){const addNode=$('addWorkout'),imported=addNode?.dataset.executionImport==='true',editIndex=workoutEditTargetId?workoutDraft.findIndex(item=>String(item.id)===String(workoutEditTargetId)):-1;if(editIndex>=0){workoutDraft.splice(editIndex,1,x);workoutEditTargetId=null;workoutEditForm=null;}else if(x.groupType!=='none'&&x.groupId){const groupKey=String(x.groupType)+':'+String(x.groupId).toLowerCase(),last=workoutDraft.map((item,index)=>({item,index})).filter(({item})=>String(item.groupType)+':'+String(item.groupId||'').toLowerCase()===groupKey).at(-1);if(last)workoutDraft.splice(last.index+1,0,x);else workoutDraft.push(x);}else workoutDraft.push(x);window.dispatchEvent(new CustomEvent('garang:workout-exercise-added',{detail:{imported,groupType:x.groupType,groupId:x.groupId}}));workoutSetDraft=[];workoutSetDetailsOpen=false;workoutSelectedExercise=x.name;toast(x.name+' 추가');render();}};
  $('clearWorkoutDraft').onclick=()=>{window.dispatchEvent(new CustomEvent('garang:workout-session-clearing'));workoutDraft=[];workoutSetDraft=[];workoutSetDetailsOpen=false;workoutEditTargetId=null;workoutReplaceIndex=null;workoutActivePlanId=null;workoutExecutionDraftId=null;render();};
  renderWorkoutSetDetails();const barPreset=$('wBarPreset');if(barPreset)barPreset.onchange=()=>{const v=barPreset.value;let metric=weightUnit()==='lb'?45/2.2046226218:20;if(v==='20kg')metric=20;else if(v==='15kg')metric=15;else if(v==='45lb')metric=45/2.2046226218;$('wBarWeight').value=String(shownWeight(metric,1));};const plateButton=$('calcWorkoutPlates');if(plateButton)plateButton.onclick=()=>{const rawTarget=Math.max(0,num($('wPlateTarget')?.value)),rounding=Math.max(0,num($('wPlateRounding')?.value)),target=rounding?Math.round(rawTarget/rounding)*rounding:rawTarget,bar=Math.max(0,num($('wBarWeight')?.value)),side=Math.max(0,(target-bar)/2),imperial=weightUnit()==='lb',profile=String($('wPlateProfile')?.value||'full'),plates=imperial?(profile==='basic'?[45,25,10,5,2.5]:[45,35,25,10,5,2.5]):(profile==='basic'?[20,10,5,2.5,1.25]:[20,15,10,5,2.5,1.25,1,.5]),parts=[];let remaining=side;for(const plate of plates){const count=Math.floor((remaining+1e-6)/plate);if(count){parts.push(count+'×'+plate+weightUnit());remaining-=count*plate;}}const loaded=bar+2*(side-remaining);$('workoutPlateResult').textContent=target<bar?'목표가 바 중량보다 낮습니다.':(parts.length?('한쪽 · '+parts.join(' + ')+' · 실제 '+Number(loaded.toFixed(2))+weightUnit()+(remaining>.05?' · 조정 필요 '+Number((remaining*2).toFixed(2))+weightUnit():'')):'추가 플레이트 없음');};const warmupButton=$('generateWorkoutWarmup');if(warmupButton)warmupButton.onclick=workoutWarmupPlan;$('saveWorkoutSession').onclick=saveWorkoutSession;
  ...
  $('workoutOverlayOnly')?.addEventListener('click',()=>saveTransparentOverlay('workout'));$('scheduleWorkoutProgram')?.addEventListener('click',()=>{if(!workoutDraft.length)return toast('먼저 프로그램에 사용할 운동 세션을 구성해 주세요.');const weeks=clamp(Math.round(num($('wProgramWeeks')?.value,4)),1,8),frequency=clamp(Math.round(num($('wProgramFrequency')?.value,state.onboarding?.weeklyFrequency||3)),1,7),name=String($('wProgramName')?.value||'Workout Program').trim()||'Workout Program',template=clone(workoutDraft),start=new Date(today()+'T12:00:00'),step=Math.max(1,Math.round(7/frequency));for(let week=0;week<weeks;week++){for(let day=0;day<frequency;day++){const d=new Date(start);d.setDate(start.getDate()+week*7+day*step);const date=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;state.planner.push({id:uid(),date,time:'18:30',type:'workout',title:name,source:'workout_program',origin:'workout_program',status:'confirmed',completed:false,done:false,programName:name,programWeek:week+1,programDay:day+1,templateExercises:template,createdAt:isoNow(),updatedAt:isoNow()});}}saveState({event:'workout_program_scheduled',source:'workout'});toast(`${weeks}주 프로그램을 Planner에 예약했습니다.`);render();});$('importHealthSignals')?.addEventListener('click',()=>$('healthSignalImport')?.click());$('healthSignalImport')?.addEventListener('change',async event=>{const file=event.target?.files?.[0];event.target.value='';if(!file)return;const api=window.GarangHealthSignalImportV1;if(!api)return toast('Health 가져오기 모듈을 불러오지 못했습니다.');try{const text=await file.text(),result=api.parse(text,{filename:file.name,source:file.name});if(result.status!=='ready'||!result.signals.length)return toast('가져올 수 있는 HRV·안정시 심박·수면·활동 신호를 찾지 못했습니다.');const before=state.physiologicalSignals.length;state.physiologicalSignals=api.merge(state.physiologicalSignals,result,{maxSignals:5000});const added=Math.max(0,state.physiologicalSignals.length-before);saveState({event:'health_signals_imported',source:'health_import'});trackEvent('health_signals_imported',{format:result.format,accepted:result.summary.accepted,added,sources:result.summary.sources.length});toast(`Health 신호 ${added}개를 가져왔습니다. Coach와 Recovery 해석에 반영됩니다.`);render();}catch(e){captureError('health_signal_import',e);toast('Health 데이터 파일을 읽지 못했습니다. JSON, CSV 또는 압축 해제한 Apple Health export.xml을 확인해 주세요.');}});
$('syncWorkoutHealth')?.addEventListener('click',async()=>{const payload=window.GarangWorkoutExecutionBridge?.healthExport?.();const native=window.GarangNativeHealthBridge;if(native?.requestAuthorization&&native?.syncWorkouts){try{await native.requestAuthorization(['workout','activeEnergy','heartRate']);await native.syncWorkouts(payload?.workouts||[]);toast('Health 동기화를 완료했습니다.');}catch(e){captureError('health_sync',e);toast('Health 동기화를 완료하지 못했습니다.');}return;}try{const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='garang-health-workouts.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('Health 호환 운동 데이터를 내보냈습니다.');}catch(e){captureError('health_export',e);toast('운동 데이터 내보내기에 실패했습니다.');}});
  if(currentCert.workout)showCert('workoutCertArea',currentCert.workout,state.workouts.at(-1),'workout');
  bindPhotoEvidenceHistory();
  document.querySelectorAll('[data-remove-workout]').forEach(b=>b.onclick=()=>{workoutDraft.splice(num(b.dataset.removeWorkout),1);workoutReplaceIndex=null;workoutEditTargetId=null;render();});document.querySelectorAll('[data-move-workout]').forEach(b=>b.onclick=()=>{const from=num(b.dataset.moveWorkout),to=from+num(b.dataset.dir);if(to<0||to>=workoutDraft.length)return;const [item]=workoutDraft.splice(from,1);workoutDraft.splice(to,0,item);workoutReplaceIndex=null;render();});document.querySelectorAll('[data-replace-workout]').forEach(b=>b.onclick=()=>{workoutReplaceIndex=num(b.dataset.replaceWorkout);toast('교체할 운동을 선택하세요.');render();requestAnimationFrame(()=>document.querySelector('[data-gws-step="exercise"]')?.click());});document.querySelectorAll('[data-execute-workout]').forEach(b=>b.onclick=()=>stageWorkoutDraftExecution(num(b.dataset.executeWorkout),true));document.querySelectorAll('[data-edit-workout]').forEach(b=>b.onclick=()=>editWorkout(num(b.dataset.editWorkout)));
}
function editWorkout(i){const x=workoutDraft[i];if(!x)return;workoutEditTargetId=x.id;workoutEditForm={index:i,name:x.name,sets:Math.max(1,num(x.sets,1)),reps:Math.max(1,num(x.reps,1)),weight:shownWeight(x.weight,1),rpe:clamp(num(x.rpe,8),1,10),rir:clamp(num(x.rir,2),0,10),setType:String(x.setType||'working'),notes:String(x.notes||''),groupType:String(x.groupType||'none'),groupId:String(x.groupId||''),duration:Math.max(1,num(x.duration,15)),body:shownWeight(x.body,1)};const sourceDetails=Array.isArray(x.setDetails)&&x.setDetails.length?x.setDetails:Array.from({length:workoutEditForm.sets},(_,index)=>({set:index+1,reps:workoutEditForm.reps,weight:num(x.weight),rpe:workoutEditForm.rpe}));workoutSetDraft=sourceDetails.map(row=>({...row,executionCompleted:true}));workoutSetDetailsOpen=true;workoutSelectedExercise=x.name;toast('수정 후 같은 위치에 저장됩니다.');render();}
...
function supplementalKey(value){let s=String(value||'');try{s=s.normalize('NFKC');}catch{}return s.toLowerCase().replace(/[\s·_\-()[\]{}.,/\\:+]/g,'');}
...
function indexedSupplementalMatch(rows,index,query,options={}){
 ...
 const reportNo=foodIdentityCore()?.normalizeReportNo?.(options.reportNo)||null;
 const exact=index?.exact?.get(key);
 if(exact&&(!reportNo||foodIdentityCore()?.normalizeReportNo?.(exact?.report_no||exact?.reportNo)===reportNo))return {food:exact,match:{status:'matched',confidence:1,reason:reportNo?'REPORT_NO_EXACT':'SUPPLEMENTAL_EXACT'}};
 const candidates=index?.prefix?.get(key.slice(0,4))||[];
 if(reportNo){
  const reportHit=foodIdentityCore()?.findReportNoMatch?.(candidates,reportNo);
...
async function loadSupplementalFoodDb(){
 ...
 if(!supplementalFoodDbPromise)supplementalFoodDbPromise=fetch(SUPPLEMENTAL_FOOD_DB_PATH,{cache:'force-cache'}).then(async response=>{if(!response.ok)throw new Error('SUPPLEMENTAL_FOOD_DB_'+response.status);const payload=await response.json(),rows=Array.isArray(payload)?payload:(Array.isArray(payload?.records)?payload.records:[]);if(rows.length<4000)throw new Error('SUPPLEMENTAL_FOOD_DB_SCALE_INVALID');supplementalFoodDb=rows;supplementalFoodIndex=buildSupplementalIndex(rows);return rows;}).catch(error=>{supplementalFoodDbPromise=null;captureError('supplemental_food_db_load',error);return [];});
 return supplementalFoodDbPromise;
}
const KOREAN_CHOSEONG=['ㄱ','ㄲ','ㄴ','ㄷ','ㄸ','ㄹ','ㅁ','ㅂ','ㅃ','ㅅ','ㅆ','ㅇ','ㅈ','ㅉ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ'];
function koreanFoodBucket(value){const s=supplementalKey(value);if(!s)return '*';const ch=s[0],code=ch.charCodeAt(0);if(code>=0xAC00&&code<=0xD7A3)return KOREAN_CHOSEONG[Math.floor((code-0xAC00)/588)]||'*';if(/[a-z]/i.test(ch))return ch.toUpperCase();if(/[0-9]/.test(ch))return '#';return '*';}
async function loadKoreanFoodManifest(){
 if(koreanFoodManifest)return koreanFoodManifest;
 if(!koreanFoodManifestPromise)koreanFoodManifestPromise=fetch(KOREAN_FOOD_MANIFEST_PATH,{cache:'force-cache'}).then(async response=>{if(response.status===404)return {shards:{},count:0};if(!response.ok)throw new Error('KOREAN_FOOD_MANIFEST_'+response.status);const payload=await response.json();return payload&&typeof payload==='object'?payload:{shards:{},count:0};}).then(payload=>{koreanFoodManifest=payload;return payload;}).catch(error=>{koreanFoodManifestPromise=null;captureError('korean_food_manifest_load',error);return {shards:{},count:0};});
 return koreanFoodManifestPromise;
}
async function loadKoreanProcessedManifest(){
 if(koreanProcessedManifest)return koreanProcessedManifest;
...
async function findSupplementalFood(q,options={}){return await findKoreanSupplementalFood(q,options)||await findUsdaSupplementalFood(q,options);}
...
function mealScanMatch(row){
 const smart=window.GarangFoodIntelligenceV2?.resolveVisionRow?.(db.food,row);let food=smart?.status==='matched'?smart.food:null;
 if(!food){const names=[row?.name,...(Array.isArray(row?.aliases)?row.aliases:[])].map(x=>String(x||'').trim()).filter(Boolean);for(const name of names){food=findFood(name);if(food)break;}}
 if(!food)return null;
 const identityConfidence=clamp(num(row?.confidence,0),0,1),portionConfidence=clamp(num(row?.portionConfidence,row?.confidence??0),0,1),matchConfidence=clamp(num(smart?.confidence,smart?.status==='matched'?1:0),0,1),confirmationRequired=identityConfidence<.65||portionConfidence<.6||matchConfidence<.72;
...
async function lookupMealScanSupplemental(rows=[]){
 ...
 for(const row of input){
  const queries=[row?.name,...(Array.isArray(row?.aliases)?row.aliases:[])].map(x=>String(x||'').trim()).filter(Boolean);let hit=null;
  ...
  const identityConfidence=clamp(num(row?.confidence,0),0,1),portionConfidence=clamp(num(row?.portionConfidence,row?.confidence??0),0,1),matchConfidence=clamp(num(hit?.match?.confidence,0),0,1),combinedConfidence=clamp(matchConfidence*(.65+.35*identityConfidence),0,1);
  if(combinedConfidence<.7){unresolved.push(row);continue;}
  const confirmationRequired=identityConfidence<.65||portionConfidence<.6||combinedConfidence<.72,item=window.GarangFoodIntelligenceV2?.toMealItem?.(hit.food,Math.max(5,num(row?.grams,100)))||foodItemFromFood(hit.food,Math.max(5,num(row?.grams,100)));
  if(!item){unresolved.push(row);continue;}
  items.push({...item,id:uid(),scanEvidence:{visionName:String(row?.name||hit.food.name),confidence:identityConfidence,identityConfidence,portionConfidence,matchConfidence:combinedConfidence,confirmationRequired,matchReason:String(hit?.match?.reason||'SUPPLEMENTAL'),source:'vision+'+(hit.source||'supplemental')}});
 }
 if(items.length)trackEvent('meal_scan_supplemental_resolved',{resolved:items.length,unresolved:unresolved.length,korean:items.filter(x=>x?.scanEvidence?.source==='vision+korea-official').length});
 return {items,unresolved};
}
async function lookupMealScanNutrition(rows=[]){
 const input=(Array.isArray(rows)?rows:[]).slice(0,6);if(!input.length||!SERVICES.nutritionLookupEndpoint)return {items:[],unresolved:input};
 try{
  toast('DB에 없는 음식의 영양정보를 확인 중입니다.');
  const r=await fetch(SERVICES.nutritionLookupEndpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({items:input.map(row=>({name:row.name,aliases:row.aliases||[],grams:row.grams})),language:document.documentElement.lang==='en'?'en':'ko'})});
  const payload=await r.json().catch(()=>({}));if(!r.ok)throw Object.assign(new Error(payload?.error?.code||`Nutrition Lookup ${r.status}`),{code:payload?.error?.code||'NUTRITION_LOOKUP_FAILED'});
  const items=(Array.isArray(payload?.items)?payload.items:[]).map((row,index)=>{
   const source=input[row?.inputIndex??index]||{},identityConfidence=clamp(num(source?.confidence,0),0,1),portionConfidence=clamp(num(source?.portionConfidence,source?.confidence??0),0,1);
   return {id:uid(),foodId:null,name:String(row?.name||source?.name||'음식'),grams:Math.max(5,num(row?.grams,source?.grams||100)),kcal:num(row?.kcal),protein:num(row?.protein),carbs:num(row?.carbs),fat:num(row?.fat),nutritionStatus:'estimated',nutritionSource:normalizedNutritionSource(row?.nutritionSource||{source:'web_search'}),userOverride:false,scanEvidence:{visionName:String(source?.name||row?.name||'음식'),confidence:identityConfidence,identityConfidence,portionConfidence,confirmationRequired:true,source:'vision+web'}};
  }).filter(item=>[item.kcal,item.protein,item.carbs,item.fat].every(Number.isFinite));
  const unresolved=(Array.isArray(payload?.unresolved)?payload.unresolved:[]).map(row=>{const source=input[row?.inputIndex]||{};return {name:String(row?.name||source?.name||'음식'),grams:Math.max(5,num(source?.grams,100)),confidence:clamp(num(source?.confidence,0),0,1),portionConfidence:clamp(num(source?.portionConfidence,source?.confidence??0),0,1)};});
  trackEvent('meal_scan_web_nutrition_resolved',{resolved:items.length,unresolved:unresolved.length});
  return {items,unresolved};
 }catch(error){captureError('meal_scan_nutrition_lookup',error);return {items:[],unresolved:input};}
}
...
async function analyzeMealScan(){
 ...
 if(SERVICES.mealScanEndpoint){
  try{
   toast(mode==='label'?'영양 라벨을 읽는 중입니다.':'사진에서 음식을 확인 중입니다.');
   const image=await prepareMealScanImage(mealScanDraft.file),r=await fetch(SERVICES.mealScanEndpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image,language:document.documentElement.lang==='en'?'en':'ko',mode})}),payload=await r.json().catch(()=>({}));
   if(!r.ok)throw Object.assign(new Error(payload?.error?.code||`Meal Scan ${r.status}`),{code:payload?.error?.code||'MEAL_SCAN_FAILED'});
   if(mode==='label'){
   ...
   const rows=Array.isArray(payload?.data?.items)?payload.data.items:(Array.isArray(payload?.items)?payload.items:[]);if(!rows.length)throw new Error('MEAL_SCAN_NO_ITEMS');
   const matched=[],needsLookup=[];
   for(const row of rows){const item=mealScanMatch(row);if(item)matched.push(item);else needsLookup.push({name:String(row?.name||'음식'),aliases:Array.isArray(row?.aliases)?row.aliases:[],grams:Math.max(5,num(row?.grams,100)),confidence:clamp(num(row?.confidence,0),0,1),portionConfidence:clamp(num(row?.portionConfidence,row?.confidence??0),0,1)});}
   const supplemental=await lookupMealScanSupplemental(needsLookup),web=await lookupMealScanNutrition(supplemental.unresolved);
   mealScanDraft.items=[...matched,...supplemental.items,...web.items];mealScanDraft.unmatched=web.unresolved;
   const resolvedItems=[...matched,...supplemental.items,...web.items],lowConfidenceCount=resolvedItems.filter(item=>item?.scanEvidence?.confirmationRequired===true).length;mealScanDraft.scanMeta={mode:'meal',overallConfidence:num(payload?.data?.overallConfidence,0),uncertain:payload?.data?.uncertain===true||lowConfidenceCount>0,requestId:payload?.data?.requestId||null,dbMatched:matched.length,supplementalMatched:supplemental.items.length,webResolved:web.items.length,lowConfidenceCount};
   trackEvent('meal_scan_draft_created',{provider:'vision',matched:matched.length,supplementalMatched:supplemental.items.length,webResolved:web.items.length,unmatched:web.unresolved.length});
   render();
   if(!mealScanDraft.items.length)return toast('음식은 인식했지만 신뢰할 수 있는 영양 출처를 찾지 못했습니다.');
   if(web.unresolved.length)toast(`${mealScanDraft.items.length}개 계산 · ${web.unresolved.length}개는 직접 확인이 필요합니다.`);
   else toast(mealScanDraft.scanMeta?.lowConfidenceCount?`${mealScanDraft.items.length}개 계산 · 음식/양을 한 번 더 확인해 주세요.`:(web.items.length?'GARANG DB와 공식/웹 출처를 확인해 식사 초안을 만들었습니다.':(supplemental.items.length?'GARANG DB와 공식 확장 DB로 식사 초안을 만들었습니다.':'사진 분석 초안을 만들었습니다.')));
   return;
  }catch(e){
 ...
 if(mode==='label')return toast('Vision API 미연결 상태에서는 영양 라벨을 읽을 수 없습니다.');
 if(!mealScanDraft.manualName)return toast('Vision API 미연결 상태입니다. 음식명을 입력하면 DB 기반 초안을 만들 수 있습니다.');
 const x=foodItem(mealScanDraft.manualName,mealScanDraft.grams);if(!x)return toast('Food DB에서 해당 음식을 찾지 못했습니다.');
 mealScanDraft.items=[x];mealScanDraft.unmatched=[];trackEvent('meal_scan_draft_created',{provider:'local_db'});render();
}
...
function activeRunElapsedMs(){
  ...
  return Math.max(0,num(runState.elapsedMs)+(runState.paused?0:Math.max(0,Date.now()-num(runState.activeStartedAt,Date.now()))));
}
function clearRunWatch(){
  if(runState?.watchId!=null&&navigator.geolocation?.clearWatch)navigator.geolocation.clearWatch(runState.watchId);
  if(runState)runState.watchId=null;
  clearInterval(runTimer);runTimer=null;
}
function beginRunWatch(){
  if(!runState||runState.paused||!navigator.geolocation)return;
  clearRunWatch();
...
function updateRoute(){
  ...
  if(pts.length<2){line.setAttribute('points','50,50');return;}
...
function bindPlanner(){$('addPlan').onclick=()=>{const title=$('planTitle').value.trim();if(!title)return toast('계획 내용을 입력해 주세요.');state.planner.push({id:uid(),date:$('planDate').value||today(),time:$('planTime').value||'',type:$('planType').value,title,source:'user',status:'confirmed',completed:false,goalClass:currentGoalClass(),goalLabel:currentGoalLabel(),createdAt:isoNow(),updatedAt:isoNow()});saveState({event:'planner_created',source:'planner'});toast('계획을 저장했습니다.');render();};document.querySelectorAll('[data-plan-start]').forEach(b=>b.onclick=()=>{const p=state.planner.find(x=>String(x.id)===String(b.dataset.planStart));if(!p||!Array.isArray(p.templateExercises)||!p.templateExercises.length)return toast('실행할 운동 템플릿이 없습니다.');if(workoutDraft.length)return toast('진행 중인 운동 초안을 먼저 저장하거나 초기화해 주세요.');workoutDraft=clone(p.templateExercises).map(x=>({...x,id:uid()}));workoutSetDraft=[];workoutSetDetailsOpen=false;workoutEditTargetId=null;workoutReplaceIndex=null;workoutActivePlanId=p.id;workoutExecutionDraftId=workoutDraft[0]?.id||null;if(workoutDraft.length)stageWorkoutDraftExecution(0,false);else workoutSelectedExercise=workoutDraft[0]?.name||workoutSelectedExercise;toast('프로그램 세션을 Workout Log에 불러왔습니다.');window.GarangRouter?.navigate?.('workout',{source:'workout-program',force:true})||(()=>{currentPage='workout';render();})();});document.querySelectorAll('[data-plan-toggle]').forEach(b=>b.onclick=()=>{const p=state.planner.find(x=>x.id===b.dataset.planToggle);if(p){p.completed=!p.completed;p.updatedAt=isoNow();saveState({event:'planner_completed',source:'planner'});render();}});document.querySelectorAll('[data-plan-delete]').forEach(b=>b.onclick=()=>{if(!confirm('이 계획을 삭제할까요?'))return;state.planner=state.planner.filter(x=>x.id!==b.dataset.planDelete);saveState({event:'planner_deleted'});render();});}
...
function bindProfile(){$('saveProfile').onclick=()=>{state.profile={...(state.profile||{}),name:$('pName').value.trim()||'GARANG 사용자',age:num($('pAge').value),gender:$('pGender').value==='female'?'female':'male',height:metricLength($('pHeight').value),weight:metricWeight($('pWeight').value),goal:$('pGoal').value.trim()||'퍼포먼스 향상'};state.onboarding.goal=state.profile.goal;saveState({event:'profile_updated'});toast('프로필을 저장했습니다.');render();};}
function bindSettings(){$('savePreferences').onclick=()=>{state.preferences={...(state.preferences||{}),language:$('languageSetting').value==='en'?'en':'ko',unit:$('unitSetting').value==='imperial'?'imperial':'metric'};state.privacy={...(state.privacy||{}),consent:{...(state.privacy?.consent||{}),analytics:$('analyticsConsentSetting')?.checked===true}};saveState({event:'settings_updated'});applyLanguageChrome();toast(state.preferences.language==='en'?'Settings saved.':'설정을 저장했습니다.');render();};$('retrySync').onclick=()=>{if(currentUser&&firebaseReady)cloudSaveNow().then(ok=>toast(ok?'동기화가 완료되었습니다.':'동기화 설정을 확인해 주세요.'));else toast('로그인 상태에서 클라우드 동기화를 사용할 수 있습니다.');};$('exportData').onclick=exportData;$('importLegacy').onclick=importLegacy;$('settingsLogout').onclick=logout;$('proInfo').onclick=()=>toast(state.plan==='PRO'?'GARANG PRO가 활성화되어 있습니다.':'실제 결제 연동 후 PRO 업그레이드를 사용할 수 있습니다.');}
...
function latestWorkoutSessionSummary(){const last=state.workouts.at(-1);if(!last)return null;const items=last.sessionId?state.workouts.filter(x=>x.sessionId===last.sessionId):[last],top=items.slice().sort((a,b)=>workoutRecordEstimated1RM(b)-workoutRecordEstimated1RM(a))[0];return {kind:'workout',date:last.date||today(),name:items.length>1?`${muscleLabelFromKey(exerciseMuscle(top?.name).key)} SESSION`:last.name,duration:sum(items,x=>num(x.duration)),volume:sum(items,workoutRecordVolume),kcal:sum(items,x=>num(x.kcal)),topLift:top?`${top.name} · ${shownWeight(workoutRecordMaxWeight(top),1)}${weightUnit()} × ${top.reps}`:'—',sets:sum(items,x=>workoutDetailRows(x).length||num(x.sets)),reps:sum(items,x=>workoutDetailRows(x).length?workoutDetailRows(x).reduce((total,row)=>total+num(row?.reps??row?.r),0):num(x.sets)*num(x.reps))};}
function certInfo(record,kind){
  if(kind==='running'){const r=record||{};return {title:'RUN VERIFIED',headline:`${Number(shownDistance(num(r.distance),2)).toFixed(2)} ${distanceUnit().toUpperCase()}`,sub:`${formatRunMinutes(num(r.duration))} · ${paceText(r)} /${distanceUnit().toUpperCase()}`,date:r.date||today(),metrics:[['TIME',formatRunMinutes(num(r.duration))],['CALORIES',`${Math.round(num(r.kcal))} kcal`],['PACE',`${paceText(r)} /${distanceUnit().toUpperCase()}`]],coords:Array.isArray(r.coords)?r.coords:[]};}
  const s=latestWorkoutSessionSummary()||record||{}, muscle=exerciseMuscle((state.workouts.at(-1)||{}).name||'');return {title:'WORKOUT VERIFIED',headline:s.name||'WORKOUT',sub:s.topLift||'',date:s.date||today(),metrics:[['DURATION',`${Math.round(num(s.duration))} min`],['VOLUME',`${Math.round(shownWeight(num(s.volume),0)).toLocaleString()} ${weightUnit()}`],['CALORIES',`${Math.round(num(s.kcal))} kcal`]],muscleKey:muscle.key,muscleLabel:muscle.label};
}
...
function paceText(r){const p=num(r?.duration)/(num(r?.distance)||1);if(!Number.isFinite(p)||!num(r?.distance))return '—';const shown=shownPace(p,3),m=Math.floor(shown),s0=Math.round((shown-m)*60),s=s0===60?0:s0,mm=s0===60?m+1:m;return `${mm}'${String(s).padStart(2,'0')}\"`;}
function routeSignatureSvg(coords=[]){let pts='20,100 35,82 28,67 52,48 44,31 72,23 88,8';if(coords.length>1){const lats=coords.map(p=>p[0]),lons=coords.map(p=>p[1]),minLat=Math.min(...lats),maxLat=Math.max(...lats),minLon=Math.min(...lons),maxLon=Math.max(...lons),dLat=maxLat-minLat||1e-6,dLon=maxLon-minLon||1e-6;pts=coords.slice(-200).map(p=>`${12+76*(p[1]-minLon)/dLon},${104-96*(p[0]-minLat)/dLat}`).join(' ');}return `<svg class="cert-route-signature" viewBox="0 0 100 112" aria-hidden="true"><polyline points="${pts}"/></svg>`;}
function showCert(id,m,record,kind){const area=$(id);if(!area)return;const isVideo=m.type.startsWith('video'),info=certInfo(record,kind);const visual=kind==='running'?routeSignatureSvg(info.coords):`<div class="cert-anatomy" data-muscle="${info.muscleKey||'full'}">${miniBodySvg(info.muscleKey||'full')}</div>`;area.className='cert premium-cert';area.innerHTML=`<div class="cert-media"><${isVideo?'video':'img'} src="${m.url}" ${isVideo?'controls muted playsinline':''} alt="GARANG 인증 미디어"><div class="cert-overlay-signature"><div class="cert-topline"><div class="cert-wordmark">GARANG<small>Know · Understand · Guide · Act</small></div><div class="cert-verified">${info.title}</div></div><div class="cert-vertical-mark"><i></i><b>가<br>랑</b><i></i></div><div class="cert-lower"><div class="cert-main-copy"><strong>${esc(info.headline)}</strong><span>${kind==='running'?'RUNNING SESSION':esc(info.muscleLabel||'PERFORMANCE SESSION')}</span></div>${visual}<div class="cert-metric-line">${info.metrics.map(x=>`<span><i>${x[0]}</i><b>${esc(x[1])}</b></span>`).join('')}</div>${info.sub?`<div class="cert-toplift"><span>${kind==='running'?'SESSION':'TOP LIFT'}</span><b>${esc(info.sub)}</b></div>`:''}<div class="cert-foot"><span>${esc(info.date)} · GARANG</span><em>Garang</em></div></div></div></div><div class="cert-controls">${isVideo?'<button class="ghost small" id="certShare">원본 영상 공유</button><div class="cert-video-note">영상 합성은 서버/네이티브 인코더 연결 전까지 원본을 변조하지 않습니다.</div>':'<button class="primary small" id="certShare">프리미엄 인증 이미지 저장 / 공유</button>'}</div>`;$('certShare').onclick=()=>isVideo?shareMedia(m):shareCompositeImage(m,record,kind);}
...
function drawRouteCanvas(c,coords,x,y,w,h){let pts=[];if(coords?.length>1){const lats=coords.map(p=>p[0]),lons=coords.map(p=>p[1]),minLat=Math.min(...lats),maxLat=Math.max(...lats),minLon=Math.min(...lons),maxLon=Math.max(...lons),dLat=maxLat-minLat||1e-6,dLon=maxLon-minLon||1e-6;pts=coords.slice(-220).map(p=>[x+w*(p[1]-minLon)/dLon,y+h-h*(p[0]-minLat)/dLat]);}else pts=[[x,y+h],[x+w*.22,y+h*.72],[x+w*.14,y+h*.52],[x+w*.52,y+h*.38],[x+w*.44,y+h*.18],[x+w*.82,y]];c.strokeStyle='rgba(247,244,238,.96)';c.lineWidth=Math.max(3,w*.025);c.lineCap='round';c.lineJoin='round';c.beginPath();pts.forEach((p,i)=>i?c.lineTo(...p):c.moveTo(...p));c.stroke();if(pts.length){c.fillStyle='#63b34f';c.beginPath();c.arc(pts[0][0],pts[0][1],Math.max(5,w*.05),0,Math.PI*2);c.fill();c.fillStyle='#d85448';c.beginPath();c.arc(pts.at(-1)[0],pts.at(-1)[1],Math.max(5,w*.05),0,Math.PI*2);c.fill();}}
function drawAnatomyBadgeCanvas(c,key,x,y,w,h){c.save();c.translate(x,y);c.strokeStyle='rgba(242,238,231,.75)';c.fillStyle='rgba(170,164,155,.72)';c.lineWidth=Math.max(2,w*.018);c.beginPath();c.arc(w*.5,h*.10,w*.09,0,Math.PI*2);c.fill();c.stroke();c.beginPath();c.moveTo(w*.34,h*.19);c.quadraticCurveTo(w*.5,h*.13,w*.66,h*.19);c.lineTo(w*.70,h*.58);c.lineTo(w*.61,h*.94);c.lineTo(w*.52,h*.94);c.lineTo(w*.50,h*.66);c.lineTo(w*.48,h*.94);c.lineTo(w*.39,h*.94);c.lineTo(w*.30,h*.58);c.closePath();c.fill();c.stroke();c.fillStyle='rgba(205,83,66,.92)';const zones={chest:[.5,.31,.17,.10],back:[.5,.31,.19,.17],shoulders:[.5,.23,.26,.07],core:[.5,.43,.10,.18],legs:[.5,.72,.22,.24],biceps:[.28,.36,.06,.16],triceps:[.72,.36,.06,.16],full:[.5,.46,.24,.40]};const z=zones[key]||zones.full;c.beginPath();c.ellipse(w*z[0],h*z[1],w*z[2],h*z[3],0,0,Math.PI*2);c.fill();c.restore();}
async function composeCertImage(m,record,kind){
  const im=await loadImageFromUrl(m.url),max=1800,scale=Math.min(1,max/Math.max(im.naturalWidth,im.naturalHeight)),w=Math.max(720,Math.round(im.naturalWidth*scale)),h=Math.max(960,Math.round(im.naturalHeight*scale)),canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const c=canvas.getContext('2d'),ratio=Math.max(w/im.naturalWidth,h/im.naturalHeight),dw=im.naturalWidth*ratio,dh=im.naturalHeight*ratio;c.drawImage(im,(w-dw)/2,(h-dh)/2,dw,dh);const info=certInfo(record,kind),pad=Math.round(w*.055);
  let top=c.createLinearGradient(0,0,0,h*.23);top.addColorStop(0,'rgba(6,6,6,.62)');top.addColorStop(1,'rgba(6,6,6,0)');c.fillStyle=top;c.fillRect(0,0,w,h*.28);let bottom=c.createLinearGradient(0,h*.46,0,h);bottom.addColorStop(0,'rgba(6,6,6,0)');bottom.addColorStop(.38,'rgba(6,6,6,.30)');bottom.addColorStop(1,'rgba(6,6,6,.78)');c.fillStyle=bottom;c.fillRect(0,h*.42,w,h*.58);
  c.strokeStyle='rgba(244,240,233,.66)';c.lineWidth=Math.max(2,w*.0022);drawRoundRect(c,pad*.55,pad*.55,w-pad*1.1,h-pad*1.1,Math.round(w*.028));c.stroke();
  c.fillStyle='#f4f0e9';c.font=`500 ${Math.round(w*.04)}px Georgia,serif`;c.fillText('GARANG',pad,pad*1.5);c.fillStyle='rgba(244,240,233,.72)';c.font=`500 ${Math.round(w*.014)}px system-ui`;c.fillText('Know · Understand · Guide · Act',pad,pad*1.92);c.textAlign='right';c.fillStyle='#f4f0e9';c.font=`700 ${Math.round(w*.019)}px system-ui`;c.fillText(info.title,w-pad,pad*1.48);c.textAlign='left';
  c.strokeStyle='rgba(244,240,233,.48)';c.lineWidth=1;c.beginPath();c.moveTo(pad,h*.22);c.lineTo(pad,h*.28);c.stroke();c.fillStyle='rgba(244,240,233,.88)';c.font=`600 ${Math.round(w*.032)}px Georgia,serif`;c.fillText('가',pad,h*.31);c.fillText('랑',pad,h*.355);
  const baseY=h-pad*5.15;c.fillStyle='#f4f0e9';c.font=`600 ${Math.round(w*(kind==='running'?.09:.066))}px Georgia,system-ui`;c.fillText(info.headline,pad,baseY);c.fillStyle='rgba(244,240,233,.76)';c.font=`600 ${Math.round(w*.018)}px system-ui`;c.fillText(kind==='running'?'RUNNING SESSION':String(info.muscleLabel||'PERFORMANCE SESSION').toUpperCase(),pad,baseY+Math.round(w*.045));
  if(kind==='running')drawRouteCanvas(c,info.coords,w-pad-w*.20,baseY-w*.28,w*.16,w*.28);else drawAnatomyBadgeCanvas(c,info.muscleKey||'full',w-pad-w*.18,baseY-w*.31,w*.15,w*.30);
  const metricY=baseY+Math.round(w*.085),cell=(w-pad*2)/3;c.strokeStyle='rgba(244,240,233,.28)';c.beginPath();c.moveTo(pad,metricY);c.lineTo(w-pad,metricY);c.stroke();info.metrics.forEach((mt,i)=>{const x=pad+i*cell;c.fillStyle='rgba(244,240,233,.60)';c.font=`700 ${Math.round(w*.013)}px system-ui`;c.fillText(mt[0],x,metricY+Math.round(w*.035));c.fillStyle='#f4f0e9';c.font=`650 ${Math.round(w*.026)}px system-ui`;c.fillText(mt[1],x,metricY+Math.round(w*.073));});
  if(info.sub){const y=metricY+Math.round(w*.118);c.strokeStyle='rgba(244,240,233,.28)';c.beginPath();c.moveTo(pad,y);c.lineTo(w-pad,y);c.stroke();c.fillStyle='rgba(244,240,233,.58)';c.font=`700 ${Math.round(w*.013)}px system-ui`;c.fillText(kind==='running'?'SESSION':'TOP LIFT',pad,y+Math.round(w*.035));c.fillStyle='#f4f0e9';c.font=`600 ${Math.round(w*.022)}px system-ui`;c.fillText(info.sub,pad,y+Math.round(w*.073));}
  c.fillStyle='rgba(244,240,233,.76)';c.font=`500 ${Math.round(w*.015)}px system-ui`;c.fillText(`${info.date} · GARANG`,pad,h-pad*1.1);c.textAlign='right';c.font=`italic ${Math.round(w*.032)}px Georgia,serif`;c.fillText('Garang',w-pad,h-pad*1.04);c.textAlign='left';return canvas;
}
