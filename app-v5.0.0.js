...
const EMPTY_DB={profile:{},workouts:[],meals:[],body:[],chat:[],api:{},coachMemory:{facts:[],topics:[],lastAdvice:"",updatedAt:""}};
...
function normalizeDB(x){
 ...
 if(!out.foodUnitSettings||typeof out.foodUnitSettings!=="object")out.foodUnitSettings={};
  if(!out.coachMemory||typeof out.coachMemory!=="object")out.coachMemory={facts:[],topics:[],lastAdvice:"",updatedAt:""};
 return out;
}
function saveLocal(){
 try{
...
function localAnswer(t){
 if(window.FitMindV5?.answer){
  const r=window.FitMindV5.answer(t,{db,foodDB,exerciseDB,aiKnowledge});
  if(r?.topic)window.FitMindV5.remember(db,t,r.topic);
  if(r?.text)return r.text;
 }
 return "로컬 코치 데이터를 불러오는 중이야. 잠시 후 다시 질문해줘.";
}
