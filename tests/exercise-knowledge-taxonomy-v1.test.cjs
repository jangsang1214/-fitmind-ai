...
const allowedEquipment=new Set(['smith_machine','barbell','ez_bar','dumbbell','kettlebell','cable','machine','resistance_band','suspension_trainer','rings','landmine','weight_plate','stability_ball','battle_rope','sled','sandbag','medicine_ball','climbing_rope','free_weight_carry','external_load','ab_wheel','bodyweight','cable_machine','variable']);
const allowedMovement=new Set(['horizontal_push','vertical_push','vertical_pull','horizontal_pull','shoulder_isolation','elbow_flexion','elbow_extension','hip_hinge','squat_lunge','knee_extension','knee_flexion','hip_abduction','hip_adduction','plantar_flexion','core_flexion','core_stability','core_rotation','core_anti_extension','power_complex','loaded_carry','conditioning','general_resistance','diagonal_push','scapular_elevation']);
for(const row of db){
...
assert.deepEqual([byName('랫풀다운').equipment,byName('랫풀다운').movement_type,byName('랫풀다운').is_compound],['cable_machine','vertical_pull',true]);
...
assert.deepEqual([byName('백스쿼트').equipment,byName('백스쿼트').movement_type,byName('백스쿼트').is_compound],['barbell','squat_lunge',true]);
