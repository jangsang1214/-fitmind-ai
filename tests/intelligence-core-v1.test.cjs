...
function fixture(){
 ...
 const dailyCheckins=Array.from({length:5},(_,i)=>({id:`c${i}`,date:date(-i),sleepHours:8,energy:8,stress:2,soreness:{general:2},painCaution:false,availableMinutes:60}));
...
test('planner pain caution blocks training intensity and requires confirmation',()=>{
 const s=fixture();s.dailyCheckins[0].painCaution=true;
 const userState=State.estimateState(s,{now}),decision=Decision.decide(userState,{memoryContext:Memory.prepareMemoryContext(s.memory,s,{now})}),score=Score.compute(s,{now,userState}),x=Planner.adaptWeek(s,{now,userState,decision,score,memoryContext:Memory.prepareMemoryContext(s.memory,s,{now})});
