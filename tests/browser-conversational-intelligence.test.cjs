...
function demoState(){return {meta:{schemaVersion:5,updatedAt:new Date().toISOString()},profile:{name:'Conversation User',age:28,height:174,weight:70,goal:'근육 증가'},onboarding:{complete:true,skipped:false,goal:'근육 증가',experience:'intermediate',weeklyFrequency:4,availableMinutes:60,preferences:''},preferences:{language:'ko',unit:'metric'},checkins:[],dailyCheckins:[],planner:[],workouts:[],meals:[],runs:[],body:[],aiChat:[],actionLog:[],errors:[],analytics:{events:[]},memory:{entries:[],facts:[],preferences:[],goals:[],events:[]},plan:'FREE'};}
...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>window.GarangConversationalIntelligenceV1?.version==='garang-conversational-intelligence-v1.0.0',null,{timeout:7000});
  ...
  await context.close();console.log('browser-conversational-intelligence follow-up -> canonical auto-log -> undo: PASS');
