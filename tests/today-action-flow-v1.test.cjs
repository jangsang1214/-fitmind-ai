...
}
{
 ...
 const m=model(state);assert.equal(m.planned,2);assert.equal(m.executed,1);assert.equal(m.remaining,1);assert.equal(m.route,'planner');assert.match(m.support,/상체 50분/);assert.equal(m.progress,50);
}
{
...
{
 const state=base();state.planner=[{id:'p1',date,type:'nutrition',title:'식단',completed:true}];state.meals=[{id:'m1',date,kcal:2200,protein:120}];state.checkins=[{id:'c1',date,sleep:7.5,energy:4}];
 const m=model(state),html=Flow.markup(m,false);assert.equal(m.route,'progress');assert.match(m.headline,/흐름/);assert.match(html,/data-gtf-details/);assert.match(html,/data-gtf-detail hidden/);assert.match(html,/GARANG/);assert.doesNotMatch(html,/confidence|신뢰도/i,'technical confidence must stay out of Today default/detail layer');
}
{
...
console.log('today-action-flow-v1: PASS');
