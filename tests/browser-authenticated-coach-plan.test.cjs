...
const watchdog=setTimeout(()=>{console.error('browser-authenticated-coach-plan: WATCHDOG TIMEOUT');process.exit(1);},65000);
...
async function coachState(page){return page.evaluate(()=>{const root=document.querySelector('.garang-coach-v2'),active=document.querySelector('#bottomNav button.active')?.dataset.page||null,r=root?.getBoundingClientRect();return {active,coach:!!root&&!!r&&r.width>0&&r.height>0,screen:document.getElementById('main')?.dataset.garangScreen||'',mainText:(document.getElementById('main')?.innerText||'').slice(0,120),prompts:document.querySelectorAll('[data-garang-prompt-id]').length,proposal:!!document.querySelector('.g4-agent-proposal'),threadKeys:Object.keys(localStorage).filter(k=>k.startsWith('garang_coach_threads_v2::')).sort()};});}
...
(async()=>{
 ...
 try{
  ...
  const after=await coachState(page);assert.equal(after.active,'coach',`approved plan must not leave Coach: ${JSON.stringify(after)}`);assert.equal(after.coach,true);assert.ok(after.mainText.includes('오늘 계획을 만들어줘')||after.proposal,`approval must not reset Coach thread: ${JSON.stringify(after)}`);
