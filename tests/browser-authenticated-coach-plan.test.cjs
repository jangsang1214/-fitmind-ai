...
async function coachState(page){return page.evaluate(()=>{const root=document.querySelector('.garang-coach-v2'),active=document.querySelector('#bottomNav button.active')?.dataset.page||null,r=root?.getBoundingClientRect();const planMessage=[...document.querySelectorAll('.g2-message.user .g2-message-text')].some(el=>String(el.textContent||'').includes('오늘 계획을 만들어줘'));return {active,coach:!!root&&!!r&&r.width>0&&r.height>0,sameRoot:!window.__coachRootRef||root===window.__coachRootRef,rootConnected:!window.__coachRootRef||window.__coachRootRef.isConnected,screen:document.getElementById('main')?.dataset.garangScreen||'',prompts:document.querySelectorAll('[data-garang-prompt-id]').length,proposal:!!document.querySelector('.g4-agent-proposal'),planMessage,threadKeys:Object.keys(localStorage).filter(k=>k.startsWith('garang_coach_threads_v2::')).sort()};});}
...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{window.__coachRootRef=document.querySelector('.garang-coach-v2');});
  let before=await coachState(page);assert.equal(before.active,'coach',`Coach must own route before plan tap: ${JSON.stringify(before)}`);assert.equal(before.coach,true);assert.equal(before.sameRoot,true);assert.equal(before.rootConnected,true);
  ...
  for(let i=0;i<12;i++){await sleep(250);await heartbeat(page,`plan settle ${i}`);const state=await coachState(page);assert.equal(state.active,'coach',`plan prompt must not leave Coach at sample ${i}: ${JSON.stringify(state)}`);assert.equal(state.coach,true,`Coach root disappeared at sample ${i}: ${JSON.stringify(state)}`);assert.equal(state.rootConnected,true,`hydration detached the active Coach root at sample ${i}: ${JSON.stringify(state)}`);assert.equal(state.sameRoot,true,`hydration rebuilt the active Coach root at sample ${i}: ${JSON.stringify(state)}`);assert.ok(state.planMessage||state.proposal,`cloud hydration must not reset the active Coach conversation at sample ${i}: ${JSON.stringify(state)}`);}
  await page.waitForFunction(()=>document.querySelector('.g4-agent-proposal'),null,{timeout:7000});
  let proposed=await coachState(page);assert.equal(proposed.active,'coach',`plan proposal must remain in Coach: ${JSON.stringify(proposed)}`);assert.equal(proposed.proposal,true);assert.equal(proposed.sameRoot,true,'proposal creation must preserve the active Coach root');
  await tap(page,'.g4-agent-proposal [data-g4-approve]','approve plan');
  await sleep(700);await heartbeat(page,'approved plan settles');
  const after=await coachState(page);assert.equal(after.active,'coach',`approved plan must not leave Coach: ${JSON.stringify(after)}`);assert.equal(after.coach,true);assert.equal(after.sameRoot,true,'approval/sync must preserve the active Coach root');assert.ok(after.planMessage,`approval must not reset Coach thread: ${JSON.stringify(after)}`);
