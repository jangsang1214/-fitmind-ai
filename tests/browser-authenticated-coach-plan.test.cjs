...
const watchdog=setTimeout(()=>{console.error('browser-authenticated-coach-plan: WATCHDOG TIMEOUT');process.exit(1);},60000);
...
async function coachState(page){return page.evaluate(()=>{const root=document.querySelector('.garang-coach-v2'),active=document.querySelector('#bottomNav button.active')?.dataset.page||null,r=root?.getBoundingClientRect();return {active,coach:!!root&&!!r&&r.width>0&&r.height>0,screen:document.getElementById('main')?.dataset.garangScreen||'',mainText:(document.getElementById('main')?.innerText||'').slice(0,120),prompts:document.querySelectorAll('[data-garang-prompt-id]').length,proposal:!!document.querySelector('.g4-agent-proposal')};});}
...
(async()=>{
 ...
 try{
  ...
  for(let i=0;i<8;i++){await sleep(250);await heartbeat(page,`plan settle ${i}`);const state=await coachState(page);assert.equal(state.active,'coach',`plan prompt must not leave Coach at sample ${i}: ${JSON.stringify(state)}`);assert.equal(state.coach,true,`Coach root disappeared at sample ${i}: ${JSON.stringify(state)}`);}
  await page.waitForFunction(()=>document.querySelector('.g4-agent-proposal'),null,{timeout:6000});
  ...
  await sleep(500);await heartbeat(page,'approved plan settles');
  const after=await coachState(page);assert.equal(after.active,'coach',`approved plan must not leave Coach: ${JSON.stringify(after)}`);assert.equal(after.coach,true);
  ...
  const trace=await page.evaluate(()=>window.__coachRouteTrace||[]);assert.equal(trace.filter(x=>x.route&&x.route!=='coach').length,0,`plan action emitted unexpected route click: ${JSON.stringify(trace)}`);
