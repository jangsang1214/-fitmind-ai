...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>window.GarangDesignSimplificationV2?.version==='garang-design-simplification-v3.0.0'&&document.querySelector('#garangWorkoutEntryDetails')&&document.querySelector('#garangWorkoutTools')&&document.querySelector('.g3-body-model[data-garang-classical-model="5"]'),null,{timeout:6000});
  assert.ok(await page.locator('.g3-body-model[data-garang-classical-model="5"]').count()>=1,'Workout/body surfaces must render Body Model v5');
  assert.equal(await page.locator('.g3-view-switch [data-g3-view]').count(),3,'Body Model v5 must expose FRONT / SIDE / BACK views');
  ...
  assert.ok(await page.locator('.g3-body-model[data-garang-anatomy-v5="side"]').count()>=1,'Body Model v5 must render a dedicated side anatomy view');
  const bodyQuality=await page.locator('.g3-body-model[data-garang-classical-model="5"]').first().evaluate(svg=>({viewBox:svg.getAttribute('viewBox'),silhouette:svg.querySelectorAll('.g5-silhouette').length,legacyHands:svg.querySelectorAll('.g3-hand').length,legacyFeet:svg.querySelectorAll('.g3-foot').length}));
  assert.deepEqual(bodyQuality,{viewBox:'0 0 260 520',silhouette:1,legacyHands:0,legacyFeet:0},'Body Model v5 must render as one continuous human silhouette instead of segmented mannequin limbs');
