...
(async()=>{
 ...
 try{
  ...
  const bootOwnership=await page.evaluate(()=>{const scripts=[...document.scripts],paths=scripts.map(node=>new URL(node.src,location.href).pathname),goal=paths.map((path,index)=>({path,index})).filter(row=>row.path.endsWith('/02_core/goal-alignment-v1.js')),shell=paths.map((path,index)=>({path,index})).filter(row=>row.path.endsWith('/06_features/ui/runtime/garang-simplified-shell-v1.js')),core=paths.map((path,index)=>({path,index})).filter(row=>row.path.endsWith('/06_features/ui/runtime/garang-core-loop-v1.js'));return {goal,shell,core,dynamicCore:scripts.filter(node=>node.dataset.garangCoreLoopV1==='1').length};});
