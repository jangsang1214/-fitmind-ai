...
async function verifyCapabilityRoute(context,screen,selector){const page=await context.newPage(),errors=[];page.on('pageerror',e=>errors.push(String(e?.stack||e?.message||e)));const snapshot=()=>page.evaluate(({screen,selector})=>{const m=document.getElementById('main');return {expected:screen,current:m?.dataset?.garangScreen||null,router:window.GarangRouter?.current?.()||null,control:document.querySelectorAll(selector).length,header:m?.querySelector('.page-head')?.innerText?.slice(0,180)||'',html:m?.innerHTML?.slice(0,260)||''};},{screen,selector});try{await page.goto(baseURL,{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>document.getElementById('appView')&&!document.getElementById('appView').hidden,{timeout:15000});await page.waitForFunction(()=>window.GarangRouter&&window.GarangScreens,{timeout:8000});const immediate=await page.evaluate(({screen,selector})=>{const ok=window.GarangRouter?.navigate?.(screen,{source:'product-consolidation-capability-preservation',force:true})===true,m=document.getElementById('main');return {ok,current:m?.dataset?.garangScreen||null,router:window.GarangRouter?.current?.()||null,control:document.querySelectorAll(selector).length,header:m?.querySelector('.page-head')?.innerText?.slice(0,180)||''};},{screen,selector});const trace=[{at:0,...immediate}];await page.waitForTimeout(100);trace.push({at:100,...await snapshot()});await page.waitForTimeout(400);trace.push({at:500,...await snapshot()});await page.waitForTimeout(700);trace.push({at:1200,...await snapshot()});console.log(`capability-route ${screen}: ${JSON.stringify(trace)}`);assert.equal(immediate.ok,true,`${screen} capability route must remain available`);const last=trace.at(-1);assert.equal(last.current,screen,`${screen} must remain the active capability screen after runtime reconciliation: ${JSON.stringify(trace)}`);assert.equal(last.control,1,`${screen} canonical control ${selector} must remain attached after runtime reconciliation: ${JSON.stringify(trace)}`);assert.deepEqual(errors,[],`${screen} capability route browser errors:\n${errors.join('\n')}`);}finally{await page.close();}}
(async()=>{
