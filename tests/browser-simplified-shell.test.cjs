...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>window.GarangProductConsolidationV1?.version==='garang-product-consolidation-v1.1.0'&&window.GarangSimplifiedShell&&window.GarangRouter&&window.GarangGoalAlignment&&window.GarangCoreLoopV1,null,{timeout:8000});
  const bootOwnership=await page.evaluate(()=>{const scripts=[...document.scripts],paths=scripts.map(node=>new URL(node.src,location.href).pathname),goal=paths.map((path,index)=>({path,index})).filter(row=>row.path.endsWith('/02_core/goal-alignment-v1.js')),shell=paths.map((path,index)=>({path,index})).filter(row=>row.path.endsWith('/06_features/ui/runtime/garang-simplified-shell-v1.js')),core=paths.map((path,index)=>({path,index})).filter(row=>row.path.endsWith('/06_features/ui/runtime/garang-core-loop-v1.js')),links=[...document.querySelectorAll('link[rel="stylesheet"]')].map((node,index)=>({path:new URL(node.href,location.href).pathname,index})),coreStyle=links.filter(row=>row.path.endsWith('/03_styles/runtime/garang-core-loop-v1.css')),shellStyle=links.filter(row=>row.path.endsWith('/03_styles/runtime/garang-simplified-shell-v1.css'));return {goal,shell,core,dynamicCore:scripts.filter(node=>node.dataset.garangCoreLoopV1==='1').length,coreStyle,shellStyle};});
  assert.equal(bootOwnership.goal.length,1,`Goal Alignment must have one explicit boot owner: ${JSON.stringify(bootOwnership)}`);assert.equal(bootOwnership.shell.length,1,`Simplified Shell must boot once: ${JSON.stringify(bootOwnership)}`);assert.equal(bootOwnership.core.length,1,`Core Loop must have one explicit boot owner: ${JSON.stringify(bootOwnership)}`);assert.equal(bootOwnership.dynamicCore,0,`Simplified Shell must not dynamically inject Core Loop: ${JSON.stringify(bootOwnership)}`);assert.ok(bootOwnership.goal[0].index<bootOwnership.shell[0].index&&bootOwnership.shell[0].index<bootOwnership.core[0].index,`Goal Alignment -> Simplified Shell -> Core Loop boot order must be deterministic: ${JSON.stringify(bootOwnership)}`);assert.equal(bootOwnership.coreStyle.length,1,`Core Loop CSS must have one explicit stylesheet owner: ${JSON.stringify(bootOwnership)}`);assert.equal(bootOwnership.shellStyle.length,1,`Simplified Shell CSS must boot once: ${JSON.stringify(bootOwnership)}`);assert.ok(bootOwnership.coreStyle[0].index<bootOwnership.shellStyle[0].index,`Core Loop -> Simplified Shell stylesheet order must be deterministic: ${JSON.stringify(bootOwnership)}`);
