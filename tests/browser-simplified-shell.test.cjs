...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>window.GarangDesignSimplificationV2?.version==='garang-design-simplification-v2.0.0'&&document.querySelector('#garangWorkoutTools')&&document.querySelector('.g3-body-model[data-garang-classical-model="2"]'),null,{timeout:6000});
  assert.ok(await page.locator('.g3-body-model[data-garang-classical-model="2"]').count()>=1,'Workout/body surfaces must render the classical performance model');
