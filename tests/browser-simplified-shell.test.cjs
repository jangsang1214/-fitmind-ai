...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>window.GarangDesignSimplificationV2?.version==='garang-design-simplification-v3.0.0'&&document.querySelector('#garangWorkoutEntryDetails')&&document.querySelector('#garangWorkoutTools')&&document.querySelector('.g3-body-model[data-garang-classical-model="6"][data-garang-anatomy-v6="front"]'),null,{timeout:6000});
  assert.ok(await page.locator('.g3-body-model[data-garang-classical-model="6"][data-garang-anatomy-v6="front"]').count()>=1,'Workout/body surfaces must render the male FRONT mesh visual layer');
  assert.equal(await page.locator('.g3-view-switch [data-g3-view]').count(),3,'Body Model must preserve FRONT / SIDE / BACK views');
  const frontQuality=await page.locator('.g3-body-model[data-garang-anatomy-v6="front"]').first().evaluate(svg=>({viewBox:svg.getAttribute('viewBox'),visual:svg.querySelectorAll('[data-garang-visual-layer="mesh"]').length,zones:svg.querySelectorAll('[data-garang-interaction-layer="zones"] .g3-muscle').length,href:svg.querySelector('.g6-visual-layer')?.getAttribute('href')||'',zoneTransform:svg.querySelector('[data-garang-interaction-layer="zones"]')?.getAttribute('transform'),filter:getComputedStyle(svg).filter,revision:svg.dataset.garangVisualRevision,definitions:svg.querySelectorAll('[data-garang-definition-layer="v7"] .g7-definition').length,bodySvgCount:svg.closest('.body-view')?.querySelectorAll('svg').length}));
  assert.equal(frontQuality.viewBox,'0 0 720 1100');assert.equal(frontQuality.visual,1);assert.ok(frontQuality.zones>=6);assert.match(frontQuality.href,/body-model-v6\/male-front\.svg/);assert.equal(frontQuality.zoneTransform,null,'Body v6 muscle zones must share the mesh coordinate system directly');assert.equal(frontQuality.filter,'none','Body v6 must not rasterize the full model through a CSS filter');assert.equal(frontQuality.revision,'7');assert.ok(frontQuality.definitions>=8,'Body v7 must expose visible anatomy definition lines');assert.equal(frontQuality.bodySvgCount,1,'Body v7 must remove legacy duplicate body SVG layers');
