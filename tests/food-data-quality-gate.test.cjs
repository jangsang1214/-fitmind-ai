...
assert.equal(report.coverage.servingRows,foods.length,'all canonical foods require serving display metadata');
assert.equal(report.coverage.verifiedTraceable,report.statusCounts.verified,'every verified row requires traceable provenance');
assert.ok(report.coverage.aliasRows>=5,'alias coverage must not regress below the current reviewed baseline');
assert.equal(report.coverage.englishNameRows,0,'English-name coverage is still explicitly absent until sourced data is added');
