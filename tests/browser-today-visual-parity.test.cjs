...
function seedState(){
  ...
  return {
    meta:{schemaVersion:5,updatedAt:now},profile:{name:'Visual Parity',age:27,height:174,weight:70,gender:'male',goal:'근육 증가'},
    onboarding:{complete:true,skipped:false,goal:'근육 증가',weeklyFrequency:4,availableMinutes:60},preferences:{language:'ko',unit:'metric'},planner:[],
...
async function bootContext(browser,width,height,options={}){
  ...
  await context.addInitScript(payload=>{localStorage.setItem('garang_demo','1');localStorage.setItem('garang_demo_state_v3',JSON.stringify(payload));},seedState());
...
async function verifyHeroStable(page){
  const same=await page.evaluate(()=>new Promise(resolve=>{
    const hero=document.querySelector('#garangTodayBrandHero'),shell=hero?.querySelector('.gac5-shell');
    window.dispatchEvent(new CustomEvent('garang:state-updated'));window.dispatchEvent(new CustomEvent('garang:workout-intelligence-rendered'));
...
async function verifyCinematicContract(page,width){
  ...
  assert.equal(await page.evaluate(()=>window.GarangAccumulationMotionV1?.assetStatus),'production-required','branch must wait for mastered cinematic binaries instead of synthesizing fake water');
  assert.equal(await page.locator('#main').getAttribute('data-garang-cinematic-status'),'production-required');
  assert.equal(await page.evaluate(()=>typeof window.GarangAccumulationMotionV1?.playEvent),'function','cinematic runtime must expose event-driven playback');

  const stage=page.locator('#garangTodayBrandHero .gtd3-ripple'),stageBox=await stage.boundingBox();
  ...
  const shell=page.locator('#garangTodayBrandHero .gac5-shell');await shell.waitFor({state:'visible',timeout:5000});
  ...
  assert.equal(await legacyRipple.count(),1);assert.equal(await legacyRipple.evaluate(img=>getComputedStyle(img).display),'none','legacy SVG ripple must never be visible');
}
...
async function verifyViewport(browser,width,height){
  ...
  assert.doesNotMatch(await page.locator('#garangTodayBrandHero').innerText(),/QUIETLY|BECOMING/);
  assert.equal(await page.locator('#garangTodayBrandHero [data-gtd-coach]').count(),1);
  await verifyCinematicContract(page,width);await verifyHeroStable(page);

  const scoreHead=await page.locator('#garangTodayFlow .gtd3-score-head').innerText();assert.match(scoreHead,/GARANG SCORE/);assert.match(scoreHead,/오늘/);
  assert.equal(await page.locator('#garangTodayFlow .gtf-track').count(),3);
  const signal=await page.locator('#garangTodayFlow .gtf-signal').evaluate(node=>({radius:parseFloat(getComputedStyle(node).borderRadius||'0'),width:node.getBoundingClientRect().width,height:node.getBoundingClientRect().height}));
  assert.ok(signal.radius<8&&signal.width<=115&&signal.height<=95,`GARANG Score must stay editorial and compact: ${JSON.stringify(signal)}`);

  const checkin=page.locator('#garangTodayFlow [data-garang-checkin-access="1"]');await checkin.waitFor({state:'visible',timeout:5000});
  assert.match(await checkin.innerText(),/오늘 상태 체크인/);assert.match(await checkin.innerText(),/30초/);assert.equal(await checkin.locator('canvas').count(),0);
  const checkinBox=await checkin.boundingBox();assert.ok(checkinBox&&checkinBox.x>=0&&checkinBox.x+checkinBox.width<=width+1);
  assert.ok(await checkin.evaluate(node=>parseFloat(getComputedStyle(node).borderRadius||'0'))<4);
  ...
  const metrics=page.locator('#garangTodayDensity .gtd3-metric');assert.equal(await metrics.count(),4);
  const metricText=(await metrics.allInnerTexts()).join(' | ');assert.match(metricText,/45분/);assert.match(metricText,/1,073 kcal/);assert.match(metricText,/2개/);
  assert.equal(await page.locator('#garangTodayDensity .gtd3-record').count(),2);
  const workout=page.locator('.garang-daily-workout');assert.equal(await workout.getAttribute('data-expanded'),'0');assert.equal(await workout.locator('[data-daily-expand]').isHidden(),true);
  ...
  assert.ok(layout.scroll<=layout.client+1&&layout.main<=width+1&&layout.body<=width+1,`Today must respect viewport ${width}: ${JSON.stringify(layout)}`);
...
async function verifyInstagramInApp(browser){
  ...
  assert.equal(await page.evaluate(()=>window.GarangAccumulationMotionV1?.inAppBrowser),true);
  assert.equal(await page.locator('#garangTodayBrandHero video[playsinline]').count(),2);
  assert.equal(await page.locator('#garangTodayBrandHero canvas').count(),0);
  await verifyHeroStable(page);assert.deepEqual(errors,[],`Instagram in-app Today browser errors:\n${errors.join('\n')}`);await context.close();
}
...
async function verifyReducedMotion(browser){
  ...
  assert.equal(await page.locator('#main').getAttribute('data-garang-motion'),'reduced');assert.equal(await page.evaluate(()=>window.GarangAccumulationMotionV1?.reducedMotion),true);
  const videos=page.locator('#garangTodayBrandHero video');assert.equal(await videos.count(),2);
  for(let i=0;i<await videos.count();i++)assert.equal(await videos.nth(i).evaluate(v=>v.paused),true);
  assert.equal(await page.locator('#garangTodayBrandHero canvas').count(),0);
  const layout=await page.evaluate(()=>({scroll:document.documentElement.scrollWidth,client:document.documentElement.clientWidth}));assert.ok(layout.scroll<=layout.client+1);
  assert.deepEqual(errors,[],`Today reduced-motion browser errors:\n${errors.join('\n')}`);await context.close();
}

(async()=>{
  const server=startStaticServer(serveRoot,port);let browser;
  try{await waitForServer();browser=await webkit.launch({headless:true});await verifyViewport(browser,390,844);await verifyViewport(browser,393,852);await verifyViewport(browser,430,932);await verifyInstagramInApp(browser);await verifyReducedMotion(browser);console.log('browser-today-visual-parity GARANG cinematic asset controller v5: PASS');}
