...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>{const button=document.querySelector('.garang-daily-workout .garang-daily-head .gci-toggle');if(!button)return false;const s=getComputedStyle(button),b=button.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&b.width>0&&b.height>0;},null,{timeout:5000});
  ...
  await page.waitForFunction(()=>{const target=document.querySelector('.garang-daily-workout [data-daily-target]'),generate=document.querySelector('.garang-daily-workout [data-daily-generate]');if(!target||!generate)return false;const ts=getComputedStyle(target),gs=getComputedStyle(generate);return ts.display!=='none'&&ts.visibility!=='hidden'&&gs.display!=='none'&&gs.visibility!=='hidden';},null,{timeout:5000});
  ...
  await page.waitForFunction(()=>!!document.querySelector('.garang-daily-workout [data-garang-workout-prep-start="1"]'),null,{timeout:5000});
