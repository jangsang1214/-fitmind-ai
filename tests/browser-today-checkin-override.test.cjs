...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>window.GarangTodayCheckinOverrideV1?.version==='1.3.0'&&window.GarangTodayWorkoutPrepIntegrationV1?.version==='1.1.0'&&document.getElementById('main')?.dataset?.garangScreen==='today',{timeout:10000});
  ...
  await page.waitForFunction(()=>{const main=document.getElementById('main'),start=document.querySelector('.garang-daily-workout [data-garang-workout-prep-start="1"]'),checkin=document.querySelector('#main > [data-garang-bottom-checkin="1"]');if(!start||!checkin)return false;const ss=getComputedStyle(start),sb=start.getBoundingClientRect(),cb=checkin.getBoundingClientRect();return main?.dataset?.garangWorkoutPrepExecution==='1'&&ss.display!=='none'&&ss.visibility!=='hidden'&&sb.width>0&&sb.height>=44&&cb.height>=48;},null,{timeout:5000});
