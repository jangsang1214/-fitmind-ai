...
(async()=>{
 ...
 assert.match(endpoint,/cloudfunctions\.net\/api\/coach$/);
 ...
 assert.equal(calls.length,1);const sent=calls[0];assert.equal(sent.input,endpoint);assert.equal(sent.init.headers.get('Authorization'),'Bearer id-token-1');const body=JSON.parse(sent.init.body);assert.deepEqual(body,{message:'오늘 벤치 세게 해도 돼?',language:'ko'});assert.equal('context' in body,false);assert.equal('uid' in body,false);
