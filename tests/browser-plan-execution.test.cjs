...
(async()=>{
  ...
  try{
    ...
    assert.match(summary,/계획 실행/);assert.doesNotMatch(summary,/누적\./,'Planner must not expose the accumulation surface label');assert.match(summary,/Coach에서 지금의 흐름을 함께 풀어보세요\./);assert.match(summary,/벤치프레스/);assert.match(summary,/2,270 kcal/);assert.match(summary,/118\s*g/);assert.match(summary,/수면 7\.5h/);assert.match(summary,/GARANG INSIGHT/);
