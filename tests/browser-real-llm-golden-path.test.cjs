...
(async()=>{
 ...
 try{
  ...
  state=await page.evaluate(()=>window.GarangAgentStateBridge.getState());assert.equal(state.planner.filter(row=>row.id==='server-plan-1').length,1,'one explicit request must converge to exactly one server-owned plan');assert.equal(state.planner.find(row=>row.id==='server-plan-1')?.source,'ai');
