...
(async()=>{
 ...
 try{
  ...
  assert.equal(transport.endpoint,endpoint,'Coach must retain the production Functions endpoint');assert.equal(transport.transport,'garang-coach-gateway-transport-v1.1.1');assert.equal(transport.uid,'llm-user');assert.equal(transport.getIdToken,'function');assert.equal(transport.token,'firebase-id-token-llm-user');
  ...
  const gatewayDiag=await page.evaluate(()=>structuredClone(window.__GARANG_COACH_GATEWAY_TRANSPORT_V1__?.diagnostics||null));
  assert.equal(gatewayCalls.length>=1,true,`authenticated Coach must call the real gateway transport; assistant=${latestAssistant}; transport=${JSON.stringify(transport)}; gatewayDiag=${JSON.stringify(gatewayDiag)}`);assert.match(latestAssistant,/REAL LLM:/,`successful gateway response must be rendered as the Coach answer; assistant=${latestAssistant}`);
