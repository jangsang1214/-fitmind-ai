 ...
 try{
  ...
  await context.addInitScript(({coachEndpoint})=>{
   ...
   window.fetch=async(input,init={})=>{
    ...
    const message=String(body.message||''),confirmationIntent=/확인 필요 테스트/.test(message),deniedIntent=/거부 테스트/.test(message),planIntent=/계획|plan/i.test(message);
    const answer=confirmationIntent?'SERVER CONFIRMATION: 이 변경은 추가 확인이 필요합니다.':deniedIntent?'SERVER DENIED: 이 변경은 안전 정책상 반영하지 않았습니다.':planIntent?'SERVER PLAN: 요청한 오늘 계획을 GARANG 서버에서 반영했습니다.':'REAL LLM: 최근 훈련량은 있지만 오늘 회복과 수면 신호가 낮아 강도를 낮추는 편이 좋습니다.';
    const toolResults=confirmationIntent?[{name:'createPlan',callId:'server-confirm-1',status:'confirmation_required',code:'CONFIRMATION_REQUIRED',executed:false,duplicate:false,targetId:null}]:deniedIntent?[{name:'createPlan',callId:'server-denied-1',status:'denied',code:'POLICY_DENIED',executed:false,duplicate:false,targetId:null}]:planIntent?[{name:'createPlan',callId:'server-plan-1',status:'executed',code:null,executed:true,duplicate:false,targetId:'plan-server-1'}]:[];
    const requestId=confirmationIntent?'req-confirmation':deniedIntent?'req-denied':planIntent?'req-server-plan':'req-analysis';
    return new Response(JSON.stringify({ok:true,answer,data:{answer,decisionSummary:'GARANG은 오늘 강도 감소를 권장합니다.',reasoningSummary:'낮은 수면과 에너지 신호를 우선 반영했습니다.',suggestedNextStep:'오늘 계획의 운동 볼륨을 조정하세요.',confidence:.86,source:'llm',requestId,toolResults,metadata:{provider:'mock-openai',model:'gpt-test'},garangDecision:{mode:'reduce',reasonCodes:['SHORT_SLEEP','LOW_ENERGY']}}}),{status:200,headers:{'Content-Type':'application/json'}});
   };
  ...
  for(const scenario of [
    {message:'확인 필요 테스트 계획을 저장해줘',status:'confirmation_required',requestId:'req-confirmation'},
    {message:'거부 테스트 계획을 저장해줘',status:'denied',requestId:'req-denied'}
  ]){
    const before=await page.locator('.g2-message.assistant:not([data-thinking="1"])').count();await input.fill(scenario.message);await tap(page,'.g2-send',scenario.status);
    await page.waitForFunction(before=>document.querySelectorAll('.g2-message.assistant:not([data-thinking="1"])').length>before,before,{timeout:10000});
    await page.waitForTimeout(120);
    const snapshot=await page.evaluate(()=>{
      const assistants=[...document.querySelectorAll('.g2-message.assistant:not([data-thinking="1"])')],latestEl=assistants.at(-1);
      const key=Object.keys(localStorage).find(k=>k.startsWith('garang_coach_threads_v2::garang_user_llm-user_v3')),store=key?JSON.parse(localStorage.getItem(key)||'null'):null,thread=store?.threads?.find(t=>t.id===store.activeId)||store?.threads?.[0],latest=(thread?.messages||[]).filter(m=>m.role==='assistant').at(-1)||null;
      return {owner:latestEl?.dataset?.g4ActionOwner||null,proposalCount:document.querySelectorAll('.g4-agent-proposal').length,latest};
    });
    assert.equal(snapshot.owner,'server',`${scenario.status} must remain server-owned: ${JSON.stringify(snapshot)}`);
    assert.equal(snapshot.proposalCount,0,`${scenario.status} must not create a browser proposal: ${JSON.stringify(snapshot)}`);
    assert.equal(snapshot.latest?.requestId,scenario.requestId);assert.equal(snapshot.latest?.toolResults?.[0]?.status,scenario.status);assert.equal(snapshot.latest?.toolResults?.[0]?.executed,false);
  }
