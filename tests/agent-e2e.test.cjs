...
(async()=>{
 ...
 await test('rejected write changes nothing',async()=>{
  const before=state.planner.length;const session=Agent.createSession({getState:()=>bridge.getState(),applyWrite:(tool,args)=>bridge.applyWrite(tool,args)});const result=await session.run({message:'Create a plan',context:{},language:'en'},{adapter:Agent.createMockAdapter()});session.confirm(result.proposals[0].id,false);assert.equal(state.planner.length,before);
 });

 await test('approved goal write changes live GARANG state and persists current semantic goal',async()=>{
  const session=Agent.createSession({getState:()=>bridge.getState(),applyWrite:(tool,args)=>bridge.applyWrite(tool,args)});const result=await session.run({message:'Change my goal',context:{},language:'en'},{adapter:Agent.createMockAdapter()});session.confirm(result.proposals[0].id,true);
  assert.equal(state.profile.goal,'Improve performance');assert.equal(state.onboarding.goal,'Improve performance');
  const persisted=JSON.parse(localStorage.getItem('garang_demo_state_v3'));assert.equal(persisted.profile.goal,'Improve performance');assert.ok(persisted.actionLog.some(x=>x.action==='agent_updateGoal'&&x.userConfirmed===true));assert.ok(state.memory.entries.some(x=>x.key==='primary_goal'&&x.value==='Improve performance'&&x.status==='active'));
 });

 await test('memory write preserves changed values as temporal history',()=>{
  ...
  const rows=state.memory.entries.filter(x=>x.type==='goal'&&x.key==='race_goal');assert.equal(rows.length,2);const active=rows.find(x=>x.status==='active'),history=rows.find(x=>x.status==='superseded');assert.equal(active.value,'10K under 44 minutes');assert.equal(history.value,'10K under 45 minutes');assert.equal(history.supersededBy,active.id);assert.equal(active.userConfirmed,true);
 });

 await test('memory bridge returns only active relevant context and diagnostics',()=>{
  const contextOut=bridge.getMemoryContext('44 minutes',{limit:5});assert.ok(contextOut.entries.some(x=>x.value.includes('44 minutes')));assert.ok(!contextOut.entries.some(x=>x.value.includes('45 minutes')));const report=bridge.getMemoryDiagnostics();assert.ok(report.active>=1);assert.ok(report.superseded>=1);
 });

 await test('memory deletion records deleted ID to prevent resurrection',()=>{
