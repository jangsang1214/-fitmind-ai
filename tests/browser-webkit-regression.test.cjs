...
(async()=>{
  ...
  try{
    ...
    assert.match(await page.locator('.gws-panel:not([hidden]) .workout-set-table-head').innerText(),/SET\s+PREVIOUS\s+TARGET\s+TYPE\s+KG\s+REPS\s+RPE\s+RIR\s+DEL\s+✓/,'set-first table must expose the full commercial execution hierarchy');
    ...
    assert.equal(await page.evaluate(()=>window.GarangWorkoutExecutionBridge?.sessionContext?.().activePlanId),programPlanId,'active Planner identity must be exposed before reload');
    await page.reload({waitUntil:'domcontentloaded'});
    await page.waitForFunction(()=>document.getElementById('appView')&&!document.getElementById('appView').hidden,{timeout:15000});
    await page.evaluate(()=>window.GarangRouter?.navigate?.('workout',{source:'program-reload-regression',force:true}));
    await page.waitForFunction(()=>document.getElementById('main')?.dataset?.garangScreen==='workout',{timeout:5000});await tap(page,'[data-gws-step="log"]');
    await page.waitForFunction(id=>window.GarangWorkoutExecutionBridge?.sessionContext?.().activePlanId===id,programPlanId,{timeout:5000});
    assert.equal(await page.evaluate(()=>window.GarangWorkoutExecutionBridge?.sessionContext?.().activePlanId),programPlanId,'active Planner identity must survive workout-session reload recovery');
