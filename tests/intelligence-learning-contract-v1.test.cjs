...
test('Intelligence Bridge exposes confidence-gated User Performance context without Decision ownership',()=>{
  ...
  assert.equal(bridge.userPerformanceReady(),true);assert.equal(context.minConfidence,0.5);assert.equal(Object.keys(context.dimensions).join(','),'trainingConsistency');assert.ok(context.withheldDimensions.some(row=>row.name==='recommendationOutcomeEffectiveness'&&row.reason==='NO_VALUE'));assert.equal(context.guardrails.affectsDecision,false);assert.equal(JSON.stringify(state),before);
});
