...
test('runtime is wired before app state capture and PWA shell caches it',()=>{
 ...
 for(const token of ['workoutHistory','mealHistory','batch.commit','mergeStateWithHistory','syncTombstones','backfillIfNeeded'])assert.ok(runtime.includes(token),token);
