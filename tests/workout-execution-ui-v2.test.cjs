...
assert.ok(polish.includes('function v7DefinitionMarkup')&&polish.includes('data-garang-visual-revision="7.1"')&&polish.includes('data-garang-overlay-mask="body-alpha"')&&polish.includes('style="mask-type:alpha"')&&polish.includes("view.querySelectorAll('svg').forEach(node=>node.remove())"),'Body v7.1 must mask overlays to the body alpha, preserve anatomy definition, and remove duplicate legacy body SVGs');
...
assert.ok(polishCss.includes('GARANG Body v7')&&polishCss.includes('fill:transparent!important')&&polishCss.includes('pointer-events:all')&&polishCss.includes('opacity:.30!important')&&polishCss.includes('stroke:none!important'),'Body v7.1 hit zones must stay interactive while selected overlays remain restrained and borderless');
