...
assert.ok(polish.includes('function v6MeshSVG')&&polish.includes('male-front.svg?v=6.0.0-mesh-layer')&&polish.includes('male-side.svg?v=6.0.0-mesh-layer')&&polish.includes('data-garang-visual-layer="mesh"')&&polish.includes('data-garang-interaction-layer="zones"'),'Body Model v6 must separate mesh visual geometry from interaction/highlight zones for supported views');
assert.ok(polish.includes("gender==='male'&&(side==='front'||side==='side')")&&polish.includes('v5BackSVG(gender)')&&polish.includes('v5FrontSVG(gender)'),'Body Model v6 must fail safely to v5 where a reviewed mesh asset does not yet exist');
