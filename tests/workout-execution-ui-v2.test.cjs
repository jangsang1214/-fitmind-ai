...
assert.ok(runtime.includes("canonicalSave.hidden=true")&&runtime.includes("workout-canonical-save")&&runtime.includes("style.setProperty('display','none','important')")&&runtime.includes("aria-hidden"),'legacy text save control must be hard-hidden while remaining the canonical persistence owner');
