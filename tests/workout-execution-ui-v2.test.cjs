...
assert.ok(polish.includes('function v6MeshSVG')&&polish.includes('function v6ZoneMarkup')&&polish.includes('/${person}-${view}.svg?v=7.1.0-overlay-fit')&&polish.includes('data-garang-visual-layer="mesh"')&&polish.includes('data-garang-interaction-layer="zones"'),'Body Model v6 must separate complete mesh visual geometry from interaction/highlight zones');
...
assert.ok(polish.includes('function v7DefinitionMarkup')&&polish.includes('function v72MuscleTextureMarkup')&&polish.includes('data-garang-visual-revision="7.2"')&&polish.includes('data-garang-volume-layer="v7.2"')&&polish.includes('data-garang-overlay-mask="body-alpha"')&&polish.includes('style="mask-type:alpha"')&&polish.includes("view.querySelectorAll('svg').forEach(node=>node.remove())"),'Body v7.2 must add muscle volume/fiber definition inside the body alpha mask and remove duplicate legacy body SVGs');
assert.ok(polish.includes('data-g71-zone-side="left"')&&polish.includes('data-g71-zone-side="right"')&&polish.includes('data-g71-zone-part="left-thigh"')&&polish.includes('data-g71-zone-part="right-calf"'),'Body v7.1 FRONT limbs must use independently fitted bilateral zones');
assert.ok(polishCss.includes('GARANG Body v7.2')&&polishCss.includes('.g72-volume')&&polishCss.includes('.g72-fiber')&&polishCss.includes('fill:transparent!important')&&polishCss.includes('pointer-events:all')&&polishCss.includes('opacity:.25!important')&&polishCss.includes('stroke:none!important'),'Body v7.2 must increase muscle definition while hit zones stay interactive and selected overlays remain restrained');
for(const gender of ['male','female'])for(const view of ['front','side','back'])assert.ok(swRuntime.includes(`./05_assets/body-model-v6/${gender}-${view}.svg?v=7.1.0-overlay-fit`),`Body Model v6 ${gender}-${view} must be available in the offline app shell`);
