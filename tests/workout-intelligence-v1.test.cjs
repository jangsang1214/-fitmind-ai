...
test('per-set records own volume and weighted RPE even when legacy aggregates are stale',()=>{const s=fixture();s.workouts=[{id:'mixed',sessionId:'mixed-session',date:'2026-09-05',name:'바벨 벤치프레스',primaryMuscle:'가슴',sets:3,reps:8,weight:60,rpe:8,volume:9999,setDetails:[{set:1,reps:10,weight:40,rpe:6},{set:2,reps:8,weight:60,rpe:7},{set:3,reps:6,weight:80,rpe:9}]}];assert.equal(Core.recordVolume(s.workouts[0]),1360);const chest=Core.analyzeMuscleLoad(s,db,{now,days:7})[0];assert.equal(chest.sets,3);assert.equal(chest.volume,1360);assert.equal(chest.avgRPE,7.3);});
...
test('runtime exposes Today recommender and keeps one canonical set-entry owner',()=>{const source=fs.readFileSync(path.join(root,'06_features/ui/runtime/garang-workout-intelligence-ui-v1.js'),'utf8'),app=fs.readFileSync(path.join(root,'01_app/app.js'),'utf8');for(const token of ['garang-daily-workout','garang_workout_import_queue_v1','#addWorkout','garang_coach_threads_v2::'])assert.ok(source.includes(token),token);for(const token of ['workoutSetDetailsOpen','data-set-row','data-set-weight','data-set-reps','data-set-rpe'])assert.ok(app.includes(token),token);assert.equal(source.includes('garang-set-builder'),false,'duplicate runtime set builder must stay removed');assert.equal(source.includes('data-garang-set-row'),false,'runtime must not create a second set-row contract');});
