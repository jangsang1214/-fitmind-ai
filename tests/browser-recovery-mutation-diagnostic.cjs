...
const diagnosticInit=`\n  await context.addInitScript(()=>{\n    const NativeMutationObserver=window.MutationObserver;\n    let moSequence=0,rafSequence=0;\n    window.__garangExecutionDiagnostic={armed:false,stats:[],arm:null};\n    class DiagnosticMutationObserver{\n      constructor(callback){\n        const source=String(callback||'').replace(/\\s+/g,' ').slice(0,240);\n        const stat={id:++moSequence,total:0,armed:0,dropped:0,stack:'[callback] '+source};\n        window.__garangExecutionDiagnostic.stats.push(stat);\n        this.__native=new NativeMutationObserver((records)=>{\n          const armed=window.__garangExecutionDiagnostic.armed===true;\n          if(!armed)return callback(records,this);\n          stat.total++;\n          stat.armed++;\n          if(stat.armed<=2)console.log('[recovery-mo-enter] '+JSON.stringify({id:stat.id,armed:stat.armed,records:records.length,stack:stat.stack}));\n          if(stat.armed>40){\n            stat.dropped++;\n            if(stat.dropped===1)console.log('[recovery-mo-loop] '+JSON.stringify({id:stat.id,armed:stat.armed,stack:stat.stack}));\n            return;\n          }\n          const result=callback(records,this);\n          if(stat.armed<=2)console.log('[recovery-mo-exit] '+JSON.stringify({id:stat.id,armed:stat.armed}));\n          return result;\n        });\n      }\n      observe(...args){return this.__native.observe(...args);}\n      disconnect(){return this.__native.disconnect();}\n      takeRecords(){return this.__native.takeRecords();}\n    }\n    window.MutationObserver=DiagnosticMutationObserver;\n    window.__garangExecutionDiagnostic.arm=()=>{\n      if(window.__garangExecutionDiagnostic.armed)return;\n      window.__garangExecutionDiagnostic.armed=true;\n      for(const stat of window.__garangExecutionDiagnostic.stats){stat.total=0;stat.armed=0;stat.dropped=0;}\n      const nativeRAF=window.requestAnimationFrame.bind(window);\n      window.requestAnimationFrame=function(callback){\n        const id=++rafSequence;\n        const stack=String(new Error('requestAnimationFrame scheduled').stack||'');\n        return nativeRAF(timestamp=>{\n          if(id<500)console.log('[recovery-raf-enter] '+JSON.stringify({id,stack}));\n          const result=callback(timestamp);\n          if(id<500)console.log('[recovery-raf-exit] '+JSON.stringify({id}));\n          return result;\n        });\n      };\n    };\n  });`;
...
source=source.replace(armNeedle,"await page.evaluate(()=>window.__garangExecutionDiagnostic.arm());"+armNeedle);
