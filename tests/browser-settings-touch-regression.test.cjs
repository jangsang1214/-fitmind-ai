...
async function route(page,name){await tap(page,`#bottomNav [data-garang-primary-nav="1"][data-page="${name}"]`);await page.waitForFunction(n=>document.getElementById('main')?.dataset?.garangScreen===n,name,{timeout:5000});}
...
async function assertSettingsInteractive(page,label){
 ...
 assert.equal(state.screen,'settings',`${label}: Settings must own screen id`);assert.equal(state.gearHandler,'function',`${label}: canonical top gear handler must remain intact`);assert.equal(state.router,'garang-router-v1.3.0',`${label}: canonical router must own route transport`);assert.equal(state.privacy,'v1.5',`${label}: canonical privacy runtime must own Settings privacy`);assert.equal(state.legacySafety,null,`${label}: retired Settings safety overlay must stay absent`);assert.equal(state.hit,true,`${label}: Settings control must be hit-testable; ${JSON.stringify(state)}`);assert.deepEqual(state.blockers,[],`${label}: no stale blocker may remain; ${JSON.stringify(state)}`);
