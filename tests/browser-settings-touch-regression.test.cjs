...
(async()=>{
 ...
 try{
  ...
  stage('privacy in-app delete panel');await tap(page,'#deleteAccountSecure');await page.locator('#deleteAccountPanel').waitFor({state:'visible',timeout:3000});await tap(page,'#cancelDeleteAccount');await page.locator('#deleteAccountPanel').waitFor({state:'hidden',timeout:3000});
