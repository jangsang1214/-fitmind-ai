...
(async()=>{
 ...
 try{
  ...
  const stableDom=await page.evaluate(async()=>{const before=document.querySelector('#garangTodayFlow'),button=before?.querySelector('.gtf-next[data-gsn-action="coach"][data-gsn-step="plan"]');window.dispatchEvent(new CustomEvent('garang:state-updated',{detail:{source:'lifecycle-noop-test'}}));window.dispatchEvent(new CustomEvent('garang:route-completed',{detail:{route:'today',source:'lifecycle-noop-test'}}));await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));const after=document.querySelector('#garangTodayFlow'),afterButton=after?.querySelector('.gtf-next[data-gsn-action="coach"][data-gsn-step="plan"]');return {sameFlow:before===after,sameButton:button===afterButton,key:after?.dataset?.gtfRenderKey||null,version:window.GarangTodayActionFlowV1?.version||null};});
  assert.equal(stableDom.sameFlow,true,`no-op lifecycle events must preserve the Today flow DOM identity: ${JSON.stringify(stableDom)}`);assert.equal(stableDom.sameButton,true,`no-op lifecycle events must preserve the active route button identity: ${JSON.stringify(stableDom)}`);assert.ok(stableDom.key,'Today flow must publish a render identity key');assert.equal(stableDom.version,'garang-today-action-flow-v1.3.2');
