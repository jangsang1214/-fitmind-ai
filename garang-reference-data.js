...
(() => {
  ...
  function decorateToday(){
    ...
    greeting.innerHTML=`<span>${greetingWord()}</span><strong>${currentProfileName()}.</strong>`;
    ...
    if(snapshot&&focus){
      ...
      if(label&&!label.querySelector('.gx-focus-copy')){
        const copy=document.createElement('div');copy.className='gx-focus-copy';
        copy.innerHTML=`<small>TODAY'S FOCUS</small><strong>${decisionTitle}</strong><p>${decisionSummary||existingTitle}</p>`;
        label.appendChild(copy);
      }
    }
    const decision=document.createElement('section');decision.className='gx-today-decision-card';
    decision.innerHTML=`<span>DECISION</span><strong>${decisionTitle}</strong><p>${decisionSummary}</p>`;
    if(focus)focus.insertAdjacentElement('afterend',decision);else snapshot?.insertAdjacentElement('afterend',decision);
  }

  function latestWorkoutSummary(){
    const workouts=Array.isArray(currentState()?.workouts)?currentState().workouts:[];
    if(!workouts.length)return null;
    const last=workouts[workouts.length-1];
    const sessionId=last.sessionId;
    const date=last.date;
    const rows=sessionId?workouts.filter(x=>x.sessionId===sessionId):workouts.filter(x=>x.date===date).slice(-8);
    const volume=rows.reduce((s,x)=>s+Number(x.volume||0),0);
    const duration=rows.reduce((s,x)=>s+Number(x.duration||0),0);
    const oneRm=Math.max(0,...rows.map(x=>Number(x.estimated1RM||0)));
    return {volume, duration, oneRm, count:rows.length};
  }

  function decorateWorkout(){
    if(!main.querySelector('.workout-hero-v2')||main.querySelector('.gx-workout-summary'))return;
    const persisted=latestWorkoutSummary();
    const mini=[...main.querySelectorAll('.workout-mini-stats>div')];
    const fallbackOneRm=text(main.querySelector('.one-rm-best strong')).replace(/estimated/ig,'').trim();
    const fallbackCount=text(main.querySelector('.workout-builder-v2 .pill'),'0 exercises').match(/\d+/)?.[0]||'0';
    const volume=persisted?Math.round(persisted.volume).toLocaleString():(mini[0]?text(mini[0].querySelector('strong')):'—');
    const duration=persisted?Math.round(persisted.duration):(mini[2]?text(mini[2].querySelector('strong')):'—');
    const oneRm=persisted&&persisted.oneRm?persisted.oneRm.toFixed(1):fallbackOneRm;
    const count=persisted?persisted.count:fallbackCount;
    const selected=text(main.querySelector('.workout-selected-name'),'Workout');
    const pageTitle=main.querySelector('.page-head h1');if(pageTitle)pageTitle.textContent=selected;
    const summary=document.createElement('section');summary.className='gx-workout-summary';
    summary.innerHTML=`<span class="gx-label">SESSION SUMMARY</span><div><article><small>VOLUME</small><strong>${volume}</strong><em>kg</em></article><article><small>EST. 1RM</small><strong>${oneRm}</strong><em>kg</em></article><article><small>EXERCISES</small><strong>${count}</strong></article><article><small>DURATION</small><strong>${duration}</strong><em>min</em></article></div>`;
    main.querySelector('.workout-builder-v2')?.insertAdjacentElement('beforebegin',summary);
  }
  ...
  function decorateBody(){
    ...
    heroMetrics.slice(0,3).forEach(x=>{const a=document.createElement('article');a.innerHTML=`<small>${text(x.querySelector('span'))}</small><strong>${text(x.querySelector('b'))}</strong>`;grid.appendChild(a);});
    main.querySelector('.body-entry-drawer')?.insertAdjacentElement('afterend',result);
  }
  ...
  observer.observe(main,{childList:true,subtree:true});decorate();
})();
