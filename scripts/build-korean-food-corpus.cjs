...
async function main(){
 ...
 if(!args.output||!args.meta)throw new Error('Use --output <json> --meta <json> [--page-size 1000 --max-pages 1000]');
 let payload;
 ...
 const corpus={version:VERSION,generatedAt:retrievedAt,purpose:'Korean official supplemental lookup corpus; curated GARANG canonical remains primary',precedence:['GARANG canonical Korean DB','Korean official supplemental','USDA supplemental','source-backed web lookup'],count:merged.records.length,records:merged.records};
...
module.exports={VERSION,legacy,dedupe,rawRows};
