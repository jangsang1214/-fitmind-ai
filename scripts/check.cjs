...
const bootScripts=[...html.matchAll(/<script\s+[^>]*src=["']\.\/([^"'?]+)(?:\?[^"']*)?["'][^>]*>/gi)].map(x=>x[1]);
const bootStyles=[...html.matchAll(/<link\s+[^>]*rel=["']stylesheet["'][^>]*href=["']\.\/([^"'?]+)(?:\?[^"']*)?["'][^>]*>/gi)].map(x=>x[1]);
if(JSON.stringify(bootScripts)!==JSON.stringify(manifest.scripts||[]))failures.push({file:'runtime-manifest.json',error:'Script boot order must exactly match index.html'});
if(JSON.stringify(bootStyles)!==JSON.stringify(manifest.styles||[]))failures.push({file:'runtime-manifest.json',error:'Style boot order must exactly match index.html'});
if(new Set(bootScripts).size!==bootScripts.length)failures.push({file:'index.html',error:'Duplicate active script runtime'});
if(new Set(bootStyles).size!==bootStyles.length)failures.push({file:'index.html',error:'Duplicate active stylesheet runtime'});
if([...bootScripts,...bootStyles].some(x=>/(^|\/)archive\//i.test(x)))failures.push({file:'index.html',error:'Archive assets must never boot'});
...
if(runtimeContract.status!=='frozen'||runtimeContract.version!==1)failures.push({file:'runtime-manifest.json',error:'Runtime contract must be frozen v1'});
...
const riskyActive=(manifest.scripts||[]).filter(x=>riskyName.test(x)).sort(),riskyAllowed=[...(runtimeContract.legacyNamedCanonical||[])].sort();
if(JSON.stringify(riskyActive)!==JSON.stringify(riskyAllowed))failures.push({file:'runtime-manifest.json',error:'New legacy-named active runtime requires explicit canonical allowlist review'});
...
for(const f of ['services/adapters.js','services/storage.js','02_core/data-schema.js','02_core/error-system-v1.js']){const text=fs.readFileSync(path.join(root,f),'utf8');if(/\beval\s*\(|new Function\s*\(/.test(text))failures.push({file:f,error:'Dynamic code execution forbidden'});}
...
for(const retired of files.map(f=>path.relative(root,f)).filter(x=>/^02_core[\\/](?:garang-v(?:8|9|10)[^\\/]*\.js|sw\.js)$/i.test(x)))failures.push({file:retired,error:'Retired core runtime must be archived or deleted'});
for(const required of ['06_features/ui/README.md','06_features/ui/i18n/translations.js','06_features/ui/i18n/runtime.js','02_core/schema-contract-v1.json','02_core/error-system-v1.js'])if(!fs.existsSync(path.join(root,required)))failures.push({file:required,error:'Missing frozen core/runtime source'});
const result={timestamp:new Date().toISOString(),status:failures.length?'FAIL':'PASS',counts,failures,scope:'Frozen runtime boot order and ownership, schema/error core contracts, JS/JSON syntax, active entry assets, shell IDs, i18n wiring, no loose root duplicates or retired core runtimes.'};
...
else if(process.argv.includes('--build')){const dist=path.join(root,'dist');fs.rmSync(dist,{recursive:true,force:true});for(const f of files){const relative=path.relative(root,f);if(relative.startsWith('tests'+path.sep)||relative.startsWith('scripts'+path.sep)||relative.split(path.sep).includes('archive'))continue;const to=path.join(dist,relative);fs.mkdirSync(path.dirname(to),{recursive:true});fs.copyFileSync(f,to);}console.log('Static build copied to dist');}
