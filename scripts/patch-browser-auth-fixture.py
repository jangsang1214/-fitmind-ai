...
p = Path('tests/browser-interaction.test.cjs')
s = p.read_text()
old = """      const context=await browser.newContext({viewport:mode.viewport,isMobile:mode.touch,hasTouch:mode.touch});
...
"""
new = """      const context=await browser.newContext({viewport:mode.viewport,isMobile:mode.touch,hasTouch:mode.touch});
...
"""
if old not in s:
    raise SystemExit('legacy demo browser fixture block not found')
s = s.replace(old, new, 1)
old_saved = "const repaired=await page.evaluate(()=>JSON.parse(localStorage.getItem('garang_demo_state_v3')));"
new_saved = "const repaired=await page.evaluate(()=>JSON.parse(localStorage.getItem('garang_user_mock-user_v3')));"
if old_saved not in s:
    raise SystemExit('legacy demo state assertion not found')
