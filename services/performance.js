(function(root){
 ...
 function workoutInsights(s,exerciseDb=[]){
  ...
  const detailRows=x=>Array.isArray(x?.setDetails)&&x.setDetails.length?x.setDetails:Array.isArray(x?.setsDetail)&&x.setsDetail.length?x.setsDetail:[];
  const records=(s.workouts||[]).map((x,index)=>{const details=detailRows(x),detailWeights=details.map(row=>Math.max(0,num(row?.weight??row?.w))),detailReps=details.map(row=>Math.max(0,num(row?.reps??row?.r))),weight=details.length?Math.max(...detailWeights):Math.max(0,num(x.weight)),sets=details.length?details.length:Math.max(0,num(x.sets)),reps=details.length?Math.round(detailReps.reduce((sum,value)=>sum+value,0)/details.length):Math.max(0,num(x.reps)),volume=details.length?Math.max(0,detailWeights.reduce((sum,value,index)=>sum+value*(detailReps[index]||0),0)):Math.max(0,num(x.volume)||weight*sets*reps),estimated1RM=details.length?Math.max(...detailWeights.map((value,index)=>value&&detailReps[index]?Math.round(value*(1+Math.min(detailReps[index],30)/30)*10)/10:0)):weight&&reps?Math.round(weight*(1+Math.min(reps,30)/30)*10)/10:0,name=String(x.name||'이름 없는 운동'),primaryMuscle=String(x.primaryMuscle||muscleByName.get(name.trim().toLowerCase())||'기타');return {...x,name,primaryMuscle,weight,sets,reps,volume,estimated1RM,_order:index};}).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||(num(b.createdAt)-num(a.createdAt))||(b._order-a._order));
