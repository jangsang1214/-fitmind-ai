...
const PersonalizedLearning=require('./personalized-intelligence-learning-v1.cjs');
...
function buildAgentContext(stateInput,options={}){
 ...
 const memory=prepareMemoryContext(state.memory,state,{query:String(options.query||''),now,limit:validMemoryLimit(options.memoryLimit),ownerUid}),userState=compactForContext(estimateState(state,{now})),outcome=compactOutcome(summarizeOutcomeLearning(state,{now,days:28,recentDays:7})),decision=compactDecision(decide(userState,{memoryContext:memory,outcomeContext:outcome})),learningGraph=Learning.buildGraph(state,{now,days:28}),userPerformanceModel=UserPerformance.build(state,{days:28,asOf:now,learningGraph}),userPerformance=UserPerformance.compactForContext(userPerformanceModel),longitudinalLearning=Longitudinal.compactForContext(Longitudinal.build(state,learningGraph,{days:28,asOf})),personalizedLearningFull=PersonalizedLearning.build(state,learningGraph,decision,{asOf}),personalizedLearning=PersonalizedLearning.compactForContext(personalizedLearningFull),personalizationPolicy=Personalization.compactForContext(Personalization.build(userPerformanceModel,outcome,{personalizedLearning:personalizedLearningFull}));
 return {schemaVersion:Number.isFinite(Number(state.schemaVersion))?Number(state.schemaVersion):null,profile,goal:profile&&Object.prototype.hasOwnProperty.call(profile,'goal')?profile.goal:null,workouts:latest(state.workouts,limit),meals:latest(state.meals,limit),runs:runsWithoutCoordinates(state.runs,limit),body:latest(state.body,limit),performanceScore:calculatePerformance(state,asOf),planner:latest(state.planner,limit),memory,userState,outcome,decision,userPerformance,longitudinalLearning,personalizedLearning,personalizationPolicy};
}
