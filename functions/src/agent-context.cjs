...
function buildAgentContext(stateInput,options={}){
 ...
 const memory=prepareMemoryContext(state.memory,state,{query:String(options.query||''),now,limit:validMemoryLimit(options.memoryLimit),ownerUid}),userState=compactForContext(estimateState(state,{now})),outcome=compactOutcome(summarizeOutcomeLearning(state,{now,days:28,recentDays:7})),decision=compactDecision(decide(userState,{memoryContext:memory,outcomeContext:outcome})),userPerformance=UserPerformance.compactForContext(UserPerformance.build(state,{days:28,asOf:now}));
