...
const VERSION='intelligence-episode-v1.0.0';
...
function build(stateInput={},graphInput={},options={}){
 ...
 for(const cycle of list(graph.cycles)){
  ...
  const date=dateKey(cycle?.date),plan=planFor(state,cycle),checkin=checkinFor(state,date),after=nextCheckin(state,date),response=resolution(events,recommendationIdValue);
  const completion=finite(cycle?.execution?.score);
  const episode={
   version:VERSION,
   episodeId:'episode_'+hash([cycle?.decisionId,recommendationIdValue,cycle?.planId,date].filter(Boolean).join('|')),
   date,
   context:{
    goal:clean(state?.profile?.goal||state?.onboarding?.goal)||null,
    readiness:finite(checkin?.readiness??checkin?.energy),
    sleepHours:finite(checkin?.sleepHours??checkin?.sleep),
    stress:finite(checkin?.stress??checkin?.stressLevel),
    soreness:finite(checkin?.soreness),
    availableMinutes:finite(checkin?.availableMinutes)
   },
   ...
   outcome:{outcomeId:clean(cycle?.outcomeId)||null,classification:clean(cycle?.outcome?.classification)||null,score:finite(cycle?.outcome?.score??cycle?.outcome?.rate),nextCheckin:{date:dateKey(after?.date),energy:finite(after?.energy??after?.energyLevel),sleepHours:finite(after?.sleepHours??after?.sleep),stress:finite(after?.stress??after?.stressLevel)}},
...
function compactForContext(value={}){const v=object(value)?value:{};return {version:String(v.version||VERSION),asOf:String(v.asOf||''),windowDays:Number(v.windowDays)||28,episodes:list(v.episodes).slice(-12).map(x=>({episodeId:x.episodeId,date:x.date,decision:x.decision,recommendation:x.recommendation,userResponse:x.userResponse,execution:x.execution,outcome:x.outcome,attribution:x.attribution})),guardrails:{derivedOnly:true,noRawChatRequired:true,noCausalClaim:true,noSilentMutation:true}};}
