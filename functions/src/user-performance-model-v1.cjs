...
function recentRows(rows,{days=28,asOf=new Date()}={}){
  const end=new Date(asOf);end.setHours(23,59,59,999);
  const start=new Date(end);start.setDate(start.getDate()-Math.max(0,days-1));start.setHours(0,0,0,0);
...
function recoveryStability(state,opts){
  ...
  for(const row of rows){
    const energy=finite(row?.energy),stress=finite(row?.stress),sleep=finite(row?.sleepHours??row?.sleep);
    const sorenessValues=object(row?.soreness)?Object.values(row.soreness).map(finite).filter(v=>v!==null):[];
    const soreness=sorenessValues.length?Math.max(...sorenessValues):finite(row?.soreness),parts=[];
    if(energy!==null)parts.push(clamp((energy-1)/4*100,0,100));
    if(stress!==null)parts.push(clamp((5-stress)/4*100,0,100));
    if(sleep!==null)parts.push(clamp(sleep/8*100,0,100));
    if(soreness!==null)parts.push(clamp((5-soreness)/5*100,0,100));
...
function validate(model){
  ...
  for(const [name,row] of Object.entries(model.dimensions||{})){
    for(const key of ['value','confidence','sampleSize','lastUpdated','evidenceIds'])if(!Object.prototype.hasOwnProperty.call(row||{},key))reasons.push(`${name}:${key}:missing`);
    if(Number(row?.confidence)<0||Number(row?.confidence)>1)reasons.push(`${name}:confidence:range`);
    if(!Array.isArray(row?.evidenceIds))reasons.push(`${name}:evidenceIds:type`);
  }
  return {valid:reasons.length===0,reasons};
}
function compactForContext(modelInput,{minConfidence=DEFAULT_CONTEXT_MIN_CONFIDENCE}={}){
  const check=validate(modelInput);
  if(!check.valid){const error=new Error('USER_PERFORMANCE_MODEL_INVALID');error.reasons=check.reasons;throw error;}
