...
function canonicalEvent(name,properties={}){
 ...
 if(key==='screen_viewed'){
  const page=clean(props.page,30);if(page)key=`screen_viewed:${page}`;
 }
...
function createTelemetryHandler({verifyIdToken,readConsent,writeEvent,clock=()=>new Date()}){
 ...
 return async function telemetry(request,response){
  ...
  let consent=false;try{consent=(await readConsent(uid))===true;}catch{return response.status(503).json({ok:false,error:{code:'CONSENT_UNAVAILABLE'}});}
  if(!consent)return response.status(202).json({ok:true,accepted:false,reason:'CONSENT_REQUIRED'});
  const kind=request.path?.includes('/errors')?'error':'analytics';
  if(kind==='analytics'){
   ...
   await writeEvent(uid,{kind,events,receivedAt:clock().toISOString()});return response.status(202).json({ok:true,accepted:true,count:events.length});
  }
  const event=safeError(request.body||{});await writeEvent(uid,{kind,event,receivedAt:clock().toISOString()});return response.status(202).json({ok:true,accepted:true,count:1});
 };
}
module.exports={canonicalEvent,safeError,createTelemetryHandler};
