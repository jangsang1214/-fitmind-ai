...
const VERSION='offline-policy-evaluation-v1.1.0';
...
function build(episodesInput={},options={}){
 const asOf=String(options.asOf||episodesInput?.asOf||new Date().toISOString().slice(0,10)),episodes=list(episodesInput?.episodes||episodesInput).filter(ep=>!ep?.date||String(ep.date)<=asOf).slice().sort((a,b)=>String(a?.date||'').localeCompare(String(b?.date||''))||String(a?.episodeId||'').localeCompare(String(b?.episodeId||''))),minHistory=Math.max(2,Math.min(12,Number(options.minHistory)||3)),rows=[],positiveScores=[],negativeScores=[];
