...
function minimalContext(full,state={}){
 const memory=rows(full?.memory?.entries).filter(row=>!directIdentifierKey(row?.key)&&String(row?.type||'').toLowerCase()!=='identity').slice(0,10).map(row=>select(row,['id','type','key','value','confidence','importance','createdAt'],{redact:true}));
...
function toolCallForExecution(row,index,context,requestId){
 ...
 if(['createPlan','updatePlan'].includes(call.name)){
  const recommendationId=clean(call.args?.recommendationId,180)||`coach:${requestId}`,episodeId=clean(call.args?.episodeId,220)||`episode:${recommendationId}`,selectedCandidate=clean(context?.personalizedLearning?.evaluation?.selectedCandidate?.id,100)||null,stateFeatures={readinessBand:clean(context?.stateIntelligence?.readiness?.band,40)||null,fatigueBand:clean(context?.stateIntelligence?.fatigue?.band,40)||null,loadBand:clean(context?.stateIntelligence?.load?.band,40)||null,goalBand:clean(context?.stateIntelligence?.goalAlignment?.band,40)||null},relevantMemoryIds=rows(context?.confirmedMemory).map(row=>clean(row?.id,180)).filter(Boolean).slice(0,12),knowledgeEvidenceIds=rows(context?.knowledgeGrounding?.evidence).map(row=>clean(row?.id||row?.ruleId||row?.key,180)).filter(Boolean).slice(0,12);
  call.args={...call.args,decisionId:clean(context?.garangDecision?.decisionId,320)||null,decisionMode:clean(context?.garangDecision?.mode,40)||null,recommendationId,episodeId,policyVersion:clean(context?.personalizationPolicy?.version,100)||null,candidateId:selectedCandidate,learningContext:{policyVersion:clean(context?.personalizationPolicy?.version,100)||null,responseModelVersion:clean(context?.personalizedLearning?.responseModel?.version,100)||null,candidateId:selectedCandidate,decisionConfidence:finite(context?.garangDecision?.confidence),decisionReasonCodes:rows(context?.decisionReasons).map(code=>clean(code,100)).filter(Boolean).slice(0,8),stateFeatures,goal:clean(context?.goal,160)||null,relevantMemoryIds,knowledgeEvidenceIds}};
 }
 return call;
}
