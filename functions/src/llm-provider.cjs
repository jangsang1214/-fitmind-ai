...
function createOpenAIProvider(options={}){
 const fetchImpl=options.fetchImpl||globalThis.fetch,apiKey=clean(options.apiKey,1000),model=clean(options.model||DEFAULT_MODEL,120),endpoint=clean(options.endpoint||'https://api.openai.com/v1/responses',500),timeoutMs=Math.max(500,Number(options.timeoutMs)||DEFAULT_TIMEOUT_MS),maxAttempts=Math.max(1,Math.min(3,Number(options.maxAttempts)||2)),retryDelayMs=Math.max(0,Number(options.retryDelayMs)||250),retryMalformed=options.retryMalformed===true,maxOutputTokens=Math.max(300,Math.min(1400,Number(options.maxOutputTokens)||650));
