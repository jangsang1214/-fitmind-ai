...
function createOpenAIProvider(options={}){
 ...
 return {name:'openai',model,async generate({message,context,language='ko',requestId,image}){
  ...
  const body={model,store:false,input:[{role:'system',content:[{type:'input_text',text:systemPrompt()}]},{role:'user',content:userContent}],max_output_tokens:950,text:{format:{type:'json_schema',name:'garang_coach_response',strict:true,schema:COACH_RESPONSE_SCHEMA}}};
  let lastError=null;
...
module.exports={DEFAULT_MODEL,DECISION_MODES,TOOL_NAMES,COACH_RESPONSE_SCHEMA,createProvider,createOpenAIProvider,parseCoachResponse,validateCoachResponse,normalizeToolCalls,extractResponseText,systemPrompt,retryableProviderError};
