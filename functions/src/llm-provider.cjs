...
const COACH_RESPONSE_SCHEMA=Object.freeze({type:'object',additionalProperties:false,required:['answer','decisionSummary','reasoningSummary','suggestedNextStep','actionIntent','confidence'],properties:{answer:{type:'string'},decisionSummary:{type:'string'},reasoningSummary:{type:'string'},suggestedNextStep:{type:'string'},actionIntent:{type:'object',additionalProperties:false,required:['type'],properties:{type:{type:'string',enum:['none','createPlan','updatePlan','askFollowup']}}},confidence:{type:['number','null'],minimum:0,maximum:1}}});
...
function systemPrompt(){return `You are the language/orchestration layer for GARANG Personal Performance Intelligence. GARANG's deterministic intelligence owns the decision. Explain the supplied GARANG decision faithfully; never replace, reverse, or invent a different training decision. Never claim to have changed user data. Never instruct direct database writes. Action intent is advisory only and requires separate user confirmation through GARANG's Agent Contract. Use only supplied context. If evidence is insufficient, say so.`;}
function createOpenAIProvider(options={}){
 ...
 return {name:'openai',model,async generate({message,context,language='ko',requestId}){
  ...
  try{
   const body={model,input:[{role:'system',content:[{type:'input_text',text:systemPrompt()}]},{role:'user',content:[{type:'input_text',text:JSON.stringify({language,message,garangContext:context})}]}],max_output_tokens:900,text:{format:{type:'json_schema',name:'garang_coach_response',strict:true,schema:COACH_RESPONSE_SCHEMA}}};
