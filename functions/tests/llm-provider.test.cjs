...
const valid=()=>JSON.stringify({answer:'오늘은 강도를 낮추세요.',decisionSummary:'REDUCE',reasoningSummary:'수면과 회복이 낮습니다.',suggestedNextStep:'볼륨을 낮춥니다.',confidence:.82,alignment:aligned(),toolCalls:[]});
(async()=>{
 ...
 await test('LLM Gateway provider accepts structured explanation success',async()=>{
  ...
  assert.equal(out.answer,'오늘은 강도를 낮추세요.');assert.equal(out.alignment.decisionMode,'reduce');assert.deepEqual(out.toolCalls,[]);assert.equal(out.actionIntent,undefined);assert.equal(out.metadata.provider,'openai');assert.equal(out.metadata.model,'model-a');assert.equal(out.metadata.attempts,1);
  assert.equal(request.url,'https://api.openai.com/v1/responses');assert.match(request.init.headers.Authorization,/Bearer secret/);const body=JSON.parse(request.init.body);assert.equal(body.model,'model-a');assert.equal(body.store,false);assert.equal(body.max_output_tokens,1800);assert.equal(body.reasoning?.effort,'none');assert.equal(body.text?.format?.type,'json_schema');assert.equal(body.text?.format?.strict,true);assert.equal(body.text?.format?.name,'garang_coach_response');assert.equal(body.text?.format?.schema?.properties?.actionIntent,undefined);assert.deepEqual(body.text?.format?.schema?.required,['answer','decisionSummary','reasoningSummary','suggestedNextStep','confidence','alignment','toolCalls']);assert.equal(body.text?.format?.schema?.properties?.toolCalls?.maxItems,4);assert.deepEqual(body.text?.format?.schema?.properties?.alignment?.required,['decisionId','decisionMode','reasonCodesUsed']);assert.equal(body.input?.[0]?.role,'system');assert.match(body.input?.[0]?.content?.[0]?.text||'',/make one concrete recommendation instead of refusing/);
 });
 ...
 await test('incomplete structured response retries once and can recover',async()=>{
  let calls=0;const provider=createOpenAIProvider({apiKey:'secret',retryDelayMs:0,fetchImpl:async()=>{calls++;if(calls===1)return {ok:true,json:async()=>({id:'resp_incomplete',status:'incomplete',incomplete_details:{reason:'max_output_tokens'},output:[]})};return response(valid());}});
  const out=await provider.generate({message:'x',context:{garangDecision:{decisionId:'d1',mode:'reduce'},decisionReasons:['SHORT_SLEEP']}});assert.equal(calls,2);assert.equal(out.metadata.attempts,2);
 });
 await test('malformed structured response retries once and can recover',async()=>{
  let calls=0;const provider=createOpenAIProvider({apiKey:'secret',retryDelayMs:0,fetchImpl:async()=>{calls++;return calls===1?response('not-json'):response(valid());}});
  const out=await provider.generate({message:'x',context:{garangDecision:{decisionId:'d1',mode:'reduce'},decisionReasons:['SHORT_SLEEP']}});assert.equal(calls,2);assert.equal(out.metadata.attempts,2);
 });
