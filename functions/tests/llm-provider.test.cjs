...
(async()=>{
 await test('LLM Gateway provider accepts structured success',async()=>{
 ...
 await test('unknown action intents cannot gain write authority',()=>{const out=parseCoachResponse(JSON.stringify({answer:'a',decisionSummary:'d',reasoningSummary:'r',suggestedNextStep:'s',actionIntent:{type:'deleteAccount'},confidence:4}));assert.equal(out.actionIntent.type,'none');assert.equal(out.confidence,1);});
