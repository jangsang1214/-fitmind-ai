...
(function(root){
 ...
 function migrate(input){
  ...
  s.scoreHistory=rows(s.scoreHistory).filter(x=>validDate(x.date)).map(x=>{const y={date:x.date};for(const k of ['total','exercise','nutrition','recovery','activity','body'])y[k]=numeric(x[k])===null?null:Math.max(0,Math.min(100,numeric(x[k])));return y;});
